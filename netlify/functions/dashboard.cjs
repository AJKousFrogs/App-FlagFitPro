...
const getDashboardData = async (userId) => {
  try {
    ...
    const totalTrainingHours = trainingSessions.reduce((sum, session) => sum + (session.duration || 0), 0) / 60;
    const avgScore = trainingSessions.length > 0 ? 
      trainingSessions.reduce((sum, session) => sum + (session.score || 0), 0) / trainingSessions.length : 0;
    ...
    const performanceValues = last7Days.map(date => {
      ...
      return sessionsOnDate.length > 0 ? 
        Math.round(sessionsOnDate.reduce((sum, s) => sum + (s.score || 70), 0) / sessionsOnDate.length) : 
    ...
    const thisWeekSessions = trainingSessions.filter(session => 
      new Date(session.completed_at) >= thisWeekStart
    );
    const weeklyTrainingHours = thisWeekSessions.reduce((sum, session) => sum + (session.duration || 0), 0) / 60;
    ...
    return {
      ...
      recentActivity: recentActivity.length > 0 ? recentActivity : getFallbackActivity(),
      performanceData: {
