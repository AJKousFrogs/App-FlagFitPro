...
async function generateProtocol(supabase, userId, payload, headers, log = logger) {
  ...
  const CNS_SPACING_HOURS =
    typeof context.age === "number" && context.age >= 40 ? 72
    : typeof context.age === "number" && context.age >= 35 ? 60
    : 48;
  ...
  const isNonTrainingIntent = ["rest", "travel", "competition"].includes(payload.intent);
