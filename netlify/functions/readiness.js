...
export default async (req) => {
  ...
  if (
    path.includes("/calc-readiness") ||
    path.includes("/readiness/calculate")
  ) {
  ...
  if (path.includes("/load-management")) {
    return dispatch(loadManagementHandler, req, url);
  }
  if (
    path.includes("/readiness-history") ||
    path.includes("/readiness/history")
  ) {
