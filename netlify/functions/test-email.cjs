    ...
    try {
      if (process.env.GMAIL_EMAIL && process.env.GMAIL_APP_PASSWORD) {
        ...
        return {
          ...
          body: JSON.stringify({
            configured: false,
            message: 'No email service configured. Please check your environment variables.',
            instructions: 'Add GMAIL_EMAIL + GMAIL_APP_PASSWORD or SENDGRID_API_KEY or SMTP credentials to .env file'
          })
        };
      }
      ...
      const mailOptions = {
        from: {
          name: 'FlagFit Pro',
          address: process.env.FROM_EMAIL || process.env.GMAIL_EMAIL || process.env.SMTP_USER
        },
        to: email,
