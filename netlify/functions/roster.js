...
export default async (req) => {
  ...
  if (path.includes("/player-stats")) {
    return dispatch(playerStatsHandler, req, url);
  }
  if (path.includes("/player-settings")) {
    return dispatch(playerSettingsHandler, req, url);
  }
  if (path.includes("/roster")) {
    return dispatch(rosterCoreHandler, req, url);
  }
  ...
  if (path.includes("/depth-chart")) {
    return new Response(
      JSON.stringify({
        ...
        message:
          "Depth chart feature is pending — supporting tables are not yet live.",
      }),
