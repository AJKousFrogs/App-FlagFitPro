 ...
 */
export async function getPrescriptionTemplate(supabase, {
  modality,
  intensityZone = null,
  positionGroup = null,
  ...
  const score = (row) => {
    let s = 0;
    if (positionGroup && row.position_group === positionGroup)       s += 4;
    if (periodizationPhase && row.periodization_phase === periodizationPhase) s += 2;
    if (intensityZone && row.intensity_zone === intensityZone)       s += 1;
    // Penalise mismatches on non-null DB cols vs null request
    if (row.position_group && !positionGroup)                        s -= 2;
    if (row.periodization_phase && !periodizationPhase)              s -= 1;
    return s;
  };

  const candidates = rows.filter((r) => {
    if (r.position_group && positionGroup && r.position_group !== positionGroup) return false;
    if (r.periodization_phase && periodizationPhase && r.periodization_phase !== periodizationPhase) return false;
    if (r.intensity_zone && intensityZone && r.intensity_zone !== intensityZone) return false;
    return true;
  });
