...
async function resolveTodaySession(supabase, userId, date) {
  ...
  if (date > todayIso) {
    return {
      ...
      override: null,
      reason:
        "Cannot resolve session for future dates. Only current or past dates allowed.",
      metadata,
    };
  }
  ...
  const override = await checkSportLayerOverrides(
    supabase,
    userId,
    date,
    dayOfWeek,
 ...
 * are applied in daily-protocol.js after team_activities resolution.
 *
 * @param {Object} supabase - Supabase client
 * @param {string} userId - User ID
