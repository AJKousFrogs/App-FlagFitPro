...
function isMerlinRequest(headers, userMetadata, token = null) {
  ...
  if (token) {
    try {
      ...
      if (parts.length === 3) {
        ...
        if (payload.role === 'merlin_readonly' || payload.aud === 'merlin_readonly') {
          return true;
        }
      }
    } catch (e) {
  ...
  if (userMetadata?.role === 'merlin' || userMetadata?.role === 'ai' || userMetadata?.role === 'merlin_readonly') {
    return true;
  }
  ...
  if (isMutationEndpoint(req.method, req.path)) {
    ...
    if (res && res.status) {
      return res.status(403).json({
        ...
        message: 'Merlin AI has read-only database access. All mutation endpoints are technically forbidden at the database role level.',
        endpoint: `${req.method} ${req.path}`,
        contract: 'Merlin AI Authority & Refusal Contract v1, Section 2.1',
        technical_note: 'merlin_readonly role lacks INSERT, UPDATE, DELETE privileges'
      });
    }
...
async function logMerlinViolation(req, violationType) {
  ...
  const merlinKey = process.env.MERLIN_READONLY_KEY;
  if (!merlinKey) {
    console.error('[Merlin Guard] MERLIN_READONLY_KEY not configured - cannot log violation');
