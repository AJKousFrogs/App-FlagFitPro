 ...
 */
async function resolveTeamActivityForAthleteDay(supabase, athleteId, teamId, dateLocal) {
  const audit = {
    athleteId,
    teamId: teamId || null,
    dateLocal,
  ...
  const hasActiveRehab = wellnessCheckin?.soreness_areas && 
  ...
  if (attendanceError) {
    audit.steps.push({ step: 'attendance_lookup', error: attendanceError.message });
  } else if (attendance) {
...
async function batchResolveTeamActivities(supabase, athleteId, teamId, dates) {
  ...
  await Promise.all(
    dates.map(async (date) => {
      const result = await resolveTeamActivityForAthleteDay(supabase, athleteId, teamId, date);
