 ...
 *
 * Routes: /api/player-programs, /api/micro-sessions, /api/decisions
 */
...
export default async (req) => {
  ...
  if (path.includes("/player-programs")) {
    return dispatch(playerProgramsHandler, req, url);
  }
  if (path.includes("/micro-sessions")) {
    return dispatch(microSessionsHandler, req, url);
  }
  if (path.includes("/decisions")) {
    return dispatch(decisionsHandler, req, url);
  }
  ...
  if (path.includes("/program-cycles")) {
    return new Response(
      JSON.stringify({
        ...
        message:
          "Program cycles feature is pending — supporting tables are not yet live.",
      }),
