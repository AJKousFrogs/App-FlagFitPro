...
async function getRecoveryMetrics(userId) {
  try {
    ...
    const recentLoad = (sessions || []).reduce((sum, s) => {
      return sum + (s.duration_minutes || 0) * (s.intensity_level === 'high' ? 1.5 : s.intensity_level === 'medium' ? 1 : 0.5);
    }, 0);
  ...
  const protocols = [
    {
      ...
      benefits: ['Reduces muscle soreness', 'Improves circulation', 'Maintains mobility']
    },
    {
      id: 'foam-rolling',
      name: 'Foam Rolling',
      ...
      benefits: ['Reduces muscle tension', 'Improves flexibility', 'Decreases soreness']
    },
    {
      id: 'stretching',
    ...
    },
    {
      ...
      benefits: ['Reduces inflammation', 'Speeds recovery', 'Decreases soreness']
    },
    {
      id: 'meditation',
    ...
    },
    {
      ...
      benefits: ['Improves recovery', 'Enhances performance', 'Reduces fatigue']
    }
  ];
  ...
  return [
    {
      ...
      summary: 'Research shows that 7-9 hours of quality sleep is crucial for recovery and performance.',
      source: 'Journal of Sports Sciences',
      relevance: 'high'
    },
    {
      title: 'Active Recovery Benefits',
      summary: 'Light exercise on rest days can improve recovery more than complete rest.',
      source: 'Sports Medicine Research',
      relevance: 'medium'
    },
    {
      title: 'Cold Therapy Timing',
      summary: 'Cold water immersion is most effective when done within 1 hour post-workout.',
      source: 'Recovery Science Review',
      relevance: 'medium'
    },
    {
      title: 'Stress and Recovery',
      summary: 'High stress levels can significantly impair recovery, even with adequate sleep.',
...
async function getProtocolEffectiveness(userId) {
  try {
    ...
    Object.keys(effectiveness).forEach(protocolId => {
      ...
      result[protocolId] = data.count > 0 
  ...
  try {
    ...
    switch (endpoint) {
      ...
      case 'start-session':
        ...
        const session = await startRecoverySession(userId, requestBody.protocolId);
