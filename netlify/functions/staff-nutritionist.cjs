...
async function getAthleteNutritionOverview(teamId) {
  ...
  for (const member of members || []) {
    ...
    athletes.push({
      ...
      bodyFat: measurements?.body_fat_percentage || nutritionProfile?.body_fat_percentage || null,
      leanMass: measurements?.muscle_mass_kg || nutritionProfile?.lean_mass_kg || null,
      ...
      lastUpdated: measurements?.measurement_date || nutritionProfile?.updated_at || null,
    });
  }
...
async function getTeamSupplementCompliance(teamId) {
  ...
  for (const member of members || []) {
    ...
    compliance.push({
      ...
      compliance: expectedDoses > 0 ? Math.round((takenCount / expectedDoses) * 100) : 0,
      takenCount,
...
async function getTeamHydrationSummary(teamId) {
  ...
  const { data: wellness } = await supabaseAdmin
    ...
    .or(`user_id.in.(${userIds.join(",")}),athlete_id.in.(${userIds.join(",")})`)
...
async function generateNutritionReport(userId, reportType = "weekly") {
  ...
  const avgHydration = hydration?.length
    ? hydration.reduce((sum, h) => sum + (h.fluid_ml || 0), 0) / hydration.length
    : 0;

  const weightChange = measurements?.length >= 2
    ? measurements[measurements.length - 1].weight_kg - measurements[0].weight_kg
    : 0;
  ...
  return {
    ...
    profile: profile || {},
    metrics: {
      currentWeight: measurements?.[measurements.length - 1]?.weight_kg || profile?.weight_kg,
      weightChange,
      bodyFatChange: measurements?.length >= 2
        ? (measurements[measurements.length - 1].body_fat_percentage || 0) -
          (measurements[0].body_fat_percentage || 0)
        : 0,
      supplementCompliance: supplementsExpected > 0
        ? Math.round((supplementsTaken / supplementsExpected) * 100)
        : 0,
      avgDailyHydration: Math.round(avgHydration),
    },
    recommendations: generateRecommendations(profile, weightChange, avgHydration),
  };
}
...
function generateRecommendations(profile, weightChange, avgHydration) {
  ...
  if (profile?.primary_goal === "muscle_gain" && weightChange < 0) {
    recommendations.push({
      type: "nutrition",
      priority: "medium",
      message: "Consider increasing protein and overall calories for muscle gain",
    });
  }
...
async function handler(event) {
  return baseHandler(event, async (event, userId) => {
    ...
    if (!access) {
      return createErrorResponse(403, "Access denied. Nutritionist role required.");
    }
    ...
    if (method === "GET" && path === "/summary") {
      ...
      const avgCompliance = supplements.length
        ? Math.round(supplements.reduce((sum, s) => sum + s.compliance, 0) / supplements.length)
        : 0;
