...
async function completeTrainingSession(userId, sessionId, completionData) {
  try {
    ...
    const workload = completionData.workload || Math.round((duration * intensity) / 10);
    ...
    const updateData = {
      ...
      ...(completionData.duration && { duration_minutes: completionData.duration }),
      ...(completionData.intensity && { intensity_level: completionData.intensity }),
      // Store completion notes if provided
      ...(completionData.notes && { notes: `${session.notes || ''}\n\nCompleted: ${completionData.notes}` }),
...
async function handleRequest(event, context, { userId }) {
  try {
    ...
    if (!result.success) {
      return createErrorResponse(result.error || 'Failed to complete training session', 400);
    }
