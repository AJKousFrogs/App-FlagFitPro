...
async function getCoachDashboard(userId) {
  try {
    ...
    for (const member of teamMembers) {
      try {
        ...
        if (sessions && sessions.length > 0) {
          const acute = sessions.slice(0, 7).reduce((sum, s) => sum + (s.workload || 0), 0);
          const chronic = sessions.length >= 14 
            ? sessions.slice(0, 14).reduce((sum, s) => sum + (s.workload || 0), 0) / 2
            : acute;
    ...
    return {
      ...
      avgWorkload: squadMembers.length > 0
        ? squadMembers.reduce((sum, m) => sum + m.workload, 0) / squadMembers.length
        : 0,
...
async function getTrainingAnalytics(userId, coachId) {
  try {
    ...
    const totalWorkload = sessions.reduce((sum, s) => sum + (s.workload || 0), 0);
...
async function createTrainingSession(userId, sessionData) {
  try {
    const session = {
      ...
      session_date: sessionData.sessionDate || sessionData.session_date || new Date().toISOString(),
      session_type: sessionData.sessionType || sessionData.session_type || 'training',
      workload: sessionData.workload || 0,
      duration_minutes: sessionData.durationMinutes || sessionData.duration_minutes || 60,
...
async function getGames(userId, coachId) {
  try {
    ...
    return (games || []).map(game => ({
      ...
      opponent_name: game.home_team_id === teamId 
        ? (game.away_team_name || 'TBD')
        : (game.home_team_name || 'TBD'),
  ...
  try {
    ...
    switch (endpoint) {
      ...
      case '/health':
        ...
        return createSuccessResponse({ status: 'healthy', service: 'coach-api' });
