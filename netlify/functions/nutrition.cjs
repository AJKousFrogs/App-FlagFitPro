...
function getDefaultGoals() {
  return [
    ...
    { nutrient: 'Carbohydrates', current: 0, target: 300, unit: 'g', priority: 'medium' },
    { nutrient: 'Fat', current: 0, target: 80, unit: 'g', priority: 'medium' },
    { nutrient: 'Fiber', current: 0, target: 30, unit: 'g', priority: 'medium' },
    { nutrient: 'Sodium', current: 0, target: 2300, unit: 'mg', priority: 'low' }
  ];
}
    ...
    mealTypes.forEach(type => {
      ...
      if (mealFoods.length > 0) {
        meals.push({
          ...
          totalCalories: mealFoods.reduce((sum, f) => sum + (f.calories || 0), 0)
        });
      }
    });
...
async function getAINutritionSuggestions(userId) {
  try {
    ...
    const proteinGoal = goals.find(g => g.nutrient === 'Protein')?.target || 150;
    const currentProtein = meals.reduce((sum, meal) => {
      return sum + meal.foods.reduce((s, f) => s + (f.nutrients?.protein || 0), 0);
    }, 0);
    ...
    if (meals.length < 3) {
      suggestions.push({
        name: 'Add More Meals',
        benefit: 'Eating 4-5 smaller meals helps maintain energy levels throughout the day.',
        priority: 'medium'
      });
    }
    ...
    suggestions.push({
      name: 'Stay Hydrated',
      benefit: 'Drink water throughout the day, especially around training sessions.',
      priority: 'medium'
    });
...
async function getPerformanceInsights(userId) {
  try {
    ...
    if (sessions && sessions.length > 0) {
      const avgScore = sessions.reduce((sum, s) => sum + (s.score || 70), 0) / sessions.length;
    ...
    if (!hasPreWorkoutMeal && meals.length > 0) {
      insights.push({
        ...
        message: 'Consider eating a meal 1-3 hours before training for optimal performance.',
        type: 'info',
        actionable: true
      });
    }
