...
async function logSync(supabase, source, result, details) {
  try {
    await supabase.from('sync_logs').insert({
      source,
      result: details.hasErrors ? (details.recordsAdded > 0 ? 'partial' : 'failure') : 'success',
      severity: details.hasErrors ? (details.recordsAdded > 0 ? 'warning' : 'error') : 'success',
...
async function handleSync(event) {
  ...
  try {
    ...
    if (mode === 'search' && searchQuery) {
      ...
      console.log(`Starting ${mode} sync, pages: ${maxPages}, pageSize: ${pageSize}`);
