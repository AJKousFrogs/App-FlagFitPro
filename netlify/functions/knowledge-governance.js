...
function getQualityIssues(entry) {
  ...
  if (entryType === "nutrition" || entryType === "supplement") {
    ...
    if (!hasSafetySignal) {
      issues.push(
        "Nutrition/supplement entries must include safety considerations",
      );
    }
  }
...
const handler = async (event, context) =>
  baseHandler(event, context, {
    ...
    handler: async (event, _ctx, requestContext) => {
      ...
      const path =
        event.path.replace("/.netlify/functions/knowledge-governance", "") ||
        "/";
      ...
      if (event.httpMethod === "GET") {
        if (pathParts[0] === "audit" && pathParts[1]) {
          ...
          const { data: targetEntry, error: targetEntryError } =
            await supabaseAdmin
        ...
        if (pathParts[0] === "my") {
          ...
          return createSuccessResponse(
            { entries: data || [], total: (data || []).length },
            requestId,
          );
        }
      ...
      if (event.httpMethod === "POST" && pathParts[0] !== "review") {
        ...
        const entryType = normalizeText(
          body.entry_type || "nutrition",
        ).toLowerCase();
        const evidenceStrength = normalizeText(
          body.evidence_strength || "limited",
        ).toLowerCase();
        const consensusLevel = normalizeText(
          body.consensus_level || "low",
        ).toLowerCase();
      ...
      if (
        ...
        const { data: reviewData, error: reviewError } = await supabaseAdmin
          ...
          .eq("id", entryId)
          .select(
            "id, topic, merlin_approval_status, is_merlin_approved, merlin_approved_by_role",
          )
          .single();
        ...
        const { error: auditError } = await supabaseAdmin
          ...
          .insert({
            ...
            quality_gate_override:
              action === "approve" ? overrideQualityGate : false,
            quality_issues:
              action === "approve" ? getQualityIssues(existingEntry) : [],
          });
        if (auditError) {
          logger.error("review_audit_write_failed", auditError, {
            entry_id: reviewData.id,
          });
        }
