...
-- Migration: 20260508120000_competition_schedule_spine.sql
-- Purpose:
--   Canonical competition + event model that feeds periodization, readiness,
--   nutrition, and recovery. Replaces the fragmented mix of `tournaments`,
--   `fixtures`, `games`, and per-feature ad-hoc schedule logic for read paths.
--
-- Design:
--   competitions       -- shared registry (Slovenian League 2026, Big Bowl 2026 …)
--   competition_events -- per-(competition × team × date) calendar slot, with
--                         expected_game_count for density-aware load planning.
--   v_athlete_schedule -- read view: union across an athlete's active teams.
--
-- Coexists with the legacy `tournaments` / `games` tables; legacy writes still
-- work, but new athlete-facing reads should use this spine.
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS idx_competitions_kind
  ON public.competitions (kind);
CREATE INDEX IF NOT EXISTS idx_competitions_season_year
  ON public.competitions (season_year);
CREATE INDEX IF NOT EXISTS idx_competitions_country
  ON public.competitions (country);
CREATE INDEX IF NOT EXISTS idx_competitions_starts_on
  ON public.competitions (starts_on);

-- =============================================================================
-- COMPETITION EVENTS (per team × date calendar slot)
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS idx_comp_events_team_starts
  ON public.competition_events (team_id, starts_at);
CREATE INDEX IF NOT EXISTS idx_comp_events_competition
  ON public.competition_events (competition_id);
CREATE INDEX IF NOT EXISTS idx_comp_events_starts_at
  ON public.competition_events (starts_at);
CREATE INDEX IF NOT EXISTS idx_comp_events_status
  ON public.competition_events (status);
...
-- Competitions: readable by all authenticated users (shared registry)
...
-- Authenticated users can register a competition (federation import or manual).
-- Updates limited to creator; in a later pass this should expand to "team staff
-- of any team participating", but for the first cut we keep it tight.
...
-- Competition events: read if active member of the team, write if team staff
...
-- Single canonical read shape for "what's on my calendar" — union across all
-- active team memberships. Server reads should select FROM here.
-- =============================================================================
