...
const DOC_RIGHTS = {
  practitioners_guide_calf_achilles: {
    source_url:
      "https://resources.valdperformance.com/practitioners-guide-to-the-calf-achilles-complex",
    licence_expression: "LicenseRef-Proprietary",
    permitted_use: "internal_only",
  },
  practitioners_guide_hamstrings: {
    source_url:
      "https://resources.valdperformance.com/practitioners-guide-to-hamstrings",
    licence_expression: "LicenseRef-Proprietary",
    permitted_use: "internal_only",
  },
  practitioners_guide_shoulders: {
    source_url:
      "https://resources.valdperformance.com/practitioners-guide-to-shoulders",
    licence_expression: "LicenseRef-Proprietary",
    permitted_use: "internal_only",
  },
  practitioners_guide_hip_groin: {
    source_url:
      "https://resources.valdperformance.com/practitioners-guide-to-hip-groin",
    licence_expression: "LicenseRef-Proprietary",
    permitted_use: "internal_only",
  },
...
const QUANTITATIVE_CLAIMS = {
  calf_achilles_assessment_protocols: [
    {
      ...
      operator: ">=",
      value_range: { min: 10, max: 15 },
      unit: "percent",
      population_context: "athletes",
      interpretation: "commonly_used_often_arbitrary_prefer_MDC_and_context",
      source_ref: "systematic_review_2022",
    },
  ],
  calf_achilles_rehabilitation: [
    {
      ...
      value_range: {},
      unit: "multidomain",
      population_context: "return_to_sport",
      interpretation:
        "strength_function_sport_specific_symptom_response_per_CPG",
    },
  ],
  ...
  soleus_gastrocnemius_differentiation: [
    {
      metric: "soleus_RTP_days",
      operator: "=",
      value_range: { mean: 25.4, SD: 16.2 },
      unit: "days",
      population_context: "AFL_registry",
      interpretation: "mean_SD",
    },
    {
      metric: "gastrocnemius_RTP_days",
      operator: "=",
      value_range: { mean: 19.1, SD: 14.1 },
      unit: "days",
      population_context: "AFL_registry",
      interpretation: "mean_SD",
    },
  ...
  hamstring_injuries_pathologies: [
    {
      metric: "reinjury_rate",
      operator: ">=",
      value_range: { min: 12, max: 30 },
      unit: "percent",
      population_context: "hamstring_strain",
      interpretation: "reported_range",
    },
    {
      metric: "bilateral_asymmetry",
      operator: ">=",
      value_range: { value: 15 },
      unit: "percent",
      population_context: "risk_factor",
      interpretation: "flag_for_investigation",
    },
  ],
  hamstring_injury_prevention: [
    {
      metric: "nordic_curl_risk_reduction",
      operator: ">=",
      value_range: { min: 50, max: 70 },
      unit: "percent",
      population_context: "injury_prevention",
      interpretation: "reported_benefit",
    },
    {
      metric: "bilateral_asymmetry",
      operator: ">=",
      value_range: { value: 10 },
      unit: "percent",
      population_context: "screening",
      interpretation: "flag_threshold",
    },
  ],
  flag_football_training_load_monitoring: [
    {
      metric: "load_spike_threshold",
      operator: ">=",
      value_range: { min: 10, max: 15 },
      unit: "percent",
      population_context: "week_to_week",
      interpretation: "injury_risk_increase",
    },
  ],
  flag_football_acwr_interpretation: [
    {
      metric: "acwr_sweet_spot",
      operator: ">=",
      value_range: { min: 0.8, max: 1.3 },
      unit: "ratio",
      population_context: "load_management",
      interpretation: "optimal_range",
    },
    {
      metric: "acwr_high_risk",
      operator: ">",
      value_range: { value: 1.5 },
      unit: "ratio",
      population_context: "load_management",
      interpretation: "high_risk_threshold",
    },
  ],
};
...
function buildRights(doc, ra) {
  const key =
    Object.keys(DOC_RIGHTS).find((k) => doc.includes(k)) ||
    "practitioners_guide_calf_achilles";
...
function upgradeEntry(entry, idx, docId) {
  const entryId =
    entry.entry_id ||
    uuidFromString(`${docId}:${entry.topic}:${entry.question}`);
  ...
  if (
    !upgraded.evidence_framework &&
    (upgraded.evidence_strength || upgraded.consensus_level)
  ) {
...
function upgradeFile(filename) {
  ...
  const upgraded = {
    ...
    research_article: ra,
    knowledge_base_entries: (content.knowledge_base_entries || []).map((e, i) =>
      upgradeEntry(e, i, documentId),
    ),
  };

  writeFileSync(path, JSON.stringify(upgraded, null, 2), "utf8");
  console.log(
    `  Upgraded: ${filename} (${upgraded.knowledge_base_entries.length} entries)`,
  );
}
