...
--radius-full: 9999px;
```
**Migration:** 10px → 8px or 12px | 14px → 12px or 16px
...
- Default shape: `--radius-lg`
- Pill shape: `--radius-full` (only when intentional)
- `[rounded]="true"` is **not** default
- `raised` allowed for **one primary CTA per view**
...
// Module types
.bento-item--metric { /* 3 cols */ }
.bento-item--chart { /* 6 cols */ }
.bento-item--list { /* 4 cols */ }
```
...
> These decisions govern *behavior*, not just appearance. High UX impact, low effort.
...
```scss
// ❌ One-off utilities
.workout-card-special-padding { padding: 1.5rem; }
.dashboard-flex-thing { display: flex; gap: 12px; }

// ✅ Use recipes or existing utilities
.card { /* recipe */ }
.flex.gap-3 { /* existing utilities */ }
```
...
--font-family-sans-latin: "Poppins", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;

/* Japanese-optimized */
--font-family-sans-jp: "Noto Sans JP", "Hiragino Kaku Gothic ProN", "Yu Gothic", "Meiryo", sans-serif;
...
.tab { min-width: 80px; max-width: 200px; }
...
<i class="pi pi-star"></i> <!-- Using star when you need "favorite workout" -->
...
**CORE Decisions Approved by:** ________________________  
**Date:** ________________________

**OPERATIONAL Decisions Approved by:** ________________________  
**Date:** ________________________
