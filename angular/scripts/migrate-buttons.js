...
function migrateFile(filePath, dryRun = false) {
  ...
  if (content.includes('import { ButtonModule } from "primeng/button"') || 
    ...
    const needsIconButton = /<p-button[^>]*icon="[^"]*"[^>]*(?:aria-?label|ariaLabel)/.test(content) && 
                           !/<p-button[^>]*label=/.test(content);
    
    if (!hasButtonComponent) {
      // Find the right place to add the import
      const importMatch = content.match(/import \{[^}]*\} from ['"]primeng\/button['"]/);
      if (importMatch) {
        let newImport = 'import { ButtonComponent } from "../../shared/components/button/button.component";\n';
        if (needsIconButton && !hasIconButtonComponent) {
          newImport += 'import { IconButtonComponent } from "../../shared/components/button/icon-button.component";\n';
        }
    ...
    if (!content.includes("ButtonComponent,") && !content.includes("ButtonComponent\n")) {
      content = content.replace(
        /imports:\s*\[([^\]]*)\]/s,
        (match, imports) => {
  ...
  const pButtonRegex = /<p-button([^>]*)(?:\/>|><\/p-button>|>\s*<\/p-button>)/g;
  ...
  content = content.replace(pButtonRegex, (match, attrs) => {
    ...
    if (icon && !label) {
      ...
      const iconButtonAttrs = newAttrs.filter(a => !a.startsWith("iconLeft="));
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        if (result.modified) {
          ...
          console.log(`${action}: ${relativePath} (${result.replacementCount} replacements)`);
        }
      } catch (error) {
...
function main() {
  ...
  console.log(`Files ${dryRun ? "to be modified" : "modified"}: ${totalFilesModified}`);
