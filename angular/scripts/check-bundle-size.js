...
function checkBundleSizes() {
  console.log(`${COLORS.bold}${COLORS.cyan}📦 FlagFit Pro Bundle Size Report${COLORS.reset}\n`);
  
  if (!fs.existsSync(DIST_DIR)) {
    console.log(`${COLORS.red}❌ Build directory not found: ${DIST_DIR}${COLORS.reset}`);
  ...
  jsBundles.forEach(({ file, size, sizeKb, category }) => {
    ...
    if (sizeKb > limit) {
      ...
      warnings.push(`${file} approaching ${limit}KB limit (${formatSize(size)})`);
    }
    
    console.log(`  ${status} ${file.padEnd(40)} ${formatSize(size).padStart(12)}`);
  });
  ...
  cssFiles.forEach(file => {
    ...
    const size = getFileSize(filePath);
    totalCssSize += size;
    console.log(`  ${COLORS.green}✓${COLORS.reset} ${file.padEnd(40)} ${formatSize(size).padStart(12)}`);
  });
