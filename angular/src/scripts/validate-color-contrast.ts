...
function validateContrast(test: ColorTest): ContrastResult {
  ...
  const requiredRatioAAA = test.isUIComponent ? 4.5 : (test.isLargeText ? 4.5 : 7);
...
passed.forEach(result => {
  ...
  console.log(`   FG: ${result.test.foreground} / BG: ${result.test.background}\n`);
});
...
if (failed.length > 0) {
  ...
  failed.forEach(result => {
    console.log(`❌ ${result.test.name}`);
    console.log(`   Ratio: ${result.ratio.toFixed(2)}:1 (Required: ${result.test.isUIComponent ? '3' : '4.5'}:1)`);
    console.log(`   FG: ${result.test.foreground} / BG: ${result.test.background}`);
...
console.log(`✅ Compliance: ${failed.length === 0 ? 'WCAG 2.1 AA ✅' : 'Needs fixes ❌'}`);
