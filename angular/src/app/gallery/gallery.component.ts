...
@Component({
  ...
  template: `
    ...
    <main class="screen">
      <div class="strip">
        <span class="inline"
          ><lucide-icon name="flag" /> 2d → Big Bowl ·
          <lucide-icon name="cloud-rain" /> 12°C rain</span
        >
      ...
      <div class="card stack">
        ...
        <button class="btn primary block">
          <lucide-icon name="plus" /> Block button
        </button>
        <button class="btn ghost sm">
          <lucide-icon name="plus" /> Small ghost
        </button>
      </div>
        ...
        <div class="row">
          <div>
            ...
            <p class="muted" style="margin:var(--s-2) 0 0">
              Physio block — right hamstring. RPE 5, no sprints.
            </p>
          </div>
          <div class="gauge grad" style="--p:72">
            <div class="inner"><b>72</b><small>ready</small></div>
          </div>
        </div>
        <div class="tiles">
      ...
      <div class="card">
        <div class="lrow">
          <span id="g-cr"
            ><lucide-icon name="pill" class="suppi-i" /> Creatine
            <small>5 g</small></span
          >
          <button
            class="sw"
            role="switch"
            [attr.aria-checked]="creatine()"
            aria-labelledby="g-cr"
            (click)="creatine.set(!creatine())"
          ></button>
        </div>
        <div class="lrow">
          <span id="g-cf"
            ><lucide-icon name="coffee" class="suppi-i" /> Caffeine
            <small>200 mg · pre-session</small></span
          >
          <button
            class="sw"
            role="switch"
            [attr.aria-checked]="caffeine()"
            aria-labelledby="g-cf"
            (click)="caffeine.set(!caffeine())"
          ></button>
        </div>
        <div class="lrow">
          <span id="g-ba"
            ><lucide-icon name="shield-half" class="suppi-i" /> Beta-alanine
            <small>4 g</small></span
          >
          <button
            class="sw"
            role="switch"
            [attr.aria-checked]="beta()"
            aria-labelledby="g-ba"
            (click)="beta.set(!beta())"
          ></button>
        </div>
      </div>
      ...
      <div class="section-h">
        <h2>Session video (YouTube IFrame player)</h2>
      </div>
      <app-yt-video
        videoId="M7lc1UVf-VE"
        poster="assets/images/rebuild/video-session.jpg"
        title="Tap to play"
        duration="0:15"
      />
      ...
      >
        ...
        <div style="flex:1">
          ...
          <div class="muted" style="font-size:var(--fs-sm)">
            Restock at La Prima Fit
          </div>
        </div>
        <lucide-icon name="arrow-up-right" class="lc-go" />
      </a>
      <div class="empty">
        No wellness log yet — check in to calculate readiness.
      </div>
    </main>
  `,
  styles: [
