...
export const debugInterceptor: HttpInterceptorFn = (
  ...
  console.log('Request:', {
    ...
    body: req.body,
    params: req.params.keys().map(key => ({ key, value: req.params.get(key) })),
  });
  ...
  return next(req).pipe(
    tap((event) => {
      if (event instanceof HttpResponse) {
        ...
        console.log('Response:', {
          ...
          headers: event.headers.keys().map(key => ({ key, value: event.headers.get(key) })),
          body: event.body,
    ...
    catchError((error: HttpErrorResponse) => {
      ...
      // Track in debug service
      debugService.logApiCall(req.url, req.method, error.status, duration, error);
