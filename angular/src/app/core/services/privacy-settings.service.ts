...
export type EmergencySharingLevel = 'none' | 'medical_only' | 'medical_and_location' | 'full';
...
export const METRIC_CATEGORIES = [
  { value: 'performance', label: 'Performance Metrics', description: 'Speed, agility, strength scores' },
  { value: 'training_load', label: 'Training Load', description: 'ACWR, session RPE, volume' },
  { value: 'readiness', label: 'Readiness Scores', description: 'Daily readiness assessments' },
  { value: 'wellness', label: 'Wellness Data', description: 'Sleep, stress, recovery' },
  { value: 'injury_history', label: 'Injury History', description: 'Past injuries and recovery' },
  { value: 'body_composition', label: 'Body Composition', description: 'Weight, body fat, measurements' },
] as const;
...
export class PrivacySettingsService {
  ...
  // Computed signals
  readonly aiProcessingEnabled = computed(() => this._settings()?.aiProcessingEnabled ?? true);
  readonly researchOptIn = computed(() => this._settings()?.researchOptIn ?? false);
  readonly emergencySharingLevel = computed(() => this._settings()?.emergencySharingLevel ?? 'medical_only');
  readonly hasParentalConsent = computed(() => this._parentalConsent()?.status === 'verified');
  ...
  async loadSettings(): Promise<void> {
    ...
    try {
      ...
      if (error && error.code === 'PGRST116') {
        ...
        const { data: newSettings, error: insertError } = await this.supabase.client
          .from('privacy_settings')
  ...
  private async loadTeamSettings(): Promise<void> {
    ...
    try {
      ...
      const teamSettingsMap = new Map(settings?.map(s => [s.team_id, s]) || []);
      ...
      const teamSettings: TeamSharingSettings[] = memberships.map(m => {
        ...
        const teamsData = m.teams as { id: string; name: string } | { id: string; name: string }[] | null;
        ...
        return {
          ...
          performanceSharingEnabled: existing?.performance_sharing_enabled ?? false,
   ...
   */
  async updateEmergencyContacts(contacts: EmergencyContact[]): Promise<boolean> {
    return this.updateSetting({
   ...
   */
  async updateDataSharingDefaults(performance: boolean, health: boolean): Promise<boolean> {
    return this.updateSetting({
   ...
   */
  private async updateSetting(updates: Record<string, unknown>): Promise<boolean> {
    const userId = this.authService.getUser()?.id;
    if (!userId) {
  ...
  async updateTeamSharing(
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to update team sharing';
      this.toastService.error(message);
   ...
   */
  async requestParentalConsent(guardianEmail: string, guardianName?: string): Promise<boolean> {
    const userId = this.authService.getUser()?.id;
    if (!userId) {
      this.toastService.error('Not authenticated');
      return false;
  ...
  async getAiConsentStatus(): Promise<{
    ...
    try {
      const { data, error } = await this.supabase.client
        .rpc('get_ai_consent_status', { p_user_id: userId });
  ...
  async requireAiConsent(): Promise<void> {
    ...
    try {
      const { error } = await this.supabase.client
        .rpc('require_ai_consent', { p_user_id: userId });

      if (error) {
        // Check if it's our custom consent error
        if (error.message.includes('AI_CONSENT_REQUIRED')) {
          throw new Error('AI processing is disabled. Enable AI processing in Privacy Settings to use this feature.');
        }
        throw error;
      }
    } catch (err) {
      if (err instanceof Error && err.message.includes('AI processing is disabled')) {
        throw err;
      }
  ...
  async canCoachAccessPlayerData(
    ...
    try {
      const functionName = dataType === 'performance' 
  ...
  async isMetricCategoryAllowed(
    ...
    try {
      const { data, error } = await this.supabase.client
        .rpc('check_metric_category_allowed', {
          p_player_id: playerId,
          p_team_id: teamId,
          p_category: category,
        });
   ...
   */
  hasParentalConsentFor(feature: 'health' | 'biometrics' | 'location' | 'research'): boolean {
    const consent = this._parentalConsent();
    if (!consent || consent.status !== 'verified') return false;

    switch (feature) {
      case 'health': return consent.healthDataConsent;
      case 'biometrics': return consent.biometricsConsent;
      case 'location': return consent.locationConsent;
      case 'research': return consent.researchConsent;
  ...
  async getConsentAwareLoadMonitoring(playerId?: string): Promise<{
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to fetch load monitoring data';
  ...
  private mapDbToSettings(db: Record<string, unknown>): PrivacySettings {
    return {
      ...
      aiProcessingConsentDate: db['ai_processing_consent_date'] as string | null,
      researchOptIn: db['research_opt_in'] as boolean,
      researchConsentDate: db['research_consent_date'] as string | null,
      emergencySharingLevel: db['emergency_sharing_level'] as EmergencySharingLevel,
