...
export class TrainingVideoService {
  ...
  async load(): Promise<void> {
    try {
      const { data } = await this.supabase.client
        ...
        .select(
          "id,title,description,video_url,thumbnail_url,duration_seconds,category,position,difficulty_level,team_id,is_active",
        )
        .eq("is_active", true)
        .order("created_at", { ascending: false });
      this.videos.set(
        (data ?? []).map((r) => this.map(r as Record<string, unknown>)),
      );
    } catch {
      this.videos.set([]);
    } finally {
  ...
  first(category: string): TrainingVideo | null {
    return (
      this.videos().find((v) => v.category === category && v.youtubeId) ?? null
    );
  }

  /** First library video matching a prescription intent (for the session video). */
  forIntent(
    intent: PrescriptionIntent | null | undefined,
  ): TrainingVideo | null {
