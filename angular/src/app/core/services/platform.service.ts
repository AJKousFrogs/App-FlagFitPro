  ...
  getLocalStorage(key: string): string | null {
    if (this.isBrowser) {
      try {
        ...
        this.logger.debug("[PlatformService] localStorage read failed, using memory fallback", error);
      }
    }
  ...
  setLocalStorage(key: string, value: string): boolean {
    if (this.isBrowser) {
      try {
        ...
        this.logger.debug("[PlatformService] localStorage write failed, using memory fallback", error);
      }
    }
  ...
  removeLocalStorage(key: string): boolean {
    if (this.isBrowser) {
      try {
        ...
        this.logger.debug("[PlatformService] localStorage remove failed, using memory fallback", error);
      }
    }
  ...
  clearLocalStorage(): boolean {
    if (this.isBrowser) {
      try {
        ...
        this.logger.debug("[PlatformService] localStorage clear failed, using memory fallback", error);
      }
    }
  ...
  getSessionStorage(key: string): string | null {
    if (this.isBrowser) {
      try {
        ...
        this.logger.debug("[PlatformService] sessionStorage read failed, using memory fallback", error);
      }
    }
  ...
  setSessionStorage(key: string, value: string): boolean {
    if (this.isBrowser) {
      try {
        ...
        this.logger.debug("[PlatformService] sessionStorage write failed, using memory fallback", error);
      }
    }
  ...
  removeSessionStorage(key: string): boolean {
    if (this.isBrowser) {
      try {
        ...
        this.logger.debug("[PlatformService] sessionStorage remove failed, using memory fallback", error);
      }
    }
