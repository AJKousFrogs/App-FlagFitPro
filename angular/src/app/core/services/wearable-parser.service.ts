...
export class WearableParserService {
   ...
   */
  async parseFile(file: File, options: ParseOptions = {}): Promise<ParsedWearableData> {
    ...
    switch (extension) {
      ...
      default:
        throw new Error(`Unsupported file format: ${extension}. Supported: CSV, JSON, XML`);
    }
  }
   ...
   * Parse CSV file
   */
  private async parseCSV(file: File, options: ParseOptions): Promise<ParsedWearableData> {
    ...
    const speedCol = this.findColumn(headers, ['speed', 'velocity', 'pace', 'speed_m_s', 'speed_ms']);
    const distanceCol = this.findColumn(headers, ['distance', 'dist', 'distance_m', 'distance_meters', 'total_distance']);
   ...
   * Parse JSON file
   */
  private async parseJSON(file: File, options: ParseOptions): Promise<ParsedWearableData> {
    const text = await file.text();
    let jsonData: any;
    ...
    if (Array.isArray(jsonData)) {
      ...
      throw new Error('JSON structure not recognized. Expected array or object with data/samples/records array');
    }
   ...
   * Parse XML file
   */
  private async parseXML(file: File, options: ParseOptions): Promise<ParsedWearableData> {
    const text = await file.text();
    ...
    const samples = xmlDoc.querySelectorAll('sample, record, point, data, trackpoint');
    const data: Array<{ speed_m_s: number; distance_m: number }> = [];
    const errors: string[] = [];
    ...
    samples.forEach((sample, index) => {
      try {
        ...
        if (speedEl) {
          speed = this.normalizeSpeed(parseFloat(speedEl.textContent || '0'), speedEl.tagName);
        }

        // Extract distance
        const distanceEl = sample.querySelector('distance, dist');
        if (distanceEl) {
          distance = this.normalizeDistance(parseFloat(distanceEl.textContent || '0'), distanceEl.tagName);
        }
  ...
  private extractSpeed(obj: any): number {
    const speedFields = ['speed_m_s', 'speed', 'velocity', 'pace', 'speed_ms', 'speedMs'];
  ...
  private extractDistance(obj: any): number {
    const distanceFields = ['distance_m', 'distance', 'dist', 'distance_meters', 'distanceMeters', 'total_distance'];
   ...
   */
  private calculateMissingValue(speed: number, distance: number): { speed: number; distance: number } {
  ...
  private normalizeDistance(value: number, fieldName: string): number {
    ...
    // Already in meters
    if (field.includes('distance_m') || field.includes('meters') || field.includes('metres')) {
      return value;
    }
