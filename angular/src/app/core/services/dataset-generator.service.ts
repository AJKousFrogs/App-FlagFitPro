...
export class DatasetGeneratorService {
  ...
  generateDataset(options: DatasetOptions = {}): GeneratedDataset {
    ...
    const cooldownSamples = includeCooldown
      ? Math.floor(totalSamples * 0.1)
      : 0;
    ...
    const totalDistance = data.reduce(
      (sum, entry) => sum + entry.distance_m,
      0,
    );
    const avgSpeed =
      data.reduce((sum, entry) => sum + entry.speed_m_s, 0) / data.length;
    const maxSpeed = Math.max(...data.map((entry) => entry.speed_m_s));
    const sprintCount = data.filter(
      (entry) => entry.speed_m_s >= this.SPRINT_THRESHOLD,
    ).length;
  ...
  generateWeeklyDataset(
    athleteProfile: "recreational" | "competitive" | "elite" = "competitive",
  ): GeneratedDataset[] {
    const sessions = [
