...
export class AchievementsService {
  ...
  readonly totalPoints = computed(
    () => this.achievementsData()?.totalPoints || 0,
  );
  readonly progress = computed(() => this.achievementsData()?.progress || 0);
  readonly unlockedCount = computed(
    () => this.achievementsData()?.unlockedCount || 0,
  );
  readonly totalCount = computed(
    () => this.achievementsData()?.totalCount || 0,
  );
  ...
  unlockAchievement(achievementId: string): Observable<UnlockResult> {
    return this.http
      .post<{ success: boolean; data: UnlockResult }>("/api/achievements", {
        achievementId,
      ...
      .pipe(
        ...
        catchError((error) => {
          this.logger.error(
            "[Achievements] Error unlocking achievement",
            error,
          );
          return of({
            success: false,
   ...
   */
  syncFromLocalStorage(): Observable<{
    synced: number;
    ...
    try {
      ...
      return this.http
        .put<{
          success: boolean;
          data: { synced: string[]; alreadyUnlocked: string[] };
        ...
        .pipe(
          ...
          catchError((error) => {
            this.logger.error(
              "[Achievements] Error syncing from localStorage",
              error,
            );
            return of({ synced: 0, alreadyUnlocked: 0 });
          }),
  ...
  getNextToUnlock(limit = 3): Achievement[] {
    ...
    const prioritized = locked.filter((a) =>
      categoriesWithProgress.has(a.category),
    );
    const others = locked.filter(
      (a) => !categoriesWithProgress.has(a.category),
    );
