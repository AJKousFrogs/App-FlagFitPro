...
describe("detectTournamentRecoveryDay", () => {
  ...
  it("returns null when the tournament has not finished yet (date is during the event)", () => {
    ...
    expect(detectTournamentRecoveryDay(tournamentEndingMay9, duringEvent)).toBeNull();
  });
});
...
describe("isHighCnsSessionType — flag-football drill RPE gate", () => {
  ...
  it.each(["sprint", "Sprint", "plyo", "speed work", "max velocity", "accel", "agility", "bound"])(
  ...
  it.each(["route", "routes", "post", "fade", "hook", "evade", "evasion", "flag pull", "flag-pull"])(
    "flag drill '%s' at RPE ≥ 6 is high-CNS",
    (type) => {
  ...
  it.each(["route", "routes", "post", "fade", "hook", "evade", "evasion", "flag pull"])(
    "flag drill '%s' at RPE 5 is NOT high-CNS (sub-threshold)",
    (type) => {
  ...
  it("flag drill CNS guard blocks a planned sprint day when recent session has RPE ≥ 6", () => {
    ...
    const rx = prescribeFor(
      inputs({
        ...
        phase: "accumulation",
        recentSessions: [
          { at: new Date(tuesday.getTime() - 20 * 3_600_000).toISOString(), type: "route", rpe: 8 },
        ],
        ageYears: 30,
      }),
    );
  ...
  it("flag drill at RPE 4 does NOT block a subsequent sprint session", () => {
    ...
    const rx = prescribeFor(
      inputs({
        ...
        phase: "accumulation",
        recentSessions: [
          { at: new Date(tuesday.getTime() - 20 * 3_600_000).toISOString(), type: "routes", rpe: 4 },
        ],
        ageYears: 30,
      }),
    );
...
describe("modulateIntentForLoad", () => {
  ...
  it("elevated ACWR (1.4) downgrades strength → mobility", () => {
    expect(modulateIntentForLoad("strength", 1.4, false, false)).toBe("mobility");
  });
  ...
  it("heavy density downgrades strength → technical", () => {
    expect(modulateIntentForLoad("strength", 1.0, true, false)).toBe("technical");
  });
  ...
  it("weekly progression unsafe downgrades strength → technical", () => {
    expect(modulateIntentForLoad("strength", 1.0, false, true)).toBe("technical");
  });
  ...
  it("technical is not changed by weekly progression unsafe (already low-intensity)", () => {
    expect(modulateIntentForLoad("technical", 1.0, false, true)).toBe("technical");
  });
});
