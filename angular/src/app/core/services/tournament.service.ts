...
export class TournamentService {
  ...
  readonly nextTournament = computed(() => {
    ...
    return upcoming.reduce(
      (closest, t) => {
        if (!closest) return t;
        return new Date(t.start_date) < new Date(closest.start_date)
          ? t
          : closest;
      },
  ...
  readonly teamTournaments = computed(() =>
    this.tournaments().filter(
      (t) => t.visibility_scope === "team" || !t.visibility_scope,
    ),
  );
  ...
  async fetchTournament(id: string): Promise<Tournament | null> {
    ...
    try {
      const response = await firstValueFrom(
        this.apiService.get<{ tournament: Tournament }>(
  ...
  async createTournament(
    ...
    try {
      const response = await firstValueFrom(
        this.apiService.post<{ tournament: Tournament; message: string }>(
          "/api/tournaments",
          data,
        ),
      );
  ...
  async updateTournament(
    ...
    try {
      const response = await firstValueFrom(
        this.apiService.put<{ tournament: Tournament; message: string }>(
          `/api/tournaments?id=${id}`,
          data,
        ),
      );
