...
import { signal, effect, computed, inject } from "@angular/core";
...
export class Example5_PerformanceMonitoring {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (profile && wellness && training) {
        ...
        this.debugService.logPerformance(
  ...
  async loadDashboard() {
    ...
    try {
      ...
      this.debugService.logLifecycle(
        "PlayerDashboard",
  ...
  private async loadProfile() {
    ...
    return new Promise((resolve) =>
      setTimeout(() => resolve({ name: "John" }), 200),
    );
  }

  private async loadWellness() {
    return new Promise((resolve) =>
      setTimeout(() => resolve({ score: 85 }), 150),
    );
  }

  private async loadTraining() {
    return new Promise((resolve) =>
      setTimeout(() => resolve({ sessions: 10 }), 100),
    );
  }
}
