...
@Component({
  ...
  template: `
    @if (variant() === 'text') {
      <div [class]="skeletonClass()" [style.width]="width()" [style.height]="height()">
        <div class="skeleton-shimmer"></div>
      </div>
    } @else if (variant() === 'circle') {
      <div [class]="skeletonClass()" [style.width]="size()" [style.height]="size()">
        <div class="skeleton-shimmer"></div>
      </div>
...
export class SkeletonComponent {
  ...
  skeletonClass = computed(() => {
    const baseClass = this.variant() === 'custom' ? 'skeleton-container' : `skeleton skeleton-${this.variant()}`;
