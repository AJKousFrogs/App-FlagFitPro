...
export type DataStateType = 'NO_DATA' | 'INSUFFICIENT_DATA' | 'DEMO_DATA' | 'REAL_DATA';
...
export const DELETION_MESSAGES = {
  ...
  pending: {
    ...
    reason: 'Your account is scheduled for deletion. You have time to change your mind.',
  ...
  completed: {
    ...
    action: 'Thank you for using FlagFit Pro. You can create a new account anytime.',
  ...
  gracePeriodInfo: {
    ...
    reason: 'We keep your data for 30 days after deletion request in case you change your mind.',
    action: 'After 30 days, your data will be permanently and irreversibly deleted.',
...
export const METRIC_INSUFFICIENT_DATA = {
  acwr: {
    ...
    reason: 'ACWR (Acute:Chronic Workload Ratio) requires 28 days of training data.',
    action: 'Keep logging your training. You need {{daysNeeded}} more days of data.',
  ...
  chronicLoad: {
    ...
    reason: 'Chronic load requires 28 days of training data to establish baseline.',
  ...
  tsb: {
    ...
    action: 'Continue your training journey. You need {{daysNeeded}} more days.',
...
export function getDataStateMessage(
  ...
  if (dataState === 'INSUFFICIENT_DATA' && options?.metricType) {
    ...
    if (metricMessage) {
      const daysNeeded = options.requiredDataPoints && options.currentDataPoints
      ...
      return {
        ...
        action: metricMessage.action.replace('{{daysNeeded}}', String(daysNeeded)),
      };
    }
  }
