 ...
 * ```
 */
export const trackByKey = <T extends Record<string, unknown>>(key: keyof T) =>
  (index: number, item: T): unknown => item[key];
 ...
 * ```
 */
export const trackByKeys = <T extends Record<string, unknown>>(
  keys: Array<keyof T>,
) =>
 ...
 * ```
 */
export const trackByFn = <T>(fn: (item: T) => string | number) =>
  (index: number, item: T): string | number => fn(item);
