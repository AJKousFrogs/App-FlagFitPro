...
@Component({
  ...
  template: `
    <p-toast></p-toast>
    <div class="import-dataset-container bg-surface-primary p-6 rounded-lg shadow-medium max-w-4xl mx-auto">
      <div class="header mb-6">
        <h2 class="text-2xl font-bold text-text-primary mb-2">Import Training Data</h2>
        <p class="text-text-secondary">Upload wearable device files or paste JSON data</p>
      </div>
      ...
      @if (activeTab() === 'upload') {
        <div class="upload-section">
          <div class="upload-area p-8 border-2 border-dashed border-gray-300 rounded-lg text-center hover:border-brand-primary transition-colors">
            <input 
              type="file" 
              id="fileInput" 
              class="hidden"
            ...
            <label for="fileInput" class="cursor-pointer">
              ...
              <div class="text-lg font-semibold text-text-primary mb-2">Click to upload or drag and drop</div>
              <div class="text-sm text-text-secondary">CSV, JSON, or XML files</div>
            </label>
          </div>
          @if (selectedFile()) {
            <div class="file-info mt-4 p-4 bg-surface-secondary rounded-lg">
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <div class="text-sm text-text-secondary">{{ (selectedFile()?.size || 0) / 1024 | number:'1.0-0' }} KB</div>
                </div>
                <button 
      ...
      @if (activeTab() === 'paste') {
        <div class="paste-section">
          <div class="mb-4">
            <label for="athleteId" class="block text-sm font-semibold text-text-primary mb-2">Athlete ID</label>
            <input
              id="athleteId"
              type="text"
              [value]="athleteId()"
...
[
  ...
  { "speed_m_s": 7.8, "distance_m": 2.9 }
            ...
            <small class="text-text-secondary text-xs mt-1 block">
              Format: Array of objects with speed_m_s (or speed) and distance_m (or distance) fields
            </small>
          </div>
      ...
      @if (activeTab() === 'generate') {
        <div class="generate-section">
          <div class="mb-4">
            <label for="generateAthleteId" class="block text-sm font-semibold text-text-primary mb-2">Athlete ID</label>
            <input
          ...
          @if (importResult()?.success) {
            <div class="text-green-800">
              ...
              <ul class="list-disc list-inside space-y-1">
                <li>Total Volume: {{ importResult()?.metrics?.total_volume }}m</li>
                <li>High-Speed Distance: {{ importResult()?.metrics?.high_speed_distance }}m</li>
                <li>Sprint Count: {{ importResult()?.metrics?.sprint_count }}</li>
                <li>Duration: {{ importResult()?.metrics?.duration_minutes }} minutes</li>
              </ul>
            </div>
          } @else {
...
export class ImportDatasetComponent {
  ...
  async parseAndImport() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        this.messageService.add(ErrorHandlerUtil.createSuccessMessage('File parsed and imported successfully!'));
        this.selectedFile.set(null);
      } else {
        throw new Error('Import failed');
      }
    } catch (error: any) {
      const errorMessage = ErrorHandlerUtil.extractErrorMessage(error, 'Failed to parse and import file');
      this.importResult.set({
        success: false,
        error: errorMessage
      });
      this.messageService.add(ErrorHandlerUtil.createErrorMessage(errorMessage, 'Import Failed'));
    } finally {
  ...
  async generateAndImport() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        this.messageService.add(ErrorHandlerUtil.createSuccessMessage('Test dataset generated and imported!'));
      } else {
        throw new Error('Import failed');
      }
    } catch (error: any) {
      const errorMessage = ErrorHandlerUtil.extractErrorMessage(error, 'Failed to generate and import dataset');
      this.importResult.set({
        success: false,
        error: errorMessage
      });
  ...
  async import() {
    ...
    try {
      ...
      const errorMessage = ErrorHandlerUtil.extractErrorMessage(error, 'Failed to import dataset');
      this.importResult.set({
        success: false,
        error: errorMessage
      });
