  ...
  template: `
    <div class="microcycle-planner bg-surface-primary rounded-lg shadow-medium p-6">
      <div class="header mb-6">
        <h2 class="text-2xl font-bold text-text-primary mb-2">Weekly Microcycle Planner</h2>
        <p class="text-text-secondary">AI-powered sprint load suggestions based on ACWR</p>
      </div>
      ...
      <div class="current-status mb-6 p-4 bg-surface-secondary rounded-lg">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="text-lg font-semibold text-text-primary">Current ACWR Status</h3>
            <p class="text-sm text-text-secondary">Last updated: {{ lastUpdate() | date:'short' }}</p>
          </div>
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          <div class="stat-card">
            ...
            <div class="stat-value">{{ chronicLoad() | number:'1.0-0' }} AU</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">ACWR Ratio</div>
            <div class="stat-value" [class]="'text-' + getRiskColor()">{{ currentACWR() | number:'1.2-2' }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="weekly-plan">
        <h3 class="text-xl font-semibold text-text-primary mb-4">7-Day Sprint Load Plan</h3>
        <div class="days-grid grid grid-cols-1 md:grid-cols-7 gap-4">
          @for (day of weeklyPlan(); track day.day) {
            <div class="day-card p-4 rounded-lg border-2" [class]="getDayCardClass(day)">
              <div class="day-header mb-3">
                <div class="day-name font-bold text-lg">{{ day.day }}</div>
                <div class="day-date text-sm text-text-secondary">{{ day.date | date:'MMM d' }}</div>
              </div>
              
              <div class="day-content">
                <div class="intensity-badge mb-3" [class]="'intensity-' + day.suggestedIntensity">
                  {{ day.suggestedIntensity | titlecase }}
                </div>
                
                @if (day.suggestedIntensity !== 'rest') {
                  <div class="sprint-load mb-2">
                    <div class="text-xs text-text-secondary mb-1">Sprint Load</div>
                    <div class="text-2xl font-bold" [style.color]="getSprintLoadColor(day.suggestedSprintLoad)">
                      {{ day.suggestedSprintLoad }}
                    </div>
                    <div class="text-xs text-text-secondary">Max: {{ day.maxSprints }}</div>
                  </div>
                  
                  <div class="duration mb-2">
                    <div class="text-xs text-text-secondary mb-1">Duration</div>
                    <div class="font-semibold">{{ day.recommendedDuration }} min</div>
                  </div>
                  
                  <div class="projected-acwr mb-2">
                    <div class="text-xs text-text-secondary mb-1">Projected ACWR</div>
                    <div class="font-semibold" [class]="'text-' + getACWRColor(day.acwrProjection)">
                ...
                <div class="reasoning mt-3 pt-3 border-t border-gray-200">
                  <div class="text-xs text-text-secondary italic">{{ day.reasoning }}</div>
                </div>
              </div>
            </div>
          }
      ...
      <div class="summary mt-6 p-4 bg-surface-secondary rounded-lg">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <div class="text-lg font-bold">{{ avgDailyLoad() | number:'1.0-0' }} AU</div>
          </div>
          <div>
            <div class="text-xs text-text-secondary">End-of-Week ACWR</div>
            <div class="text-lg font-bold" [class]="'text-' + getACWRColor(endOfWeekACWR())">
...
export class MicrocyclePlannerComponent implements OnInit {
  ...
  trainingDays = computed(() => 
    this.weeklyPlan().filter(day => day.suggestedIntensity !== 'rest').length
  );
  ...
  generateWeeklyPlan() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const plan = this.calculateDayPlan(date, i, currentACWR, chronic, this.gameDays());
      days.push(plan);
    }
  ...
  private calculateDayPlan(date: Date, dayIndex: number, currentACWR: number, chronic: number, gameDays: Date[]): DayPlan {
    ...
    if (gameProximity.hoursUntilGame >= 0 && gameProximity.hoursUntilGame <= 24) {
    ...
    } else if (gameProximity.hoursUntilGame > 24 && gameProximity.hoursUntilGame <= 48) {
    ...
    } else if (gameProximity.hoursUntilGame > 48 && gameProximity.hoursUntilGame <= 72) {
   ...
   */
  private getGameProximity(date: Date, gameDays: Date[]): { hoursUntilGame: number; hasGame: boolean } {
