...
@Component({
  ...
  template: `
    <div class="flag-load-container bg-surface-primary p-6 rounded-lg shadow-medium">
      <!-- ACWR Table -->
        ...
        <div class="overflow-x-auto bg-white rounded-lg shadow-low">
          <table class="w-full">
            <thead class="bg-surface-secondary">
              <tr>
                <th class="px-4 py-3 text-left text-sm font-semibold text-text-primary">Date</th>
                <th class="px-4 py-3 text-left text-sm font-semibold text-text-primary">Load</th>
                <th class="px-4 py-3 text-left text-sm font-semibold text-text-primary">Acute</th>
                <th class="px-4 py-3 text-left text-sm font-semibold text-text-primary">Chronic</th>
                <th class="px-4 py-3 text-left text-sm font-semibold text-text-primary">ACWR</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200">
              @for (row of acwrData(); track row.session_date) {
                <tr class="hover:bg-surface-tertiary transition-colors">
                  <td class="px-4 py-3 text-sm text-text-primary">{{ row.session_date | date:'shortDate' }}</td>
                  <td class="px-4 py-3 text-sm text-text-primary">{{ row.load | number:'1.0-0' }}</td>
                  <td class="px-4 py-3 text-sm text-text-primary">{{ row.acute_load | number:'1.0-0' }}</td>
                  <td class="px-4 py-3 text-sm text-text-primary">{{ row.chronic_load | number:'1.0-0' }}</td>
                  <td class="px-4 py-3">
                    <span 
                      class="font-semibold"
      ...
      <div class="metrics-section">
        <h2 class="text-2xl font-bold text-text-primary mb-4">4-Week Flag Football Metrics</h2>
