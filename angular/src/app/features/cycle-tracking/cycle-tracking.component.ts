...
interface CyclePhase {
  ...
  focusAreas: string[];
  keyInsights: string[];
  injuryRisk?: { area: string; risk: string; reason: string; precautions: string[] };
...
const CYCLE_PHASES: CyclePhase[] = [
  {
    ...
    icon: '🌸',
    focusAreas: ['Recovery', 'Mobility work', 'Light cardio', 'Gentle stretching'],
    keyInsights: [
  ...
  },
  {
    ...
    icon: '🌱',
    focusAreas: ['Strength training', 'Skill work', 'Endurance building', 'New techniques'],
    keyInsights: [
  ...
  },
  {
    ...
    icon: '🔥',
    focusAreas: ['Power & explosive movements', 'Speed work & sprints', 'Max strength training', 'Competition / game day', 'Personal record attempts'],
    keyInsights: [
  ...
  },
  {
    ...
    icon: '🌿',
    focusAreas: ['Endurance work', 'Moderate strength', 'Tempo runs', 'Sustained efforts'],
    keyInsights: [
  ...
  },
  {
    ...
    icon: '🌙',
    focusAreas: ['Technique refinement', 'Flexibility', 'Light conditioning', 'Recovery focus'],
    keyInsights: [
  ...
  template: `
    <app-main-layout>
      ...
      <div class="cycle-tracking-page">
        ...
        <p-message 
          severity="info" 
          styleClass="privacy-notice"
        >
          <ng-template pTemplate>
            <div class="privacy-content">
              <i class="pi pi-lock"></i>
              <div>
                <strong>Privacy First:</strong> This data is private by default. Coaches only see "recovery day recommended" - never cycle details.
                <a routerLink="/settings/privacy" class="privacy-link">Manage settings</a>
              </div>
            </div>
          </ng-template>
        </p-message>
        ...
        <p-card styleClass="cycle-status-card">
          <div class="cycle-status-content">
            <div class="cycle-day-display">
              <div class="day-circle" [style.borderColor]="getCurrentPhase().color">
            ...
            <div class="phase-display">
              <div class="phase-badge" [style.backgroundColor]="getCurrentPhase().color">
            ...
            <div class="cycle-phases-timeline">
              @for (phase of cyclePhases; track phase.name) {
                <div 
                  ...
                  [style.backgroundColor]="phase.name === getCurrentPhase().name ? phase.color : 'var(--surface-secondary)'"
            ...
            <div class="cycle-predictions">
              <div class="prediction">
                ...
                <span class="prediction-value">{{ cycleStatus().nextPeriodDate | date:'MMM d' }} (±2 days)</span>
              </div>
              <div class="prediction">
                <span class="prediction-label">Cycle Length</span>
                <span class="prediction-value">{{ cycleStatus().cycleLength }} days (avg {{ cycleStatus().cyclesTracked }} cycles)</span>
              </div>
            </div>
          </div>
        </p-card>
          ...
          <p-card styleClass="recommendation-card training-card">
            <ng-template pTemplate="header">
              <div class="rec-header">
                ...
                <span>Training Recommendations ({{ getCurrentPhase().name }} Phase)</span>
              </div>
            </ng-template>

            <div class="intensity-modifier">
              <span class="modifier-label">Intensity Modifier:</span>
              <span class="modifier-value" [class.boost]="getCurrentPhase().intensityModifier > 100" [class.reduce]="getCurrentPhase().intensityModifier < 100">
                {{ getCurrentPhase().intensityModifier }}%
                @if (getCurrentPhase().intensityModifier > 100) {
                  (Can push {{ getCurrentPhase().intensityModifier - 100 }}% harder!)
                } @else if (getCurrentPhase().intensityModifier < 100) {
                  (Reduce intensity by {{ 100 - getCurrentPhase().intensityModifier }}%)
                }
              </span>
            </div>
            ...
            @if (getCurrentPhase().injuryRisk) {
              <div class="injury-warning">
                <h4><i class="pi pi-exclamation-triangle"></i> INJURY AWARENESS</h4>
                <div class="injury-content">
                  <div class="injury-risk">
                    <strong>{{ getCurrentPhase().injuryRisk!.area }} Risk:</strong>
                    <span class="risk-level">{{ getCurrentPhase().injuryRisk!.risk }}</span>
                  </div>
                  <div class="injury-reason">
                    <strong>Reason:</strong> {{ getCurrentPhase().injuryRisk!.reason }}
                  </div>
                  <div class="injury-precautions">
                    <strong>Precautions:</strong>
                    <ul>
                      @for (precaution of getCurrentPhase().injuryRisk!.precautions; track precaution) {
                        <li>{{ precaution }}</li>
                      }
                    </ul>
                  </div>
          ...
          <p-card styleClass="recommendation-card nutrition-card">
            <ng-template pTemplate="header">
              <div class="rec-header">
                ...
                <span>Nutrition Recommendations ({{ getCurrentPhase().name }} Phase)</span>
              </div>
            </ng-template>
            ...
            <div class="nutrition-modifiers">
              <div class="modifier-item">
                ...
                <span class="modifier-val">{{ getCurrentPhase().calorieModifier }}%</span>
              </div>
              <div class="modifier-item">
                <span class="modifier-name">Hydration:</span>
                <span class="modifier-val">+{{ getCurrentPhase().hydrationModifier - 100 }}%</span>
              </div>
            </div>
            ...
            <div class="priority-nutrients">
              ...
              <div class="nutrients-grid">
                @for (nutrient of getCurrentPhase().priorityNutrients; track nutrient.name) {
        ...
        <p-card header="ACWR Adjustment" styleClass="acwr-card">
          <div class="acwr-content">
            ...
            <div class="sweet-spot-display">
              ...
              <span class="sweet-spot-value">{{ getAcwrSweetSpot().min }} - {{ getAcwrSweetSpot().max }}</span>
              <span class="vs-standard">(vs standard 0.8 - 1.3)</span>
            </div>

            <p-message [severity]="getAcwrStatus().severity" styleClass="acwr-status">
              <ng-template pTemplate>
        ...
        <p-card styleClass="symptom-card">
          <ng-template pTemplate="header">
            <div class="symptom-header">
              ...
              <app-button size="sm" iconLeft="pi-check" [loading]="isSavingSymptoms()" (clicked)="saveSymptoms()">Log Symptoms</app-button>
            </div>
          </ng-template>
              ...
              <div class="severity-options">
                @for (sev of severityOptions; track sev.value) {
                  <div class="severity-option">
                    ...
                    ></p-radioButton>
                    <label [for]="'severity-' + sev.value">{{ sev.label }}</label>
                  </div>
                }
              </div>
            </div>
        ...
        <p-card header="Recent Cycles" styleClass="history-card">
          @if (cycleHistory().length === 0) {
            <div class="empty-state">
              ...
              <p>No cycles logged yet. Log your first period to start tracking.</p>
            </div>
          } @else {
            <p-table [value]="cycleHistory()" [rows]="5" styleClass="p-datatable-sm">
              <ng-template pTemplate="header">
                <tr>
                  <th>Cycle</th>
                  <th>Start Date</th>
              ...
              <ng-template pTemplate="body" let-cycle let-i="rowIndex">
                <tr>
                  <td>{{ i === 0 ? 'Current' : getMonthName(cycle.startDate) }}</td>
                  ...
                  <td class="symptoms-cell">{{ formatSymptoms(cycle.symptoms) }}</td>
                </tr>
              </ng-template>
            </p-table>

            <div class="cycle-stats">
              <span>Average Cycle Length: <strong>{{ averageCycleLength() }} days</strong></span>
              <span>Cycle Regularity: <strong>{{ cycleRegularity() }}</strong></span>
            </div>
          }
        </p-card>
        ...
        <p-card header="Privacy Controls" styleClass="privacy-card">
          <div class="privacy-settings">
            ...
            <div class="privacy-actions">
              ...
              <app-button variant="outlined" iconLeft="pi-trash" (clicked)="confirmDeleteData()">Delete All Data</app-button>
            </div>
          </div>
        </p-card>
      </div>
      ...
      >
            ...
            <div class="symptoms-checkboxes">
              @for (symptom of symptomOptions; track symptom.value) {
                <div class="symptom-checkbox">
                  ...
                  ></p-checkbox>
                  <label [for]="'log-symptom-' + symptom.value">{{ symptom.label }}</label>
                </div>
              }
            </div>
          </div>
        ...
        <ng-template pTemplate="footer">
          ...
          <app-button iconLeft="pi-check" [loading]="isSavingPeriod()" [disabled]="!newPeriod.startDate" (clicked)="savePeriod()">Save</app-button>
        </ng-template>
      </p-dialog>
      ...
      >
        <div class="delete-warning">
          ...
          <p>Are you sure you want to delete all your cycle tracking data? This action cannot be undone.</p>
        </div>

        <ng-template pTemplate="footer">
          <app-button variant="outlined" (clicked)="showDeleteDialog.set(false)">Cancel</app-button>
          <app-button variant="danger" iconLeft="pi-trash" (clicked)="deleteAllData()">Delete All Data</app-button>
        </ng-template>
      </p-dialog>
    </app-main-layout>
  `,
...
export class CycleTrackingComponent implements OnInit {
  ...
  async loadData(): Promise<void> {
    try {
      ...
      const response: any = await firstValueFrom(this.api.get('/api/cycle-tracking'));
  ...
  getCurrentPhase(): CyclePhase {
    ...
    return CYCLE_PHASES.find(p => day >= p.dayRange[0] && day <= p.dayRange[1]) || CYCLE_PHASES[0];
  }
  ...
  getAcwrStatus(): { severity: 'success' | 'warn' | 'error' | 'info'; message: string } {
    const adjusted = this.adjustedAcwr();
    const sweetSpot = this.getAcwrSweetSpot();

    if (adjusted >= sweetSpot.min && adjusted <= sweetSpot.max) {
      return { severity: 'success', message: "You're in the optimal zone! Good to train at recommended intensity." };
    } else if (adjusted < sweetSpot.min) {
      return { severity: 'info', message: 'You can increase training load if you feel ready.' };
    } else {
      return { severity: 'warn', message: 'Consider reducing intensity during this phase.' };
    }
  }
  ...
  getNutritionTip(): string {
    ...
    const tips: Record<string, string> = {
      'Menstrual': 'Focus on iron-rich foods like lean red meat, spinach, and beans to replace losses.',
      'Follicular': 'Your body uses carbs efficiently now - fuel your workouts with quality starches.',
      'Ovulation': 'Support peak performance with quality protein and anti-inflammatory foods.',
      'Luteal Early': 'Metabolism is increasing - honor your hunger with nutrient-dense meals.',
      'Luteal Late': 'Cravings are normal! Choose dark chocolate and magnesium-rich snacks.'
    };
  ...
  async savePeriod(): Promise<void> {
    ...
    try {
      await firstValueFrom(this.api.post('/api/cycle-tracking/period', this.newPeriod));
  ...
  async saveSymptoms(): Promise<void> {
    ...
    try {
      await firstValueFrom(this.api.post('/api/cycle-tracking/symptoms', {
  ...
  getFlowSeverity(flow: string): 'success' | 'info' | 'warn' | 'danger' | 'secondary' | 'contrast' {
