...
@Component({
  ...
  template: `
    <div class="debug-console">
        ...
        <div class="test-grid">
          <div class="test-card">
            ...
            <select [(ngModel)]="selectedTable">
              ...
              <option value="daily_wellness_checkin">daily_wellness_checkin</option>
      ...
      <div class="stats-section" *ngIf="showStatsPanel">
        ...
        <div class="stats-grid">
          ...
          <div class="stat-card">
            <div class="stat-value">{{ stats?.avgDuration?.toFixed(2) || 0 }}ms</div>
            <div class="stat-label">Avg Duration</div>
          </div>
        </div>
        
        <div class="by-table" *ngIf="stats?.byTable">
          <h4>Queries by Table</h4>
          <div class="table-stats">
            <div *ngFor="let entry of objectEntries(stats.byTable)" class="table-stat">
  ...
  styles: [`
    ...
    .log-success .log-level { color: #4ec9b0; }
    ...
    .stat-value.success { color: #4ec9b0; }
...
export class DebugConsoleComponent implements OnInit, OnDestroy {
  ...
  async validateSchema() {
    ...
    const expectedColumns: { [key: string]: string[] } = {
      ...
      'injuries': ['id', 'user_id', 'injury_type', 'injury_date', 'status', 'created_at', 'updated_at'],
      'daily_wellness_checkin': ['id', 'user_id', 'sleep_hours', 'energy_level', 'created_at'],
  ...
  async checkIndexes() {
    ...
    const tables = ['user_profiles', 'injuries', 'daily_wellness_checkin', 'nutrition_logs'];
    
    try {
      const results = await this.debugService.checkIndexes(this.supabase, tables);
  ...
  async testInsert() {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Cleanup
        if (result.data?.[0]?.id) {
          await this.supabase.from(this.selectedTable).delete().eq('id', result.data[0].id);
