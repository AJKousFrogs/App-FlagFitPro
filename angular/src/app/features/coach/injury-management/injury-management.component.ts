...
const RTP_STAGES: RtpStage[] = [
  { stage: 1, name: "Rest", intensity: 0, description: "Complete rest, symptom resolution", allowedActivities: ["Rest", "Ice", "Light stretching"], restrictedActivities: ["Any physical activity"], progressionCriteria: [] },
  { stage: 2, name: "Light Activity", intensity: 20, description: "Walking, light stretching", allowedActivities: ["Walking", "Light stretching", "Low-intensity cardio"], restrictedActivities: ["Running", "Sport-specific drills"], progressionCriteria: [] },
  { stage: 3, name: "Sport-Specific (Low)", intensity: 40, description: "Basic drills, no contact", allowedActivities: ["Basic drills", "Jogging", "Position movements"], restrictedActivities: ["Full-speed running", "Contact"], progressionCriteria: [] },
  { stage: 4, name: "Sport-Specific (Med)", intensity: 60, description: "Complex drills, no contact", allowedActivities: ["Sport-specific drills", "Position-specific movements", "Moderate cardio"], restrictedActivities: ["Full-speed sprinting", "Contact drills", "Live scrimmage"], progressionCriteria: [] },
  { stage: 5, name: "Sport-Specific (High)", intensity: 80, description: "Full-speed, limited contact", allowedActivities: ["Full-speed drills", "Limited contact", "Team drills"], restrictedActivities: ["Full contact practice"], progressionCriteria: [] },
  { stage: 6, name: "Full Training", intensity: 100, description: "Full participation", allowedActivities: ["All training activities", "Full contact"], restrictedActivities: [], progressionCriteria: [] },
  { stage: 7, name: "Cleared", intensity: 100, description: "Competition cleared", allowedActivities: ["Full competition"], restrictedActivities: [], progressionCriteria: [] },
];
    ...
    <app-main-layout>
      ...
      <div class="injury-management-page">
        <app-page-header
          ...
          icon="pi-heart"
        >
          <app-button iconLeft="pi-plus" (clicked)="openReportDialog()">Report Injury</app-button>
        </app-page-header>
            ...
            @for (injury of activeInjuries(); track injury.id) {
              <p-card styleClass="injury-card new">
                <div class="injury-header">
                  <p-tag value="New Injury - Needs Evaluation" severity="danger"></p-tag>
                </div>
                <div class="injury-content">
                  <p-avatar
                    [image]="injury.avatarUrl"
                    [label]="!injury.avatarUrl ? getInitials(injury.playerName) : ''"
                    size="large"
                    shape="circle"
                  ></p-avatar>
                  <div class="injury-info">
                    <h4>{{ injury.playerName }} (#{{ injury.jerseyNumber }})</h4>
                    <p>Position: {{ injury.playerPosition }}</p>
                    <p><strong>Injury:</strong> {{ injury.bodyPart }} {{ injury.injuryType }} ({{ injury.severity }})</p>
                    <p><strong>Reported:</strong> {{ injury.injuryDate | date:'MMM d, y, h:mm a' }}</p>
                    <p class="description">"{{ injury.description }}"</p>
                    <p class="status"><i class="pi pi-exclamation-triangle"></i> Awaiting medical evaluation</p>
                  </div>
                </div>
                <div class="injury-actions">
                  <app-button iconLeft="pi-play" (clicked)="startRtp(injury)">Evaluate & Start RTP</app-button>
                  <app-button variant="secondary" iconLeft="pi-file" (clicked)="requestMedical(injury)">Request Medical Report</app-button>
                </div>
              </p-card>
            }
          </div>
        ...
        <!-- In RTP -->
        @if ((activeTab() === 'active' || activeTab() === 'rtp') && rtpInjuries().length > 0) {
          <div class="injuries-section">
            ...
            @for (injury of rtpInjuries(); track injury.id) {
              <p-card styleClass="injury-card rtp">
                <div class="injury-header">
                  <p-tag
                    [value]="'Stage ' + injury.rtpStage + ' of 7 - ' + getStageName(injury.rtpStage!)"
                    severity="warn"
                  ></p-tag>
                </div>
                <div class="injury-content">
                  ...
                  <div class="injury-info">
                    <h4>{{ injury.playerName }} (#{{ injury.jerseyNumber }})</h4>
                    <p>Position: {{ injury.playerPosition }}</p>
                    <p><strong>Injury:</strong> {{ injury.bodyPart }} {{ injury.injuryType }}</p>
                    <p><strong>Injury Date:</strong> {{ injury.injuryDate | date:'MMM d, y' }}</p>
                  ...
                  <div class="stage-indicators">
                    @for (s of [1,2,3,4,5,6,7]; track s) {
                      ...
                      >
                        ...
                        <span class="stage-name">{{ getStageShortName(s) }}</span>
                      </div>
                    }
                  </div>
                </div>
                ...
                @if (injury.todayCheckin) {
                  <div class="checkin-summary">
                    ...
                    <div class="checkin-metrics">
                      ...
                      <span>Function: {{ injury.todayCheckin.functionLevel }}/10</span>
                      <span>Confidence: {{ injury.todayCheckin.confidenceLevel }}/10</span>
                    </div>
                  </div>
                }
                ...
                <div class="injury-actions">
                  <app-button variant="secondary" (clicked)="viewRtpDetails(injury)">View Full RTP</app-button>
                  <app-button variant="secondary" (clicked)="openCheckinDialog(injury)">Update Progress</app-button>
                  <app-button iconLeft="pi-arrow-right" [disabled]="!canAdvanceStage(injury)" (clicked)="advanceStage(injury)">Advance Stage</app-button>
                </div>
              </p-card>
            }
          </div>
            ...
            @for (injury of clearedInjuries(); track injury.id) {
              <p-card styleClass="injury-card cleared">
                <div class="injury-content">
                  ...
                  <div class="injury-info">
                    <h4>{{ injury.playerName }} (#{{ injury.jerseyNumber }})</h4>
                    <p>{{ injury.bodyPart }} {{ injury.injuryType }} - Cleared</p>
          ...
          <div class="analytics-section">
            ...
            <p-card>
                ...
                <div class="analytics-chart">
                  ...
                  @for (stat of injuryByType(); track stat.type) {
                    <div class="stat-bar">
                      ...
                      <div class="bar-track">
                        <div class="bar-fill" [style.width.%]="(stat.count / totalSeasonCount()) * 100"></div>
                      </div>
              ...
              <div class="insight-box">
                ...
                <span>Hamstring injuries are 50% of total. Consider adding more dynamic warm-up time and hamstring-specific prehab exercises.</span>
              </div>
            </p-card>
          </div>
        }
      ...
      >
        <div class="report-form">
          ...
          <div class="form-row">
            <div class="form-field">
              ...
              <div class="radio-group">
                <div class="radio-option">
                  <p-radioButton name="side" value="left" [(ngModel)]="reportForm.side" inputId="sideLeft"></p-radioButton>
                  <label for="sideLeft">Left</label>
                </div>
                <div class="radio-option">
                  <p-radioButton name="side" value="right" [(ngModel)]="reportForm.side" inputId="sideRight"></p-radioButton>
            ...
            <div class="form-field">
              ...
              <div class="radio-group">
                <div class="radio-option">
                  <p-radioButton name="severity" value="mild" [(ngModel)]="reportForm.severity" inputId="sevMild"></p-radioButton>
                  <label for="sevMild">Mild</label>
                </div>
                <div class="radio-option">
                  <p-radioButton name="severity" value="moderate" [(ngModel)]="reportForm.severity" inputId="sevMod"></p-radioButton>
                  <label for="sevMod">Moderate</label>
                </div>
                <div class="radio-option">
                  <p-radioButton name="severity" value="severe" [(ngModel)]="reportForm.severity" inputId="sevSev"></p-radioButton>
          ...
          <div class="form-field">
            ...
            <div class="checkbox-group">
              <div class="checkbox-option">
                <p-checkbox [(ngModel)]="reportForm.actions.iceApplied" [binary]="true" inputId="actIce"></p-checkbox>
                <label for="actIce">Ice applied</label>
              </div>
              <div class="checkbox-option">
                <p-checkbox [(ngModel)]="reportForm.actions.removedFromActivity" [binary]="true" inputId="actRemoved"></p-checkbox>
                <label for="actRemoved">Player removed from activity</label>
              </div>
              <div class="checkbox-option">
                <p-checkbox [(ngModel)]="reportForm.actions.medicalContacted" [binary]="true" inputId="actMedical"></p-checkbox>
                <label for="actMedical">Medical professional contacted</label>
              </div>
              <div class="checkbox-option">
                <p-checkbox [(ngModel)]="reportForm.actions.sentForImaging" [binary]="true" inputId="actImaging"></p-checkbox>
        ...
        <ng-template pTemplate="footer">
          <app-button variant="secondary" (clicked)="showReportDialog = false">Cancel</app-button>
          <app-button iconLeft="pi-check" [disabled]="!reportForm.playerId || !reportForm.bodyPart" (clicked)="submitReport()">Report & Start RTP</app-button>
        </ng-template>
      </p-dialog>
      ...
      >
        ...
        <ng-template pTemplate="footer">
          <app-button variant="secondary" (clicked)="showCheckinDialog = false">Cancel</app-button>
          <app-button iconLeft="pi-check" (clicked)="submitCheckin()">Save Check-in</app-button>
        </ng-template>
      </p-dialog>
    </app-main-layout>
  `,
...
export class InjuryManagementComponent implements OnInit {
  ...
  // State
  readonly activeTab = signal<"active" | "rtp" | "cleared" | "history">("active");
  ...
  // Check-in form
  checkinForm = { painLevel: 0, functionLevel: 5, confidenceLevel: 5, notes: "" };
  ...
  readonly activeInjuries = computed(() =>
    this.injuries().filter((i) => i.status === "new" || i.status === "evaluating")
  );
  ...
  async loadData(): Promise<void> {
    ...
    try {
      ...
      const response: any = await firstValueFrom(this.api.get("/api/staff-physiotherapist/athletes"));
        ...
        for (const athlete of response.data.athletes) {
          ...
          if (athlete.currentInjury) {
            injuries.push({
              ...
              injuryType: (athlete.currentInjury.type || "strain").toLowerCase() as "strain" | "sprain" | "contusion" | "fracture" | "other",
              ...
              status: athlete.clearanceStatus === "cleared" ? "cleared" : athlete.clearanceStatus === "limited" ? "rtp" : "new",
      ...
      this.logger.error("Failed to load injuries from API, no data available", err);
  ...
  private mapGradeToSeverity(grade: string | undefined): "mild" | "moderate" | "severe" {
    if (!grade) return "moderate";
    const lower = grade.toLowerCase();
    if (lower.includes("i") || lower.includes("mild") || lower.includes("1")) return "mild";
    if (lower.includes("iii") || lower.includes("severe") || lower.includes("3")) return "severe";
    return "moderate";
  }
  ...
  private generateTimeOptions() {
    ...
    for (let h = 6; h <= 21; h++) {
      for (const m of ["00", "30"]) {
        ...
        options.push({ label: `${hour}:${m} ${ampm}`, value: `${hour}:${m} ${ampm}` });
      }
    }
    return options;
  }
  ...
  startRtp(injury: InjuryRecord): void {
    this.injuries.update((injs) =>
      injs.map((i) => (i.id === injury.id ? { ...i, status: "rtp" as InjuryStatus, rtpStage: 1, daysInProtocol: 0 } : i))
    );
    this.messageService.add({
      severity: "success",
      summary: "RTP Started",
  ...
  advanceStage(injury: InjuryRecord): void {
    ...
    this.injuries.update((injs) =>
      injs.map((i) =>
        i.id === injury.id
          ? { ...i, rtpStage: nextStage, status: nextStage === 7 ? ("cleared" as InjuryStatus) : i.status }
          : i
      )
    );
  ...
  canAdvanceStage(injury: InjuryRecord): boolean {
    ...
    return injury.todayCheckin.painLevel <= 2 && injury.todayCheckin.functionLevel >= 6;
  }
  ...
  getStatusSeverity(status: InjuryStatus): "success" | "info" | "warn" | "danger" {
    const severities: Record<InjuryStatus, "success" | "info" | "warn" | "danger"> = {
      new: "danger",
      evaluating: "warn",
      rtp: "warn",
      cleared: "success",
