...
export function resolveTodayState(
  ...
  if (status === 'no_program' || 
    ...
    return {
      ...
      errorState: {
        ...
        message: 'No training program assigned. Complete onboarding or contact your coach to get started.',
        cta: {
          label: 'Contact Coach',
          action: 'contact_coach',
        },
  ...
  if (cm.injuryProtocolActive === true || sr.override?.type === 'rehab_protocol') {
    ...
    const hasTeamActivity = protocolJson.teamActivity && 
  ...
  // ========================================================================
  if (protocolJson.weather_override === true || sr.override?.type === 'weather_override') {
    const banners: TodayViewModel['banners'] = [{
      type: 'alert',
      style: 'amber',
  ...
  if (sr.override?.type === 'flag_practice') {
    ...
    if (readiness.daysStale !== null && readiness.daysStale !== undefined && readiness.daysStale > 0) {
    ...
    return {
      ...
      primaryCta: readiness.daysStale !== null && readiness.daysStale !== undefined && readiness.daysStale > 0
        ? {
            label: 'Update Check-in',
            action: 'open_checkin',
          }
      ...
      secondaryCta: readiness.daysStale !== null && readiness.daysStale !== undefined && readiness.daysStale > 0
        ? {
            label: 'Continue to Practice Prep',
            action: 'start_training',
          }
        : undefined,
      merlinPosture: readiness.daysStale !== null && readiness.daysStale !== undefined && readiness.daysStale > 0 ? 'warning' : 'explanatory',
      headerContext: {
        practiceTime,
      },
    };
  ...
  if (sr.override?.type === 'film_room') {
    ...
    return {
      ...
      blocksDisplayed: ['morning_mobility', 'foam_roll', 'film_room', 'recovery'],
      primaryCta: {
        label: 'View Film Room Details',
        action: 'view_film_room',
      },
  ...
  if (protocolJson.taper_active === true || sr.override?.type === 'taper') {
    ...
    return {
      trainingAllowed: true,
      banners,
      blocksDisplayed: ['morning_mobility', 'foam_roll', 'taper_session', 'recovery'],
      primaryCta: {
        label: 'View Taper Plan',
        action: 'view_taper',
      },
  ...
  if (!readiness.hasData || protocolJson.readiness_score === null || protocolJson.readiness_score === undefined) {
    banners.push({
      type: 'info',
      style: 'blue',
  ...
  if (readiness.daysStale !== null && readiness.daysStale !== undefined && readiness.daysStale > 0 && readiness.daysStale <= 2) {
    const daysStale = Math.round(readiness.daysStale);
    banners.push({
      type: 'warning',
      style: 'amber',
  ...
  const blocksDisplayed: string[] = ['morning_mobility', 'foam_roll', 'main_session', 'recovery'];
