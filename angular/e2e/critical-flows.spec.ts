...
test.describe("Authentication Flow", () => {
  ...
  test("should show error for invalid credentials", async ({ page }) => {
    ...
    await expect(page.locator(".p-toast-message-error, .p-error, .error, [role='alert']")).toBeVisible({
      timeout: 10000,
    });
  });
...
test.describe("Dashboard Flow", () => {
  ...
  test("should display dashboard with key metrics", async ({ page }) => {
    ...
    await expect(
      page.locator('h1, h2, .page-header').filter({ hasText: /dashboard|overview/i }).first()
  ...
  test("should display ACWR status", async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText(/ACWR/i, { timeout: 10000 });
  });
...
test.describe("Training Log Flow", () => {
  ...
  test("should display training page", async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText(/training/i, { timeout: 10000 });
  });
  ...
  test("should show training goals step", async ({ page }) => {
    ...
    await expect(
      page.locator('text=/goals|training/i')
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test("should start a training session", async ({ page }) => {
    ...
    const generateButton = page.getByRole('button', { name: /Generate Session/i }).first();
...
test.describe("ACWR Dashboard Flow", () => {
  ...
  test("should display ACWR page", async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText(/ACWR|Workload|Load|Acute|Chronic/i, { timeout: 10000 });
  });
  ...
  test("should show traffic light indicator", async ({ page }) => {
    ...
    const trafficLight = page.locator('.traffic-light, app-traffic-light-indicator, [class*="indicator"]');
  ...
  test("should display load metrics", async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText(/load|training|session/i, { timeout: 10000 });
  });
  ...
  test("should show ACWR value or no-data state", async ({ page }) => {
    ...
    const hasContent = await page.locator('.metric-value, .acwr-value, .no-data, p-card').first().isVisible();
...
test.describe("Chat Flow", () => {
  ...
  test("should display chat page", async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText(/chat|channel|message/i, { timeout: 10000 });
  });
  ...
  test("should have header", async ({ page }) => {
    ...
    await expect(
      page.locator('h1, h2, .chat-header').first()
    ).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe("Wellness Tracking Flow", () => {
  ...
  test("should display wellness page", async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText(/wellness|health|check-in/i, { timeout: 10000 });
  });
...
test.describe("Accessibility", () => {
  ...
  test("should have no accessibility violations on dashboard", async ({
    ...
    const inputsWithoutLabels = await page
      .locator("input:not([aria-label]):not([aria-labelledby]):not([type='search']):not([placeholder])")
      .count();
