...
test.describe("Navigation & Routing - Comprehensive Click-Through", () => {
  ...
  test("should navigate through all main routes via direct URL", async ({ page }) => {
    const routes = [
  ...
  test("should navigate through sidebar navigation links", async ({ page }) => {
    ...
    const sidebarLinks = page.locator(
      'a[routerLink], a[href*="/"], [data-testid^="nav-"]',
    ...
    for (let i = 0; i < Math.min(linkCount, 15); i++) {
      ...
      const routerLink = await link.getAttribute("routerLink").catch(() => null);
  ...
  test("should click buttons and verify routing", async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(buttonCount, 10); i++) {
      ...
      const routerLink = await button.getAttribute("routerLink").catch(() => null);
      ...
      try {
        ...
        if (newUrl !== initialUrl) {
          ...
          console.log(`ℹ️  Button clicked but URL unchanged (may be modal/dialog)`);
        }
      } catch (error) {
  ...
  test("should verify Today's Practice page interactions", async ({ page }) => {
    ...
    console.log(`Found ${elementCount} clickable elements on Today's Practice page`);
    ...
    if (blockCount > 0) {
      ...
      for (let i = 0; i < Math.min(blockCount, 3); i++) {
        ...
        try {
          ...
          const exerciseList = page.locator(".exercise-list, .exercise-checkbox");
          const hasExercises = await exerciseList.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test("should verify navigation breadcrumbs and back buttons", async ({ page }) => {
  ...
  test("should verify all page headers and titles load", async ({ page }) => {
    ...
    for (const route of routes) {
      ...
      const heading = page.locator("h1, h2, .page-header, .card-header-title").first();
