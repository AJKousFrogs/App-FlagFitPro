...
test.describe('Launch Smoke Test - 100 Trials', () => {
  ...
  for (let trialNum = 1; trialNum <= TEST_CONFIG.TOTAL_TRIALS; trialNum++) {
    test(`Trial ${trialNum}/${TEST_CONFIG.TOTAL_TRIALS}: Complete Critical Flow`, async ({ page }) => {
      ...
      try {
        ...
        // ============================================================
        console.log(`  📝 Step 2: Log ${TEST_CONFIG.ENTRIES_PER_TRIAL} training entries`);
        ...
        console.log(`✅ Trial ${trialNum}/100 PASSED in ${(result.duration / 1000).toFixed(2)}s`);
        ...
        console.error(`   Steps completed: Login=${result.steps.login}, LogEntries=${result.steps.logEntries}, Dashboard=${result.steps.viewDashboard}, Logout=${result.steps.logout}`);
...
async function performLogin(page: Page, _trialNum: number): Promise<void> {
  ...
  // Wait for login form
  await page.waitForSelector('input[type="email"], input[name="email"], [formControlName="email"]', {
    timeout: 10000
  });

  // Fill credentials
  const emailSelector = 'input[type="email"], input[name="email"], [formControlName="email"]';
  const passwordSelector = 'input[type="password"], input[name="password"], [formControlName="password"]';
  
  await page.fill(emailSelector, TEST_CONFIG.TEST_USER_EMAIL);
  await page.fill(passwordSelector, TEST_CONFIG.TEST_USER_PASSWORD);

  // Submit login form
  await page.click('button[type="submit"]:has-text("Sign In"), button:has-text("Login"), button:has-text("Sign In")');
  ...
  await expect(page.locator('[data-testid="user-menu"], .user-menu, [aria-label*="user menu" i]')).toBeVisible({
    timeout: 5000
  });
}
 ...
 */
async function logTrainingEntries(page: Page, trialNum: number, count: number): Promise<void> {
  for (let i = 1; i <= count; i++) {
    console.log(`    Entry ${i}/${count}...`);
    
    // Navigate to training page
    await page.goto(`${TEST_CONFIG.BASE_URL}/training`, { waitUntil: 'domcontentloaded' });
    ...
    await page.waitForSelector('form, [role="dialog"] form, .training-form', { timeout: 5000 });

    // Fill training entry form
    const today = new Date().toISOString().split('T')[0];
    
    // Date field
    const dateSelectors = ['input[name="date"]', 'input[type="date"]', '[formControlName="date"]'];
    ...
    const durationSelectors = ['input[name="duration"]', '[formControlName="duration"]', 'input[placeholder*="duration" i]'];
    ...
    const intensitySelectors = ['input[name="intensity"]', '[formControlName="intensity"]', 'input[placeholder*="intensity" i]'];
    ...
    const typeSelectors = ['select[name="type"]', '[formControlName="type"]', 'p-dropdown[formcontrolname="type"]'];
    ...
    // Notes field
    const notesSelectors = ['textarea[name="notes"]', '[formControlName="notes"]', 'textarea[placeholder*="notes" i]'];
    for (const selector of notesSelectors) {
      try {
        if (await page.locator(selector).isVisible({ timeout: 1000 })) {
          await page.fill(selector, `Launch smoke test - Trial ${trialNum}, Entry ${i}`);
          break;
        }
      } catch { /* ignore */ }
    }
    ...
    try {
      await page.waitForSelector('.p-toast-message-success, .success-message, [role="alert"]:has-text("success")', {
        timeout: 5000,
        state: 'visible'
      });
    } catch {
...
async function viewDashboard(page: Page, _trialNum: number): Promise<void> {
  ...
  if (!elementFound) {
    throw new Error('Dashboard did not load correctly - no expected elements found');
  }
...
function generateSummaryReport(): void {
  ...
  const avgDuration = trialResults.reduce((sum, r) => sum + r.duration, 0) / totalTrials;
  ...
  if (failedTrials > 0) {
    console.log(`\nFailed Trials:`);
    trialResults.filter(r => r.status === 'FAIL').forEach(r => {
      console.log(`  Trial ${r.trial}: ${r.error}`);
      console.log(`    Steps: Login=${r.steps.login}, Log=${r.steps.logEntries}, Dashboard=${r.steps.viewDashboard}, Logout=${r.steps.logout}`);
    });
  }
  ...
  if (successRate === '100.00') {
    ...
    console.log(`⚠️  LAUNCH HOLD: Only ${successRate}% success rate (need 100%)`);
  }
  console.log('='.repeat(80) + '\n');
