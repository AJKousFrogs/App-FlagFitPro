...
class UniversalChartAccessibility {
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            const charts = node.querySelectorAll('canvas[id*="chart"], canvas[id*="Chart"], .chart-canvas canvas');
  ...
  analyzeChart(chartCanvas) {
    const container = chartCanvas.closest('.chart-container, .stat-card, .progress-section, .analytics-card');
    const title = container?.querySelector('h1, h2, h3, h4, h5, h6, .chart-title, .section-title');
  ...
  generateChartDescription(chartInfo) {
    ...
    switch (type) {
      ...
      case 'doughnut':
        if (dataPoints.length > 0) {
          const total = dataPoints.reduce((sum, val) => sum + val, 0);
          const percentages = dataPoints.map(val => Math.round((val / total) * 100));
          const segments = percentages.map((pct, i) => 
            `${labels[i] || `segment ${i + 1}`}: ${pct}%`
          );
  ...
  createTableToggle(chartCanvas, tableContainer, title) {
    const toggleBtn = document.createElement('button');
    toggleBtn.className = 'chart-table-toggle btn btn-secondary btn-sm';
    toggleBtn.textContent = 'View Data Table';
    toggleBtn.setAttribute('aria-expanded', 'false');
    toggleBtn.setAttribute('aria-controls', tableContainer.querySelector('table').id);
    ...
    const chartContainer = chartCanvas.closest('.chart-container') || chartCanvas.parentNode;
    if (chartContainer) {
  ...
  addKeyboardNavigation(chartCanvas, chartInfo) {
    ...
    helpText.textContent = 'Press Enter for summary, arrow keys to navigate data points, Home/End to go to first/last point';
  ...
  generateDetailedDescription(chartInfo) {
    ...
    if (data && data.datasets && data.datasets.length > 0) {
      ...
      if (dataPoints.length > 0) {
        ...
        description += 'Use arrow keys to navigate through individual data points. ';
      }
    }
    
    description += 'Press Enter to hear a summary, or use the View Data Table button for detailed information.';
