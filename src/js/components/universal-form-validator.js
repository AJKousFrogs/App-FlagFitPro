...
class UniversalFormValidator {
  ...
  setupFields() {
    ...
    formFields.forEach((field) => {
      ...
      if (fieldId) {
        fieldConfig.errorContainer = document.getElementById(
          `${fieldId}-error`,
        );
        fieldConfig.successContainer = document.getElementById(
          `${fieldId}-success`,
        );
        fieldConfig.helpContainer =
          document.getElementById(`${fieldId}-help`) ||
  ...
  setupAccessibility() {
    ...
    if (
      !this.form.getAttribute("aria-label") &&
      !this.form.getAttribute("aria-labelledby")
    ) {
  ...
  setupCSRFProtection() {
    ...
    let csrfField = this.form.querySelector(
  ...
  showFieldSuccess(field) {
    const config = this.fields.get(field);
    if (!config || !config.successContainer || !this.options.showSuccessStates)
      return;
