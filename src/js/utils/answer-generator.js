...
export class AnswerGenerator {
  ...
  generateDosageAnswer({ question, knowledge, articles, entities }) {
    ...
    if (knowledge && knowledge.answer && !bodyStats.weight && !bodyStats.height) {
    ...
    if (knowledge && knowledge.dosage_guidelines) {
      ...
      if (bodyStats.height && bodyStats.weight && supplement && supplement.includes('iron')) {
        const baseIron = 8;
    ...
    // Add safety warnings
    if (knowledge && knowledge.safety_warnings && knowledge.safety_warnings.length > 0) {
      answer += `\n**⚠️ Safety Warnings:**\n`;
    ...
    // Add absorption tips
    if (knowledge && knowledge.best_practices && knowledge.best_practices.length > 0) {
      answer += `\n**💡 Best Practices:**\n`;
  ...
  generateTimingAnswer({ question, knowledge, articles }) {
    ...
    if (knowledge && knowledge.protocols && knowledge.protocols.timing) {
      ...
      answer += 'Timing recommendations vary based on the specific supplement or method. ';
      ...
      answer += '• **With meals:** For better absorption and reduced stomach upset\n';
  ...
  generateHowToAnswer({ question, knowledge, articles, entities }) {
    const topic = entities.injuries[0] || entities.recovery[0] || entities.training[0] || 'this';
    ...
    if (knowledge && knowledge.protocols) {
      ...
      answer += knowledge.best_practices.map((step, i) => `${i + 1}. ${step}`).join('\n\n');
    } else if (articles.length > 0) {
  ...
  generateProtocolAnswer({ knowledge, articles }) {
    ...
    if (knowledge && knowledge.protocols) {
      ...
      answer += 'Protocol recommendations vary. Consult the latest research or a qualified professional for specific protocols.';
    }
  ...
  generateWhatIsAnswer({ knowledge, articles, entities }) {
    const topic = entities.supplements[0] || entities.recovery[0] || entities.training[0] || 'this';
  ...
  generateWhyAnswer({ knowledge, articles }) {
    ...
    if (knowledge && knowledge.answer) {
      const whyMatch = knowledge.answer.match(/because|due to|helps|improves|enhances/i);
      if (whyMatch) {
        answer += knowledge.answer;
      } else {
      ...
      answer += 'Research shows multiple benefits. Consult recent studies for specific mechanisms and outcomes.';
    }
  ...
  generateComparisonAnswer({ question, knowledge, articles }) {
    ...
    const comparisonMatch = question.original.match(/(\w+)\s+(?:vs|versus|or)\s+(\w+)/i);
    ...
    if (articles.length > 0) {
      ...
      answer += 'Both have their benefits. The best choice depends on your specific goals, body composition, and training schedule. ';
      answer += 'Consult with a sports nutritionist for personalized recommendations.';
    }
  ...
  generateGeneralAnswer({ knowledge, articles, question }) {
    ...
    if (knowledge && knowledge.answer) {
      ...
      answer = this.synthesizeGeneralAnswerFromArticles(articles, question.original);
    } else {
  ...
  formatProtocol(protocol) {
    ...
    if (protocol.steps) {
      formatted += `\n**Steps:**\n`;
      formatted += protocol.steps.map((step, i) => `${i + 1}. ${step}`).join('\n');
    }
  ...
  synthesizeDosageFromArticles(articles, supplement) {
    ...
    articles.forEach(article => {
      ...
      const dosageMatch = text.match(/(\d+(?:\.\d+)?)\s*(?:mg|g|mcg)\s*(?:per\s*day|daily|/day)/i);
      if (dosageMatch) {
    ...
    if (dosages.length > 0) {
      return `Based on research, recommended dosage ranges from ${dosages[0]} to ${dosages[dosages.length - 1]}. ` +
  ...
  extractProtocolFromArticles(articles) {
    ...
    return protocols.slice(0, 5).map(p => `• ${p}`).join('\n');
  }
  ...
  synthesizeDefinitionFromArticles(articles, topic) {
    ...
    return `Based on research, ${topic} is an important aspect of athletic performance. ` +
  ...
  extractBenefitsFromArticles(articles) {
    ...
    articles.forEach(article => {
      const text = `${article.abstract || ''} ${article.key_findings || ''}`;
      if (text.includes('improves') || text.includes('enhances') || text.includes('benefits')) {
        const benefitMatch = text.match(/(?:improves|enhances|benefits)\s+([^\.]+)/i);
        if (benefitMatch) {
    ...
    return benefits.slice(0, 5).map(b => `• ${b}`).join('\n');
  }

  extractBenefitsFromText(text) {
    const benefits = [];
    const benefitMatches = text.matchAll(/(?:improves|enhances|helps|benefits)\s+([^\.]+)/gi);
  ...
  addEvidenceFooter(knowledge, articles) {
    ...
    if (knowledge) {
      ...
      if (knowledge.supporting_articles && knowledge.supporting_articles.length > 0) {
  ...
  getDefaultDefinition(topic) {
    return `${topic} is an important aspect of athletic performance and recovery. ` +
