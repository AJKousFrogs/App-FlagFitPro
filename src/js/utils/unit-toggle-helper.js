 ...
 */
export function createUnitToggleHTML(
  type,
  inputId,
  unitLabelId,
  hiddenInputId,
  ...
  const configs = {
    weight: {
      imperial: {
        label: "Pounds (lbs)",
        unit: "lbs",
        storageKey: "flagfit_weight_unit",
      },
      metric: {
        label: "Kilograms (kg)",
        unit: "kg",
      ...
      default:
        defaultValue ||
        localStorage.getItem("flagfit_weight_unit") ||
        "imperial",
    },
    distance: {
      imperial: {
        label: "Yards",
        unit: "yds",
        storageKey: "flagfit_distance_unit",
      },
      metric: {
        label: "Meters",
        unit: "m",
      ...
      default:
        defaultValue ||
        localStorage.getItem("flagfit_distance_unit") ||
        "imperial",
    },
    time: {
      minutes: {
        label: "Minutes",
        unit: "min",
        storageKey: "flagfit_time_unit",
      },
      seconds: { label: "Seconds", unit: "s", storageKey: "flagfit_time_unit" },
      default:
        defaultValue || localStorage.getItem("flagfit_time_unit") || "minutes",
    },
  };
  ...
  const isImperial =
    config.default === "imperial" || config.default === "minutes";
...
export function setupUnitToggle(type, inputId, unitLabelId, hiddenInputId) {
  ...
  const configs = {
    ...
    distance: {
      imperial: {
        unit: "yds",
        storageKey: "flagfit_distance_unit",
        convert: (val) => val * 1.09361,
        reverse: (val) => val / 1.09361,
      },
      metric: {
        unit: "m",
        storageKey: "flagfit_distance_unit",
  ...
  const savedUnit =
    localStorage.getItem(config.imperial.storageKey) ||
    ...
    if (currentUnit !== imperialKey && input.value) {
      ...
      if (!isNaN(currentValue)) {
        ...
        input.value = convertedValue.toFixed(
          type === "weight" ? 1 : type === "distance" ? 1 : 0,
        );
      }
    }
...
export function convertValue(value, type, fromUnit, toUnit = null) {
  ...
  switch (type) {
    case "weight":
      return unitManager.convertWeight(
        value,
        fromUnit === "imperial" ? "lbs" : "kg",
        toUnit === "imperial" ? "lbs" : "kg",
      );
    case "distance":
      return unitManager.convertDistance(
        value,
        fromUnit === "imperial" ? "yards" : "meters",
        toUnit === "imperial" ? "yards" : "meters",
      );
    case "time":
