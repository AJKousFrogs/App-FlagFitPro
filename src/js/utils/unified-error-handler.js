...
export class UnifiedErrorHandler {
  ...
  trackError(errorInfo, context) {
    ...
    this.diagnostics.errorCounts[key] =
      (this.diagnostics.errorCounts[key] || 0) + 1;
    this.diagnostics.errorTypes[errorInfo.type] =
      (this.diagnostics.errorTypes[errorInfo.type] || 0) + 1;
   ...
   */
  getDiagnostics() {
    const totalErrors = Object.values(this.diagnostics.errorCounts).reduce(
      (a, b) => a + b,
      0,
    );
    const errorRate =
      totalErrors > 0
        ? (this.diagnostics.errorCounts[
            `${ErrorType.NETWORK}_${ErrorSeverity.ERROR}`
          ] || 0) / totalErrors
        : 0;
    ...
    return {
      ...
      summary: {
        ...
        mostCommonError:
          Object.entries(this.diagnostics.errorTypes).sort(
            ([, a], [, b]) => b - a,
          )[0]?.[0] || "none",
        networkStatus: this.diagnostics.networkStatus,
        performanceIssues: this.diagnostics.performanceMetrics.filter(
          (m) => m.type === "longTask",
        ).length,
      },
