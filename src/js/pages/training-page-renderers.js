...
function renderDayCard(day, scheduleSettings) {
    const { date, dayName, isToday, isGameDay, isCompleted, plannedWorkout } = day;
 ...
 */
export function renderWeeklySchedule(weeklySchedule, scheduleSettings, viewMode = 'detailed') {
    ...
    if (toggle) {
        toggle.textContent = viewMode === 'detailed' ? 'Compact View' : 'Detailed View';
    }
...
export function renderWorkoutCards() {
    ...
    const workoutCards = workoutsSection.querySelectorAll('.workout-card[data-workout-type]');
