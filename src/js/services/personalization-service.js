...
class PersonalizationService {
  ...
  generatePersonalizedRecommendations(parsedQuestion, baseAnswer) {
    ...
    if (hasActiveInjuries && activeInjuries && activeInjuries.length > 0) {
      personalized += this.getInjuryAwareAdvice(
        activeInjuries,
    ...
    if (
      bodyStats &&
      (parsedQuestion.intent === "dosage" ||
        parsedQuestion.entities.supplements?.length > 0)
    ) {
  ...
  getInjuryAwareAdvice(activeInjuries, intent, entities) {
    ...
    const questionAboutInjury = entities.injuries?.some((i) =>
      injuryTypes.some(
        (it) => i.toLowerCase().includes(it) || it.includes(i.toLowerCase()),
      ),
    );
    ...
    if (questionAboutInjury) {
      ...
      advice +=
        "• Consult with your healthcare provider before starting any new protocols\n";
    ...
    if (
      injuryTypes.some(
        (it) => it.includes("hamstring") || it.includes("hamstring"),
      )
    ) {
      advice += "\n**Hamstring-Specific:**\n";
