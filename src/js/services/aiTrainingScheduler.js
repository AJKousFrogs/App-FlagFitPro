...
class AITrainingScheduler {
  ...
  calculatePostEventRecovery(daysSinceEvent, nextEventImportance = null) {
    ...
    let phase;
    if (daysSinceEvent <= 1) {
      phase = recoveryPhases["0-1"];
    } else if (daysSinceEvent <= 3) {
      phase = recoveryPhases["2-3"];
    } else if (daysSinceEvent <= 5) {
      phase = recoveryPhases["4-5"];
    } else if (daysSinceEvent <= 7) {
      phase = recoveryPhases["6-7"];
    } else {
      phase = null;
  ...
  getAllRelevantDates(playerProfile, startDate, endDate) {
    ...
    dates.tournaments = this.tournamentDates.filter((t) => {
  ...
  groupDatesByWeek(allDates, startDate, endDate) {
    ...
    while (currentDate <= endDate) {
      ...
      const week = {
        ...
        dates: {
          tournaments: allDates.tournaments.filter((t) => {
  ...
  determinePeriodization(week, allDates) {
    ...
    if (upcomingTournaments.length > 0) {
      ...
      const isTournamentWeek =
        nearestTournament.startDate <= weekEndDate &&
      ...
      if (isTournamentWeek) {
        ...
        adjustments.rationale =
          "Tournament competition week - minimal training outside tournament days";
      } else if (taperParams) {
  ...
  generateDaySchedule(dayDate, week, periodization, playerProfile, allDates) {
    ...
    const tournament = week.dates.tournaments.find((t) => {
    ...
    if (daySchedule.training && daySchedule.training.volume < 0.1) {
      ...
      daySchedule.training.intensity = Math.max(
        daySchedule.training.intensity,
        0.3,
      );
    }
  ...
  getRecommendations(options = {}) {
    ...
    try {
      ...
      const upcomingTournaments = this.tournamentDates.filter((t) => {
        const daysUntil = Math.floor(
          (t.startDate - week) / (1000 * 60 * 60 * 24),
        );
      ...
      if (upcomingTournaments.length > 0) {
        const nearestTournament = upcomingTournaments[0];
        const daysUntil = Math.floor(
          (nearestTournament.startDate - week) / (1000 * 60 * 60 * 24),
        );
        const eventImportance =
          nearestTournament.eventImportance || EVENT_IMPORTANCE.MEDIUM;
        ...
        if (daysUntil <= 14 && daysUntil > 0) {
          ...
          if (taperParams) {
            recommendations.push({
              ...
              priority:
                eventImportance === EVENT_IMPORTANCE.MAJOR ? "high" : "medium",
              action: {
                type: "apply_taper",
          ...
          recommendations.push({
            ...
            action: {
              ...
              sessions: sessions.map(
                (s) => s.title || s.name || "Training session",
              ),
            },
          });
        }
      ...
      const sortedSessions = [...currentSchedule].sort(
        ...
        if (
          ...
          recommendations.push({
            ...
            action: {
              type: "add_recovery",
              sessions: [
                current.title || "Session 1",
                next.title || "Session 2",
              ],
            },
          });
        }
      ...
      if (upcomingEvents.length > 0) {
        ...
        recommendations.push({
          ...
          action: {
            ...
            event: event.name,
            startDate: new Date(
              event.startDate.getTime() - 21 * 24 * 60 * 60 * 1000,
            ), // 3 weeks before
          },
        });
      }
      ...
      logger.warn(
        "[AITrainingScheduler] Error generating recommendations:",
        error,
      );
    }
