...
const LoginPage = ({ onLogin }) => {
  ...
  // Use safe form validation hook
  const { setFieldValidation, clearFieldValidation, clearAllValidation } = useSafeFormValidation({
    formRef,
    onValidationChange: (fieldName, message) => {
      console.log(`Validation changed for ${fieldName}:`, message);
    },
    onValidationClear: (fieldName) => {
      console.log(`Validation cleared for ${fieldName}`);
    }
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Call the onLogin function from App.jsx if provided
      if (onLogin) {
        onLogin(formData);
      }
      ...
      if (err.message.includes('Too many login attempts')) {
        const generalError = 'Account temporarily locked due to too many failed attempts. Please try again later.';
        setErrors({ general: generalError });
      } else if (err.message.includes('Invalid credentials')) {
        const generalError = 'Invalid email or password. Please check your credentials and try again.';
        setErrors({ general: generalError });
      } else {
