...
export class EventCleanupManager {
    ...
    removeEventListener(listenerId) {
        ...
        if (listener) {
            ...
            logger.debug(`Event listener removed: ${listener.event} (ID: ${listenerId})`);
        }
    }
    ...
    createObserver(ObserverClass, callback) {
        ...
        return {
            ...
            disconnect: () => {
                ...
                logger.debug(`Observer disconnected: ${ObserverClass.name} (ID: ${observerId})`);
            }
        };
    }
     ...
     */
    cleanup() {
        logger.debug(`Starting cleanup: ${this.registeredListeners.size} listeners, ${this.timers.size} timers, ${this.intervals.size} intervals, ${this.observers.size} observers`);
    ...
    getStats() {
        return {
            ...
            totalResources: this.registeredListeners.size + this.timers.size + 
                          this.intervals.size + this.observers.size + this.abortControllers.size
        };
    }
...
export class ComponentWithCleanup {
    ...
    addEventListener(target, event, handler, options) {
        return this.cleanupManager.addEventListener(target, event, handler, options);
    }
