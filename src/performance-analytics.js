...
export class PerformanceAnalytics {
    ...
    savePerformanceData() {
        try {
            localStorage.setItem(this.storageKey, JSON.stringify(this.performanceData));
        } catch (error) {
    ...
    generateSampleData(data) {
        ...
        for (let i = daysBack; i >= 0; i--) {
            ...
            if (i % 7 === 0) {
                ...
                data.performanceTrends.speed.push(70 + Math.random() * 15 + (daysBack - i) / 10);
                data.performanceTrends.agility.push(75 + Math.random() * 15 + (daysBack - i) / 12);
                data.performanceTrends.strength.push(65 + Math.random() * 20 + (daysBack - i) / 8);
                data.performanceTrends.endurance.push(72 + Math.random() * 18 + (daysBack - i) / 15);
                
                const avg = (data.performanceTrends.speed[data.performanceTrends.speed.length - 1] +
                           data.performanceTrends.agility[data.performanceTrends.agility.length - 1] +
                           data.performanceTrends.strength[data.performanceTrends.strength.length - 1] +
                           data.performanceTrends.endurance[data.performanceTrends.endurance.length - 1]) / 4;
            ...
            if (Math.random() > 0.3) { // 70% chance of training day
        ...
        testDates.forEach((date, index) => {
            ...
            data.performanceTests.fortyYardDash.push(4.8 - (index * 0.05) + (Math.random() * 0.1 - 0.05));
            data.performanceTests.verticalJump.push(28 + (index * 0.5) + (Math.random() * 2 - 1));
            data.performanceTests.broadJump.push(9.2 + (index * 0.1) + (Math.random() * 0.2 - 0.1));
            data.performanceTests.proAgility.push(4.5 - (index * 0.03) + (Math.random() * 0.1 - 0.05));
            data.performanceTests.lDrill.push(7.2 - (index * 0.05) + (Math.random() * 0.1 - 0.05));
            data.performanceTests.benchPress.push(185 + (index * 5) + (Math.random() * 10 - 5));
            data.performanceTests.squat.push(225 + (index * 10) + (Math.random() * 15 - 7.5));
        });
    }
    ...
    updatePerformanceScores() {
        ...
        if (tests.squat.length > 0 && tests.benchPress.length > 0) {
            ...
            scores.strength = Math.max(0, Math.min(100, (latestSquat / 3 + latestBench / 2.5) / 2));
        }
        
        // Calculate overall score
        scores.overall = Math.round((scores.speed + scores.agility + scores.strength + scores.endurance) / 4);
    ...
    getPerformanceStats(timeframe = '30d') {
        ...
        return {
            performanceTrends: this.filterDataByDate(this.performanceData.performanceTrends, cutoffDate),
            wellness: this.filterDataByDate(this.performanceData.wellness, cutoffDate),
            training: this.filterDataByDate(this.performanceData.training, cutoffDate),
    ...
    filterDataByDate(data, cutoffDate) {
        ...
        Object.keys(filteredData).forEach(key => {
            if (Array.isArray(filteredData[key])) {
                filteredData[key] = filteredData[key].filter((_, index) => validIndices.includes(index));
            }
        });
    ...
    calculateImprovements(timeframe) {
        const improvements = {};
        const tests = this.performanceData.performanceTests;
    ...
    getWellnessSummary(days = 7) {
        ...
        const recentData = wellness.dates.slice(-days);
        
        const summary = {};
        ['sleep', 'energy', 'soreness', 'stress', 'nutrition', 'hydration'].forEach(metric => {
    ...
    getTrainingLoadAnalysis() {
        ...
        return {
            ...
            averageIntensity: training.intensityScore.slice(-7).reduce((a, b) => a + b, 0) / Math.min(7, training.intensityScore.length),
            recoveryScore: training.recoveryScore.slice(-7).reduce((a, b) => a + b, 0) / Math.min(7, training.recoveryScore.length),
    ...
    getTrainingRecommendation() {
        ...
        if (wellness.soreness.average > 3.5) {
            return { type: 'recovery', message: 'High soreness detected. Consider recovery day.' };
        } else if (wellness.energy.average < 6) {
            return { type: 'light', message: 'Low energy levels. Light training recommended.' };
        } else if (training.averageIntensity > 8.5) {
            return { type: 'moderate', message: 'High recent intensity. Moderate session today.' };
        } else {
            return { type: 'normal', message: 'All systems go! Normal training intensity.' };
        }
    }
        ...
        this.chartInstances[canvasId] = new Chart(canvas, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: [scores.speed, scores.agility, scores.strength, scores.endurance, scores.overall],
...
export const PerformanceCalculations = {
    ...
    getPerformanceGrade(testType, value, age = 20, gender = 'male') {
        ...
        if (['fortyYardDash', 'proAgility', 'lDrill'].includes(testType)) {
            ...
            if (value <= testStandards.excellent) percentile = 95;
            else if (value <= testStandards.good) percentile = 80;
            else if (value <= testStandards.average) percentile = 60;
            else if (value <= testStandards.belowAverage) percentile = 40;
            else percentile = 20;
        } else {
    ...
    getPerformanceStandards(age = 20, gender = 'male') {
        // These are example standards - would need sport-specific research data
        if (gender === 'male') {
            return {
                fortyYardDash: { excellent: 4.3, good: 4.5, average: 4.7, belowAverage: 5.0 },
                verticalJump: { excellent: 32, good: 28, average: 24, belowAverage: 20 },
                broadJump: { excellent: 10.0, good: 9.5, average: 9.0, belowAverage: 8.5 },
                proAgility: { excellent: 4.0, good: 4.2, average: 4.4, belowAverage: 4.6 },
                lDrill: { excellent: 6.8, good: 7.0, average: 7.3, belowAverage: 7.6 },
                benchPress: { excellent: 225, good: 185, average: 155, belowAverage: 135 },
                squat: { excellent: 315, good: 275, average: 225, belowAverage: 185 }
            };
        } else {
            return {
                fortyYardDash: { excellent: 4.8, good: 5.0, average: 5.2, belowAverage: 5.5 },
                verticalJump: { excellent: 24, good: 20, average: 16, belowAverage: 12 },
                broadJump: { excellent: 8.5, good: 8.0, average: 7.5, belowAverage: 7.0 },
                proAgility: { excellent: 4.4, good: 4.6, average: 4.8, belowAverage: 5.0 },
                lDrill: { excellent: 7.2, good: 7.5, average: 7.8, belowAverage: 8.1 },
                benchPress: { excellent: 135, good: 115, average: 95, belowAverage: 75 },
