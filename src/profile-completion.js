...
export class ProfileCompletionManager {
    ...
    modal.innerHTML = `
      ...
      <div class="profile-completion-content">
        <div class="profile-completion-header">
          ...
          ${!required ? '<button class="profile-completion-close" onclick="window.profileCompletionManager.closeModal()" aria-label="Close dialog">
  ...
  async handleSubmit(e, required) {
    ...
    if (required) {
      setTimeout(() => {
        if (window.authManager && typeof window.authManager.redirectToDashboard === 'function') {
