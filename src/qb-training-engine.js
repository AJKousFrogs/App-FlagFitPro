...
import { QB_TRAINING_PROGRAM, QB_EXERCISE_LIBRARY, QB_ASSESSMENTS, TOURNAMENT_SIMULATION } from './qb-training-program-data.js';
...
export class QBTrainingEngine extends TrainingProgramEngine {
    ...
    getQBWeekData(weekNumber) {
        ...
        return this.mergeDualTrackWorkouts(lowerBodyData, qbUpperBodyData, weekNumber, phase);
    }
    ...
    getQBUpperBodyWorkouts(weekNumber, phase) {
        ...
        return this.applyQBProgressions(baseTemplate, weekNumber, phase, weekInPhase);
    }
        ...
        const templates = {
            foundation: {
                armStrength: {
                    ...
                    exercises: [
                        { name: "Band External Rotation", sets: 3, reps: 15, load: "Light band" },
                        { name: "Band Internal Rotation", sets: 3, reps: 12, load: "Light band" },
                ...
                mobility: {
                    ...
                    exercises: [
                        ...
                        { name: "Thoracic Extension", sets: 2, reps: 10, load: "Bodyweight" },
                        { name: "Cross-body Stretch", sets: 2, duration: "45s", side: "each" }
                    ]
                },
                throwing: {
                    frequency: 3,
            ...
            strength: {
                armStrength: {
                    frequency: 3,
                    exercises: [
                        { name: "Band External Rotation", sets: 4, reps: 12, load: "Medium band" },
                        { name: "Band Internal Rotation", sets: 3, reps: 15, load: "Medium band" },
                        { name: "Weighted External Rotation", sets: 3, reps: 8, load: "3-5 lbs" },
                        { name: "Lat Pulldowns", sets: 3, reps: 10, load: "75% BW" },
                        { name: "Face Pulls", sets: 3, reps: 12, load: "Medium resistance" }
                    ]
                },
                mobility: {
                    frequency: 6,
                    exercises: [
                        ...
                        { name: "Thoracic Extension", sets: 3, reps: 12, load: "Light weight" },
                        { name: "Posterior Capsule Stretch", sets: 2, duration: "60s", side: "each" }
                    ]
                },
                throwing: {
                    frequency: 4,
                    volume: "250-350 throws",
                    focus: ["Velocity development", "Endurance building", "Weighted balls"]
                }
            },
            power: {
                armStrength: {
                    frequency: 4,
                    exercises: [
                        { name: "Medicine Ball Rotational Throws", sets: 4, reps: 8, load: "8-10 lbs" },
                        { name: "Weighted Ball Throws", sets: 3, reps: 6, load: "16 oz ball" },
                        { name: "Explosive External Rotation", sets: 3, reps: 6, load: "Heavy band" },
                ...
                mobility: {
                    frequency: 6,
                    exercises: [
                        { name: "Dynamic Sleeper Stretch", sets: 3, reps: 15, side: "each" },
                ...
                throwing: {
                    ...
                    focus: ["Peak velocity", "Tournament simulation", "Endurance testing"]
                }
            },
            competition: {
                armStrength: {
                    frequency: 2,
                    exercises: [
                        { name: "Band External Rotation", sets: 2, reps: 12, load: "Light band" },
                ...
                mobility: {
                    ...
                    exercises: [
                        { name: "Sleeper Stretch", sets: 2, duration: "60s", side: "each" },
                        { name: "Couch Stretch", sets: 2, duration: "90s", side: "each" },
                        { name: "Gentle Thoracic Mobility", sets: 2, reps: 10, load: "Bodyweight" }
                    ]
                },
                throwing: {
                    frequency: 3,
                    volume: "200-320 throws",
                    focus: ["Velocity maintenance", "Competition simulation", "Mental preparation"]
                }
            }
        };
    ...
    mergeDualTrackWorkouts(lowerBodyData, qbUpperBodyData, weekNumber, phase) {
        ...
        Object.keys(mergedData.days).forEach(day => {
            ...
            dayData.qbWork = this.getQBWorkForDay(day, dayIndex, qbUpperBodyData, weekNumber, phase);
        ...
        mergedData.qbSpecific = {
            ...
            assessmentWeek: QB_ASSESSMENTS.throwingVelocity.frequency.includes(`Week ${weekNumber}`),
    ...
    getSessionVolume(sessionType, weeklyTarget, weekNumber) {
        const sessionMultipliers = {
            mechanics: 0.15,    // 15% of weekly volume
            progression: 0.25,  // 25% of weekly volume  
            velocity: 0.20,     // 20% of weekly volume
            endurance: 0.30,    // 30% of weekly volume
    ...
    logThrowingSession(weekNumber, day, sessionData) {
        ...
        this.throwingVolume.weeklyVolumes[`week${weekNumber}`] += sessionData.volume;
    ...
    saveThrowingProgress() {
        try {
            localStorage.setItem('flagfit_throwing_progress', JSON.stringify(this.throwingVolume));
        } catch (error) {
    ...
    getThrowingVolumeProgress() {
        ...
        const targetVolume = this.parseVolumeRange(QB_TRAINING_PROGRAM.throwingVolumeProgression.byPhase[phase]);
        const currentWeekVolume = this.throwingVolume.weeklyVolumes[`week${status.currentWeek}`] || 0;
    ...
    initializeArmCareSchedule() {
        return {
            daily: {
                ...
                evening: ["Posterior capsule stretch", "Cross-body stretch", "Gentle rotation"]
            },
            postThrowing: ["Progressive cool-down throws", "Ice if needed", "Gentle stretching"],
            offDays: ["Extended mobility work", "Soft tissue work", "Recovery protocols"]
        };
    }
    ...
    getQBSpecificProgress() {
        ...
        return Math.round((throwingProgress + velocityProgress + mobilityProgress) / 3);
    }
