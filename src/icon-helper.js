...
function renderIcon(iconName, size = 20, color = 'currentColor', strokeWidth = 2) {
...
function replaceEmojiWithIcon(element, emoji, iconName, size = 20) {
    if (element.textContent && element.textContent.includes(emoji)) {
        element.innerHTML = element.innerHTML.replace(emoji, `<i data-lucide="${iconMap[iconName] || iconName}" style="width: ${size}px; height: ${size}px;"></i>`);
    }
}
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { getIcon, renderIcon, replaceEmojiWithIcon, initLucideIcons, iconMap };
}
