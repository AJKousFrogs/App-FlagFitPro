...
export class AccessibilityUtils {
  ...
  static enhanceFormAccessibility() {
    document.querySelectorAll('form').forEach(form => {
      ...
      form.addEventListener('submit', (e) => {
        ...
        if (errors.length > 0) {
          this.announce(`Form has ${errors.length} errors. Please review and correct.`, 'assertive');
        }
      });
  ...
  static enhanceInputAccessibility(input) {
    ...
    input.addEventListener('invalid', () => {
      ...
      this.announce(`${label?.textContent || 'Field'}: ${input.validationMessage}`, 'assertive');
    });
  ...
  static handleEscapeKey() {
    ...
    // Close dropdowns
    document.querySelectorAll('.dropdown.open, .menu.open').forEach(dropdown => {
  ...
  static handleArrowKeyNavigation(e) {
    ...
    if (target.getAttribute('role') === 'tab') {
      e.preventDefault();
      const tabs = Array.from(target.closest('[role="tablist"]').querySelectorAll('[role="tab"]'));
