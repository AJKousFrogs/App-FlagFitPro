...
import { TRAINING_PROGRAM, WEEKLY_SCHEDULES, EXERCISE_LIBRARY, PERFORMANCE_TESTS } from './training-program-data.js';
...
export class TrainingProgramEngine {
    ...
    getProgramStatus() {
        ...
        const daysSinceStart = Math.floor((currentDate - this.programStartDate) / (1000 * 60 * 60 * 24));
    ...
    modifyExercise(exercise, weekInPhase, phase, totalWeek) {
        ...
        if (exercise.sets && progressionRules.volumeIncrease) {
            ...
            exercise.sets = Math.min(baseSet + Math.floor((weekInPhase - 1) * progressionRules.volumeIncrease), baseSet + 2);
        }

        // Apply intensity progression
        if (exercise.load && progressionRules.intensityIncrease) {
            exercise.load = this.progressLoad(exercise.load, weekInPhase, progressionRules.intensityIncrease);
        }
    ...
    progressLoad(loadString, weekInPhase, intensityIncrease) {
        ...
        if (loadString.includes('%')) {
            ...
            const newPercentage = Math.round(percentage * (1 + intensityIncrease * (weekInPhase - 1)));
    ...
    isSprintExercise(exercise) {
        const sprintKeywords = ['sprint', 'acceleration', 'flying', 'dash', 'tempo'];
    ...
    progressSprintWork(exercise, phase, weekInPhase) {
        ...
        if (exercise.sets) {
            ...
            exercise.sets = Math.round(baseSets * Math.pow(rules.sprintVolumeMultiplier, weekInPhase - 1));
        }
    ...
    progressPlyometrics(exercise, phase, weekInPhase) {
        ...
        if (exercise.sets) {
            const baseSets = parseInt(exercise.sets);
            exercise.sets = Math.round(baseSets * Math.pow(rules.plyoVolumeMultiplier, weekInPhase - 1));
        }
        
        // Progress box height for box jumps
        if (exercise.name.toLowerCase().includes('box jump') && exercise.boxHeight) {
    ...
    getExerciseVariation(exerciseName, userLevel = 'intermediate', conditions = {}) {
    ...
    getCompletionStats() {
        ...
        const completedCount = Object.keys(this.userProgress.completedWorkouts || {}).length;
    ...
    getPerformanceTests(weekNumber) {
        ...
        const phase = this.getPhaseForWeek(weekNumber);
        const tests = {
            foundation: ['40-Yard Sprint', 'Vertical Jump', 'Broad Jump', 'Nordic Curl Max'],
            strength: ['40-Yard Sprint', 'Vertical Jump', 'Broad Jump', 'Pro-Agility'],
            power: ['40-Yard Sprint', 'Vertical Jump', 'Broad Jump', 'Pro-Agility', 'L-Drill'],
    ...
    saveUserProgress() {
        try {
            localStorage.setItem('flagfit_training_progress', JSON.stringify(this.userProgress));
        } catch (error) {
