...
export class PerformanceAPI {
    ...
    async savePhysicalMeasurements(data) {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.measurements}`, {
                method: 'POST',
    ...
    async getPhysicalMeasurements(athleteId, timeframe = '6m') {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.measurements}?athlete=${athleteId}&timeframe=${timeframe}`, {
                headers: this.getAuthHeaders()
            });
    ...
    async savePerformanceTest(testType, data) {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.performanceTests}`, {
                method: 'POST',
                headers: this.getAuthHeaders(),
                body: JSON.stringify({
                    testType: testType,
    ...
    async getPerformanceHistory(testType, timeframe = '12m') {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.performanceTests}/${testType}?timeframe=${timeframe}`, {
                headers: this.getAuthHeaders()
            });
    ...
    async saveWellnessData(data) {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.wellness}`, {
                method: 'POST',
                headers: this.getAuthHeaders(),
                body: JSON.stringify({
                    sleep: data.sleep,
    ...
    async getWellnessHistory(timeframe = '30d') {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.wellness}?timeframe=${timeframe}`, {
                headers: this.getAuthHeaders()
            });
    ...
    async logSupplementIntake(supplementName, time, notes = '') {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.supplements}`, {
                method: 'POST',
                headers: this.getAuthHeaders(),
                body: JSON.stringify({
            ...
            return this.saveToLocalStorage('supplements', { supplement: supplementName, time, notes });
        }
    }

    async getSupplementHistory(timeframe = '30d') {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.supplements}?timeframe=${timeframe}`, {
                headers: this.getAuthHeaders()
            });
    ...
    async reportInjury(injuryData) {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.injuries}`, {
                method: 'POST',
                headers: this.getAuthHeaders(),
                body: JSON.stringify({
    ...
    async updateInjuryStatus(injuryId, status, notes = '') {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.injuries}/${injuryId}`, {
                method: 'PATCH',
                headers: this.getAuthHeaders(),
                body: JSON.stringify({
                    status: status,
    ...
    async getPerformanceTrends(timeframe = '12m') {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.trends}?timeframe=${timeframe}`, {
                headers: this.getAuthHeaders()
            });
    ...
    async getComparativeAnalysis(athleteIds, metrics) {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.trends}/compare`, {
                method: 'POST',
                headers: this.getAuthHeaders(),
                body: JSON.stringify({
                    athletes: athleteIds,
    ...
    async exportPerformanceData(format = 'csv', timeframe = '12m') {
        try {
            const response = await fetch(`${this.baseUrl}/athlete/export?format=${format}&timeframe=${timeframe}`, {
                headers: this.getAuthHeaders()
            });
...
export const TrendAnalyzer = {
    ...
    generateInsights(performanceData, wellnessData) {
        ...
        if (wellnessData.length > 5) {
            const avgSleep = wellnessData.reduce((sum, w) => sum + w.sleep, 0) / wellnessData.length;
            const avgStress = wellnessData.reduce((sum, w) => sum + w.stress, 0) / wellnessData.length;
            
            if (avgSleep < 6) {
                insights.push({
                    type: 'sleep_concern',
                    message: 'Low sleep quality may be impacting performance',
                    recommendation: 'Focus on sleep hygiene and consider magnesium supplementation'
                });
            }
            ...
            if (avgStress > 7) {
                insights.push({
                    ...
                    recommendation: 'Consider stress management techniques and recovery protocols'
                });
            }
        }
