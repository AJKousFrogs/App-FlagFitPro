...
class AnalyticsDataService {
  ...
  async getPerformanceTrendsData(userId = "1", weeks = 7) {
    ...
    try {
      const data = await this.apiCall(
        `/analytics/performance-trends?userId=${userId}&weeks=${weeks}`,
      );
  ...
  async getTeamChemistryData(userId = "1") {
    ...
    try {
      const data = await this.apiCall(
        `/analytics/team-chemistry?userId=${userId}`,
      );
      this.setCachedData(cacheKey, data);
  ...
  async getTrainingDistributionData(userId = "1", period = "30days") {
    ...
    try {
      const data = await this.apiCall(
        `/analytics/training-distribution?userId=${userId}&period=${period}`,
      );
      this.setCachedData(cacheKey, data);
  ...
  async getPositionPerformanceData(userId = "1") {
    ...
    try {
      const data = await this.apiCall(
        `/analytics/position-performance?userId=${userId}`,
      );
      this.setCachedData(cacheKey, data);
  ...
  async getOlympicQualificationData(userId = "1") {
    ...
    try {
      const data = await this.apiCall(
        `/dashboard/olympic-qualification?userId=${userId}`,
      );
      this.setCachedData(cacheKey, data);
  ...
  async getInjuryRiskData(userId = "1") {
    ...
    try {
      const data = await this.apiCall(
        `/analytics/injury-risk?userId=${userId}`,
      );
      this.setCachedData(cacheKey, data);
  ...
  async getSpeedDevelopmentData(userId = "1", weeks = 7) {
    ...
    try {
      const data = await this.apiCall(
        `/analytics/speed-development?userId=${userId}&weeks=${weeks}`,
      );
      this.setCachedData(cacheKey, data);
  ...
  async getUserEngagementData(period = "30days") {
    ...
    try {
      const data = await this.apiCall(
        `/analytics/user-engagement?period=${period}`,
      );
      this.setCachedData(cacheKey, data);
  ...
  formatTrainingDistributionData(data) {
    try {
      const trainingTypes = data.trainingTypes || [
        "Agility Training",
        "Speed Development",
        "Technical Skills",
        "Strength Training",
        ...
        datasets: [
          {
            ...
            data: data.fortyYardTimes || [
              4.65, 4.62, 4.58, 4.55, 4.52, 4.49, 4.46,
            ],
            borderColor: "#F59E0B",
          ...
          },
          {
            ...
            data: data.tenYardTimes || [
              1.68, 1.65, 1.62, 1.6, 1.58, 1.56, 1.54,
            ],
            borderColor: "#F97316",
