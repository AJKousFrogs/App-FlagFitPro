...
export class EnhancedChartConfig {
    ...
    getDatasetConfig(label, data, color, options = {}) {
        const isDark = this.theme === 'dark';
        const baseColor = typeof color === 'string' ? color : color.color || this.colors.overall;
        ...
        return {
            ...
            pointHoverRadius: options.pointHoverRadius !== undefined ? options.pointHoverRadius : 6,
    ...
    getPerformanceTrendsConfig(data, timeframe = '6m') {
        return {
            type: 'line',
            data: {
                labels: data.dates.map(date => new Date(date).toLocaleDateString()),
                datasets: [
                    this.getDatasetConfig('Overall', data.overall, this.colors.overall, { fill: true }),
                    this.getDatasetConfig('Speed', data.speed, this.colors.speed),
                    this.getDatasetConfig('Agility', data.agility, this.colors.agility),
                    this.getDatasetConfig('Strength', data.strength, this.colors.strength),
                    this.getDatasetConfig('Endurance', data.endurance, this.colors.endurance)
                ]
            },
            options: {
                responsive: true,
    ...
    getWellnessChartConfig(data, timeframe = '30d') {
        return {
            type: 'line',
            data: {
                labels: data.dates.map(date => new Date(date).toLocaleDateString()),
                datasets: [
                    ...
                    this.getDatasetConfig('Energy (1-10)', data.energy, this.colors.energy, {
                        yAxisID: 'y1',
                        pointRadius: 3
                    }),
                    this.getDatasetConfig('Soreness (1-5)', data.soreness, this.colors.soreness, {
                        yAxisID: 'y1',
                        pointRadius: 3
                    })
                ]
