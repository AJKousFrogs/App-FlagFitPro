...
import { performanceAPI, TrendAnalyzer } from './performance-api.js';
...
export class PerformanceCharts {
    ...
    async createPerformanceTrendsChart(canvasId) {
        ...
        try {
            const performanceHistory = await performanceAPI.getPerformanceHistory('all', '6m');
    ...
    async createBodyCompositionChart(canvasId) {
        ...
        try {
            const measurementHistory = await performanceAPI.getPhysicalMeasurements('current', '6m');
    ...
    prepareOverviewData(trends) {
        const performanceScores = {
            ...
            'Agility': this.calculatePerformanceScore(trends.performance?.['ProAgility']),
            'Endurance': this.calculatePerformanceScore(trends.performance?.['300YardShuttle']),
    ...
    showMockPerformanceChart(canvas) {
        ...
        ctx.fillText('Performance data will appear here', canvas.width/2, canvas.height/2);
        ctx.fillText('when connected to backend', canvas.width/2, canvas.height/2 + 25);
    }
    ...
    showMockWellnessChart(canvas) {
        ...
        ctx.font = '16px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Wellness trends will appear here', canvas.width/2, canvas.height/2);
    }
    ...
    showMockBodyCompositionChart(canvas) {
        ...
        ctx.font = '16px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Body composition trends will appear here', canvas.width/2, canvas.height/2);
    }
    ...
    showMockOverviewChart(canvas) {
        ...
        ctx.font = '16px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Performance overview will appear here', canvas.width/2, canvas.height/2);
    }

    showChartError() {
        console.warn('Charts could not be initialized - falling back to mock data display');
    }
