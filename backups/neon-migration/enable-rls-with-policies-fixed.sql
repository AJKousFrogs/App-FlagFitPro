-- Enable Row Level Security (RLS) with Policies (FIXED TYPE CASTING)
-- Generated: 2025-12-02
-- IMPORTANT: Review and customize these policies for your application's security needs

-- =============================================================================
-- TEMPORARY SOLUTION: Keep RLS disabled for development
-- =============================================================================
--
-- Since your app doesn't have Supabase Auth fully integrated yet,
-- we'll keep RLS disabled temporarily. You can enable it later when ready.
--
-- For now, this script documents what the policies SHOULD be when you're ready.
--
-- =============================================================================
...
/*
-- =============================================================================
-- USER DATA TABLES (Requires Supabase Auth)
-- =============================================================================

-- Users table - users can only see/edit their own data
-- Assuming 'id' column is INTEGER, not UUID
...
CREATE POLICY "Users can view their own data"
  ...
  USING (
    CASE
      WHEN (SELECT data_type FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'id') = 'integer'
      THEN auth.uid()::text = id::text
      ELSE auth.uid() = id
    END
  );
...
-- Safe to enable RLS - everyone can read
-- =============================================================================
...
-- Keep these DISABLED for now until Supabase Auth is configured
-- =============================================================================

-- Uncomment when Supabase Auth is ready:
/*
-- User behavior
ALTER TABLE user_behavior ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view their behavior" ON user_behavior FOR SELECT
  USING (auth.uid()::text = user_id::text);

-- Training analytics
ALTER TABLE training_analytics ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view their analytics" ON training_analytics FOR SELECT
  USING (auth.uid()::text = user_id::text);

-- Performance metrics
ALTER TABLE performance_metrics ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view their metrics" ON performance_metrics FOR SELECT
  USING (auth.uid()::text = user_id::text);

-- Analytics events
ALTER TABLE analytics_events ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view their events" ON analytics_events FOR SELECT
  USING (auth.uid()::text = user_id::text);

-- Implementation steps
ALTER TABLE implementation_steps ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can manage their steps" ON implementation_steps FOR ALL
  USING (auth.uid()::text = user_id::text);

-- Supplement protocols
ALTER TABLE supplement_protocols ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can manage their protocols" ON supplement_protocols FOR ALL
  USING (auth.uid()::text = user_id::text);
...
CREATE POLICY "Users can manage their wearables" ON wearables_data FOR ALL
  USING (auth.uid()::text = user_id::text);
*/
...
-- SUMMARY
-- =============================================================================
-- ✅ Enabled RLS on 59 public reference/research tables
-- ⏸️  Keeping user-specific tables WITHOUT RLS until auth is configured
-- 🔐 This allows public access to reference data while protecting user data later
