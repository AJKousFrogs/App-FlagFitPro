...
async function insertTestData(supabase) {
  ...
  const workouts = SYNTHETIC_DATASET
    .filter(d => d.duration > 0) // Skip rest days
    .map(d => ({
  ...
  for (const [dayStr, expected] of Object.entries(EXPECTED_VALUES)) {
    ...
    if (lmData?.acute_load !== undefined && expected.acuteLoad !== null) {
      try {
        ...
        console.log(`     ✅ Acute: ${lmData.acute_load} (expected: ${expected.acuteLoad})`);
      } catch (e) {
    ...
    if (lmData?.chronic_load !== undefined && expected.chronicLoad !== null) {
      try {
        ...
        console.log(`     ✅ Chronic: ${lmData.chronic_load} (expected: ${expected.chronicLoad})`);
      } catch (e) {
...
async function testRiskLevelClassification(supabase) {
  ...
  const testCases = [
    ...
    { acwr: 0.80, expected: 'Optimal' },  // Lower bound of optimal
    { acwr: 1.00, expected: 'Optimal' },  // Middle of optimal
    { acwr: 1.30, expected: 'Optimal' },  // Upper bound of optimal
    { acwr: 1.31, expected: 'Moderate' }, // Just over optimal
    { acwr: 1.50, expected: 'Moderate' }, // Upper moderate
    { acwr: 1.51, expected: 'High' },     // Danger zone
    { acwr: 2.00, expected: 'High' },     // Deep danger zone
  ];
...
async function testMinimumChronicLoadFloor(supabase) {
  ...
  try {
    ...
    const { data: chronicData, error: chronicError } = await supabase.rpc('calculate_chronic_load', {
    ...
    if (actualChronic >= MIN_CHRONIC_FLOOR) {
      console.log(`   ✅ Chronic load floor enforced: ${actualChronic} AU >= ${MIN_CHRONIC_FLOOR} AU`);
      ...
      console.log(`   ACWR with floor: ${acwrWithFloor.toFixed(2)} (safe - shows under-training)`);
      console.log(`   ACWR without floor: ${acwrWithoutFloor.toFixed(2)} (misleading - looks optimal)`);
      
      return true;
    } else {
      console.log(`   ❌ Chronic load floor NOT enforced: ${actualChronic} AU < ${MIN_CHRONIC_FLOOR} AU`);
      return false;
    }
