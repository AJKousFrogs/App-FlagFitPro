...
describe("player-settings validation hardening", () => {
  ...
  // Regression: partial settings saves must NOT wipe unrelated fields. Every
  // optional column is written only when its source is present in the payload
  // (conditional-spread), so an omitted key is left out of the upsert entirely.
  it("a partial save omits unrelated fields from the upsert (does not wipe them to defaults)", async () => {
    ...
    // must not include season_calendar/available_equipment/birth_date/etc. in the
    // upsert — those keys being absent is what leaves previously-saved values
    // untouched at the DB layer.
    const response = await handler(
      {
        httpMethod: "POST",
        path: "/.netlify/functions/player-settings",
