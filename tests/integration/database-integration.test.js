...
describe("Database Integration Tests", () => {
    ...
    it("should handle user profile updates", async () => {
      ...
      const result = await mockDatabase.query(
        ...
        [
          ...
          JSON.stringify(updatedData.stats),
          1,
        ],
      );
  ...
  describe("Training Session Management", () => {
    it("should create training session with exercises", async () => {
      ...
      mockDatabase.query.mockResolvedValueOnce({
        rows: sessionData.exercises.map((ex, idx) => ({
          id: idx + 1,
          session_id: 101,
          ...ex,
        })),
      ...
      const exercisesResult = await mockDatabase.query(
        ...
        [
          101,
          sessionData.exercises[0].name,
          sessionData.exercises[0].sets,
          sessionData.exercises[0].reps,
          sessionData.exercises[0].notes,
        ],
      );
    ...
    it("should track session progress and analytics", async () => {
      ...
      const result = await mockDatabase.query(
        ...
        [
          ...
          JSON.stringify(progressData.performance_metrics),
          progressData.notes,
        ],
      );
      ...
      expect(
        JSON.parse(result.rows[0].performance_metrics || "{}"),
      ).toMatchObject(progressData.performance_metrics);
    });
  });
    ...
    it("should log daily nutrition intake", async () => {
      ...
      const result = await mockDatabase.query(
        ...
        [
          ...
          nutritionData.date,
          JSON.stringify(nutritionData.meals),
          JSON.stringify(nutritionData.daily_totals),
        ],
      );

      expect(result.rows[0].user_id).toBe(1);
      expect(JSON.parse(result.rows[0].daily_totals || "{}")).toMatchObject(
        nutritionData.daily_totals,
      );
    });
    ...
    it("should generate nutrition recommendations", async () => {
      ...
      const result = await mockDatabase.query(
        ...
        [
          ...
          userStats.height,
          userStats.age,
          userStats.activity_level,
          JSON.stringify(userStats.goals),
        ],
      );
    ...
    it("should store and analyze performance trends", async () => {
      ...
      const recordResult = await mockDatabase.query(
        ...
        [
          ...
          performanceData.session_date,
          JSON.stringify(performanceData.metrics),
          JSON.stringify(performanceData.context),
        ],
      );
      ...
      expect(trendResult.rows[0].trend_analysis.overall_trend).toBe(
        "improving",
      );
    });
  });
  ...
  describe("Team and Coach Management", () => {
    it("should manage team roster and coaching assignments", async () => {
      ...
      const assignmentResult = await mockDatabase.query(
        ...
        [
          playerAssignment.team_id,
          playerAssignment.player_id,
          playerAssignment.position,
          playerAssignment.jersey_number,
          playerAssignment.status,
        ],
      );
  ...
  describe("AI Coach Data Integration", () => {
    it("should store and retrieve AI coaching interactions", async () => {
      ...
      const result = await mockDatabase.query(
        ...
        [
          coachingData.user_id,
          coachingData.interaction_type,
          JSON.stringify(coachingData.context),
          JSON.stringify(coachingData.ai_response),
        ],
      );

      expect(result.rows[0].interaction_type).toBe("training_recommendation");
      expect(JSON.parse(result.rows[0].ai_response || "{}")).toMatchObject(
        coachingData.ai_response,
      );
    });
  });
    ...
    it("should handle complex transactions with rollback", async () => {
      ...
      try {
        await mockDatabase.transaction(async (trx) => {
          await trx.query("INSERT INTO users (email) VALUES ($1)", [
            "test@example.com",
          ]);
          await trx.query("INSERT INTO invalid_table (data) VALUES ($1)", [
            "data",
          ]); // This fails
        });
      } catch (error) {
