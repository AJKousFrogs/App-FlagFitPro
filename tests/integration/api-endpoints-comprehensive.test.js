...
describe("API Endpoints - Comprehensive Integration Tests", () => {
  ...
  describe("Training Session Endpoints", () => {
    describe("POST /training/sessions", () => {
      it("should create new training session", async () => {
        ...
        const response = await fetch(
          `${mockServer.baseURL}/training/sessions`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    describe("GET /training/sessions", () => {
      it("should return user training sessions", async () => {
        ...
        const response = await fetch(
          `${mockServer.baseURL}/training/sessions?page=1&limit=10`,
          {
            headers: { Authorization: `Bearer ${authToken}` },
          },
        );
    ...
    describe("GET /training/sessions/:id", () => {
      it("should return specific training session", async () => {
        ...
        const response = await fetch(
          `${mockServer.baseURL}/training/sessions/${sessionId}`,
          {
            headers: { Authorization: `Bearer ${authToken}` },
          },
        );
      ...
      it("should return 404 for non-existent session", async () => {
        ...
        const response = await fetch(
          `${mockServer.baseURL}/training/sessions/99999`,
          {
            headers: { Authorization: `Bearer ${authToken}` },
          },
        );
  ...
  describe("Nutrition Endpoints", () => {
    ...
    describe("GET /nutrition/analysis", () => {
      it("should return nutrition analysis", async () => {
        ...
        const response = await fetch(
          `${mockServer.baseURL}/nutrition/analysis?period=7_days`,
          {
            headers: { Authorization: `Bearer ${authToken}` },
          },
        );
  ...
  describe("Performance Analytics Endpoints", () => {
    describe("GET /analytics/performance", () => {
      it("should return performance analytics", async () => {
        ...
        const response = await fetch(
          `${mockServer.baseURL}/analytics/performance?period=30_days`,
          {
            headers: { Authorization: `Bearer ${authToken}` },
          },
        );
    ...
    describe("GET /analytics/olympic", () => {
      it("should return Olympic qualification data", async () => {
        ...
        const response = await fetch(
          `${mockServer.baseURL}/analytics/olympic`,
          {
            headers: { Authorization: `Bearer ${authToken}` },
          },
        );
  ...
  describe("AI Coach Endpoints", () => {
    describe("POST /ai/coach/ask", () => {
      it("should respond to coaching questions", async () => {
        ...
        const aiResponse = {
          answer:
            "To improve your 40-yard dash time, focus on explosive starts, proper running form, and plyometric exercises. Based on your current performance data, I recommend...",
          confidence: 0.95,
          sources: [
      ...
      it("should handle complex coaching scenarios", async () => {
        const complexQuery = {
          question:
            "I'm recovering from a minor ankle injury. How should I modify my Olympic preparation training?",
          context: {
        ...
        const aiResponse = {
          answer:
            "Given your Grade 1 ankle sprain recovery and upcoming competition, here's a modified training approach...",
          riskAssessment: {
            reinjuryRisk: "low",
  ...
  describe("API Rate Limiting", () => {
    it("should respect rate limits for expensive operations", async () => {
      ...
      const response = await fetch(
        `${mockServer.baseURL}/analytics/performance`,
        {
          headers: { Authorization: `Bearer ${authToken}` },
        },
      );
