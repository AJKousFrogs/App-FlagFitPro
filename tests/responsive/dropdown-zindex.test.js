...
test.describe('Mobile Dropdown Z-Index Tests', () => {
  for (const device of IPHONE_DEVICES) {
    test.describe(device.name, () => {
      ...
      test('autocomplete dropdown should appear above form content', async ({ page }) => {
        // Navigate to onboarding or any page with autocomplete
        await page.goto('/onboarding');
        await page.waitForLoadState('networkidle');

        // Find team autocomplete field
        const teamInput = page.locator('input#onboarding-team, p-autoComplete input').first();
        ...
        if (await teamInput.count() > 0) {
          ...
          const dropdown = page.locator('.p-autocomplete-overlay, .p-autocomplete-panel');
          ...
          if (await dropdown.count() > 0) {
            ...
            const formContent = page.locator('.onboarding-content, .form-group').first();
            const contentZIndex = await formContent.evaluate((el) => {
              return window.getComputedStyle(el).zIndex;
            });

            // Dropdown z-index should be much higher
            expect(parseInt(dropdownZIndex)).toBeGreaterThan(parseInt(contentZIndex) || 0);
            ...
            const firstOption = dropdown.locator('.p-autocomplete-list-item, .p-autocomplete-item').first();
      ...
      test('dropdown should fit within viewport on mobile', async ({ page }) => {
        await page.goto('/onboarding');
        await page.waitForLoadState('networkidle');
