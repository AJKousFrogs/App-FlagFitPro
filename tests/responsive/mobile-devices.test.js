...
const MOBILE_DEVICES = {
  ...
  samsung: [
    ...
    },
    {
      ...
      isMobile: true,
      hasTouch: true,
      userAgent: 'Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36 Chrome/121.0.0.0 Mobile Safari/537.36',
    },
    {
  ...
  xiaomi: [
    {
      ...
      isMobile: true,
      hasTouch: true,
      userAgent: 'Mozilla/5.0 (Linux; Android 13; MI 11) AppleWebKit/537.36 Chrome/120.0.0.0 Mobile Safari/537.36',
    },
    {
      ...
      isMobile: true,
      hasTouch: true,
      userAgent: 'Mozilla/5.0 (Linux; Android 13; Redmi Note 10) AppleWebKit/537.36 Chrome/120.0.0.0 Mobile Safari/537.36',
    },
    {
      ...
      isMobile: true,
      hasTouch: true,
      userAgent: 'Mozilla/5.0 (Linux; Android 13; Redmi Note 11) AppleWebKit/537.36 Chrome/120.0.0.0 Mobile Safari/537.36',
    },
    {
      name: 'Xiaomi 12',
      viewport: { width: 360, height: 800 },
      ...
      isMobile: true,
      hasTouch: true,
      userAgent: 'Mozilla/5.0 (Linux; Android 13; 2201123G) AppleWebKit/537.36 Chrome/121.0.0.0 Mobile Safari/537.36',
    },
    {
      name: 'Xiaomi 13',
      viewport: { width: 360, height: 800 },
      ...
      isMobile: true,
      hasTouch: true,
      userAgent: 'Mozilla/5.0 (Linux; Android 14; 2211133G) AppleWebKit/537.36 Chrome/122.0.0.0 Mobile Safari/537.36',
    },
    {
      name: 'Xiaomi Poco X3',
      ...
      isMobile: true,
      hasTouch: true,
      userAgent: 'Mozilla/5.0 (Linux; Android 13; POCO X3 NFC) AppleWebKit/537.36 Chrome/120.0.0.0 Mobile Safari/537.36',
    },
  ],
};
    ...
    for (const device of deviceList) {
      test.describe(`${device.name}`, () => {
        ...
        test('should render header correctly', async ({ page }) => {
          await page.goto('/');
          await page.waitForSelector('app-header, .app-header, header', { timeout: 10000 });
          
          const header = page.locator('app-header, .app-header, header').first();
        ...
        test('should have adequate touch targets on buttons', async ({ page }) => {
          await page.goto('/');
          await page.waitForLoadState('networkidle');
          
          // Find all clickable buttons
          const buttons = page.locator('button:visible, a.p-button:visible, .btn:visible');
        ...
        test('should render form inputs correctly', async ({ page }) => {
          ...
          const inputs = page.locator('input[type="text"]:visible, input[type="email"]:visible, input[type="password"]:visible');
        ...
        test('should handle navigation on mobile', async ({ page }) => {
          ...
          const nav = page.locator('nav, .p-menubar, .p-tabmenu, app-navigation');
        ...
        test('should render cards/panels without overflow', async ({ page }) => {
          await page.goto('/');
          await page.waitForLoadState('networkidle');
          
          // Check for cards
          const cards = page.locator('.p-card:visible, .card:visible, [class*="card-"]:visible');
        ...
        test('should have working touch interactions', async ({ page }) => {
          ...
          if (await clickables.count() > 0) {
            ...
            const hasHoverState = await clickables.evaluate((el) => {
              ...
              return styles.cursor === 'pointer' || el.tagName === 'BUTTON' || el.tagName === 'A';
            });
        ...
        test('should handle safe areas on iOS', async ({ page }) => {
          ...
          if (device.name.toLowerCase().includes('iphone')) {
            ...
            const usesSafeArea = await page.evaluate(() => {
              ...
              const safeAreaBottom = computed.getPropertyValue('padding-bottom');
        ...
        test('should render modals/dialogs correctly', async ({ page }) => {
          ...
          const dialogs = page.locator('.p-dialog:visible, .modal:visible, [role="dialog"]:visible');
...
test.describe('Cross-Device Responsive Consistency', () => {
  test('should render consistently across all iPhone models', async ({ page }) => {
    const screenshots = [];
    
    for (const device of MOBILE_DEVICES.iphone.slice(0, 3)) { // Test first 3 iPhones
  ...
  test('should render consistently across Samsung devices', async ({ page }) => {
    for (const device of MOBILE_DEVICES.samsung.slice(0, 3)) { // Test first 3 Samsung
      await page.setViewportSize(device.viewport);
      await page.goto('/');
      await page.waitForLoadState('networkidle');
  ...
  test('should render consistently across Xiaomi devices', async ({ page }) => {
    for (const device of MOBILE_DEVICES.xiaomi.slice(0, 3)) { // Test first 3 Xiaomi
      await page.setViewportSize(device.viewport);
      await page.goto('/');
      await page.waitForLoadState('networkidle');
