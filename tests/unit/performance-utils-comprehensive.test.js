...
import { setupTestEnvironment, createLoadTestScenario } from '../test-helpers.js'
...
describe('Performance Utils - Comprehensive Tests', () => {
  ...
  describe('Execution Time Measurement', () => {
    it('should measure function execution time accurately', async () => {
      ...
      performanceUtils.measureExecutionTime.mockImplementation(async (fn, label) => {
      ...
      const measurement = await performanceUtils.measureExecutionTime(testFunction, 'test-function')
    ...
    it('should handle synchronous function measurement', () => {
      ...
      const measurement = performanceUtils.measureExecutionTime(syncFunction, 'heavy-calculation')
    ...
    it('should track multiple nested measurements', async () => {
      const nestedFunction = async () => {
        ...
        const step1 = await new Promise(resolve => setTimeout(() => resolve('step1'), 50))
        const step2 = await new Promise(resolve => setTimeout(() => resolve('step2'), 30))
        return { step1, step2 }
      }
      ...
      performanceUtils.measureExecutionTime.mockImplementation(async (fn, label) => {
        ...
        const wrappedFn = async () => {
          measurements.push({ label: 'nested-start', time: performance.now() })
      ...
      const result = await performanceUtils.measureExecutionTime(nestedFunction, 'nested-operation')
  ...
  describe('Page Load Performance', () => {
    ...
    it('should identify performance bottlenecks', () => {
      const mockBottlenecks = [
        { type: 'slow_api_call', endpoint: '/api/training/analysis', duration: 5000 },
      ...
      expect(analysis.recommendations).toContain('Optimize API response times for /api/training/analysis')
    })
  })
  ...
  describe('User Interaction Performance', () => {
    it('should track user interaction response times', () => {
      ...
      performanceUtils.trackUserInteraction.mockImplementation((type, element) => {
        const interaction = interactions.find(i => i.type === type && i.element === element)
        return {
          ...interaction,
          timestamp: Date.now(),
      ...
      const clickResult = performanceUtils.trackUserInteraction('click', 'save-training-btn')
      const scrollResult = performanceUtils.trackUserInteraction('scroll', 'training-list')
    ...
    it('should measure input responsiveness', () => {
      ...
      performanceUtils.measureInputResponsiveness = vi.fn().mockImplementation((element) => {
        const event = inputEvents.find(e => e.element === element)
        return {
          ...event,
          totalDelay: event.inputDelay + event.renderDelay,
          responsiveness: event.inputDelay + event.renderDelay <= 50 ? 'excellent' : 'needs_improvement'
        }
      })
      
      const searchResponse = performanceUtils.measureInputResponsiveness('search-box')
      const dropdownResponse = performanceUtils.measureInputResponsiveness('exercise-dropdown')
  ...
  describe('API Call Performance', () => {
    it('should track API call performance metrics', async () => {
      const apiCalls = [
        { endpoint: '/api/training/sessions', method: 'GET', duration: 250, status: 200 },
        { endpoint: '/api/nutrition/log', method: 'POST', duration: 180, status: 201 },
        { endpoint: '/api/analytics/performance', method: 'GET', duration: 1200, status: 200 }
      ]
      
      performanceUtils.trackApiCall.mockImplementation((endpoint, method) => {
        const call = apiCalls.find(c => c.endpoint === endpoint && c.method === method)
        return {
          ...call,
          timestamp: Date.now(),
      ...
      const fastCall = performanceUtils.trackApiCall('/api/nutrition/log', 'POST')
      const slowCall = performanceUtils.trackApiCall('/api/analytics/performance', 'GET')
    ...
    it('should identify slow API endpoints', () => {
      const endpointMetrics = [
        { endpoint: '/api/training/analysis', averageTime: 3000, callCount: 45 },
        { endpoint: '/api/user/profile', averageTime: 150, callCount: 120 },
        { endpoint: '/api/olympic/rankings', averageTime: 800, callCount: 30 }
      ]
      
      performanceUtils.identifySlowEndpoints = vi.fn().mockImplementation(() => {
  ...
  describe('Memory Usage Tracking', () => {
    it('should monitor memory usage patterns', () => {
      ...
      performanceUtils.measureMemoryUsage.mockImplementation(() => {
        ...
        return {
          ...
          status: memory.usedJSHeapSize / memory.totalJSHeapSize > 0.8 ? 'high' : 'normal'
        }
      })
    ...
    it('should detect memory leaks', () => {
      ...
      performanceUtils.detectMemoryLeaks = vi.fn().mockImplementation(snapshots => {
        const growthRate = snapshots.slice(-3).reduce((acc, snapshot, index) => {
          if (index === 0) return acc
          return acc + (snapshot.used - snapshots[snapshots.length - 3 + index - 1].used)
        }, 0) / 2
        
        return {
          growthRate,
          trend: growthRate > 5 ? 'increasing_rapidly' : 'stable',
          recommendation: growthRate > 5 ? 'investigate_potential_leak' : 'normal_usage',
  ...
  describe('Render Performance', () => {
    it('should track component render times', () => {
      ...
      performanceUtils.trackRenderTime.mockImplementation((component) => {
        ...
        return {
          ...
          efficiency: render.reRenderCount <= 5 ? 'efficient' : 'excessive_rerenders'
        }
      })
      
      const dashboardRender = performanceUtils.trackRenderTime('TrainingDashboard')
    ...
    it('should identify render bottlenecks', () => {
      const renderBottlenecks = [
        { component: 'AnalyticsChart', issue: 'expensive_calculations', impact: 'high' },
        { component: 'UsersList', issue: 'large_list_rendering', impact: 'medium' },
        { component: 'VideoPlayer', issue: 'heavy_dom_manipulation', impact: 'high' }
      ]
      
      performanceUtils.identifyRenderBottlenecks = vi.fn().mockImplementation(() => {
        return {
          bottlenecks: renderBottlenecks,
          recommendations: {
            AnalyticsChart: ['implement memoization', 'use web workers for calculations'],
            UsersList: ['implement virtualization', 'add pagination'],
            VideoPlayer: ['optimize DOM updates', 'use requestAnimationFrame']
          },
  ...
  describe('Optimization Utilities', () => {
      ...
      performanceUtils.throttle.mockImplementation((func, limit) => {
        return (...args) => {
          if (canExecute) {
            func(...args)
            canExecute = false
            setTimeout(() => { canExecute = true }, limit)
          }
        }
      })
    ...
    it('should implement lazy loading', () => {
      ...
      performanceUtils.lazy.mockImplementation((selector, options) => {
      ...
      const lazyLoader = performanceUtils.lazy('.lazy-image', { threshold: 0.1 })
  ...
  describe('Performance Reporting', () => {
    ...
    it('should track performance over time', () => {
      ...
      performanceUtils.trackPerformanceHistory = vi.fn().mockImplementation(() => {
        return {
          data: historicalData,
          trend: 'improving',
