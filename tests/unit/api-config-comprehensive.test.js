...
import { createMockApiResponse, createMockUser, setupTestEnvironment } from '../test-helpers.js'
...
describe('API Configuration - Comprehensive Tests', () => {
  ...
  describe('Authentication Token Management', () => {
    ...
    it('should include auth header in authenticated requests', async () => {
      ...
      apiConfig.makeRequest = vi.fn().mockImplementation(async (url, method, data, options) => {
  ...
  describe('Request Handling', () => {
    ...
    it('should handle POST requests with data', async () => {
      ...
      const result = await apiConfig.makeRequest('/test-endpoint', 'POST', postData)
    ...
    it('should handle PUT requests for updates', async () => {
      ...
      const result = await apiConfig.makeRequest('/test-endpoint/1', 'PUT', updateData)
      
      expect(result).toEqual(updateData)
    })

    it('should handle DELETE requests', async () => {
      const mockResponse = await createMockApiResponse({ success: true, deleted: true })
      
      global.fetch.mockResolvedValue(mockResponse)
      apiConfig.makeRequest = vi.fn().mockResolvedValue({ success: true, deleted: true })
  ...
  describe('Error Handling', () => {
    it('should handle 401 unauthorized errors', async () => {
      ...
      apiConfig.makeRequest = vi.fn().mockRejectedValue(new Error('Unauthorized'))

      await expect(apiConfig.makeRequest('/protected-endpoint', 'GET'))
    ...
    it('should handle 403 forbidden errors', async () => {
      ...
      await expect(apiConfig.makeRequest('/admin-endpoint', 'GET'))
    ...
    it('should handle 404 not found errors', async () => {
      ...
      await expect(apiConfig.makeRequest('/nonexistent-endpoint', 'GET'))
    ...
    it('should handle 500 server errors', async () => {
      ...
      apiConfig.makeRequest = vi.fn().mockRejectedValue(new Error('Internal Server Error'))

      await expect(apiConfig.makeRequest('/test-endpoint', 'GET'))
        .rejects.toThrow('Internal Server Error')
    })

    it('should handle network errors', async () => {
      global.fetch.mockRejectedValue(new Error('Network error'))
      apiConfig.makeRequest = vi.fn().mockRejectedValue(new Error('Network error'))
  ...
  describe('Retry Logic', () => {
    ...
    it('should not retry on client errors (4xx)', async () => {
      ...
      apiConfig.makeRequest = vi.fn().mockRejectedValue(new Error('Bad Request'))
  ...
  describe('Request Timeout', () => {
    it('should timeout long-running requests', async () => {
      const timeoutPromise = new Promise(resolve => 
        setTimeout(() => resolve(createMockApiResponse({ data: 'delayed' })), 10000)
      )
      
      global.fetch.mockReturnValue(timeoutPromise)
      apiConfig.makeRequest = vi.fn().mockRejectedValue(new Error('Request timeout'))

      await expect(apiConfig.makeRequest('/slow-endpoint', 'GET'))
  ...
  describe('Request Caching', () => {
    it('should cache GET requests when enabled', async () => {
      const cacheKey = 'GET:/api/cached-endpoint'
      const cachedData = { data: 'cached-response', timestamp: Date.now() }
      
      // Mock cache implementation
      const cache = new Map()
      apiConfig.getFromCache = vi.fn().mockImplementation(key => cache.get(key))
      apiConfig.setCache = vi.fn().mockImplementation((key, data) => cache.set(key, data))
      ...
      apiConfig.makeRequest = vi.fn()
        ...
        .mockImplementationOnce(async (url, method) => {
          return apiConfig.getFromCache(`${method}:${url}`) || { data: 'fallback' }
        })

      const firstResult = await apiConfig.makeRequest('/cached-endpoint', 'GET')
      const secondResult = await apiConfig.makeRequest('/cached-endpoint', 'GET')
  ...
  describe('Request Interceptors', () => {
    it('should apply request interceptors', async () => {
      ...
      apiConfig.addRequestInterceptor = vi.fn().mockImplementation(interceptor => {
    ...
    it('should apply response interceptors', async () => {
      ...
      apiConfig.addResponseInterceptor = vi.fn().mockImplementation(interceptor => {
  ...
  describe('Content Type Handling', () => {
    ...
    it('should handle form data uploads', async () => {
      ...
      global.fetch.mockResolvedValue(mockResponse)
      apiConfig.makeRequest = vi.fn().mockResolvedValue({ uploaded: true, fileId: 'file-123' })
  ...
  describe('Rate Limiting', () => {
    it('should handle rate limit responses', async () => {
      ...
      apiConfig.makeRequest = vi.fn().mockRejectedValue(new Error('Rate limit exceeded'))

      await expect(apiConfig.makeRequest('/api-endpoint', 'GET'))
