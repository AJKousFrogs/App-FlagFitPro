...
async function runCommand(command, args, description) {
  return new Promise((resolve, reject) => {
    ...
    process.on('error', (error) => {
      console.log(chalk.red(`❌ Error running ${description}: ${error.message}\n`))
      reject(error)
    })
  })
}
...
async function runTests() {
  try {
    ...
    switch (testType) {
      ...
      case 'integration':
        await runCommand('npx', ['vitest', 'run', 'tests/integration'], 'Integration Tests')
        break

      case 'e2e':
        console.log(chalk.blue('Installing Playwright browsers...'))
        await runCommand('npx', ['playwright', 'install'], 'Playwright Browser Installation')
        await runCommand('npx', ['playwright', 'test'], 'End-to-End Tests')
        break

      case 'coverage':
        await runCommand('npx', ['vitest', 'run', '--coverage'], 'Tests with Coverage')
        break
      ...
      default:
        ...
        console.log(chalk.yellow('Available options: unit, integration, e2e, coverage, watch, all'))
        process.exit(1)
    }
