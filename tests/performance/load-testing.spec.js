...
import { createLoadTestScenario, setupTestEnvironment } from '../test-helpers.js'
...
class PerformanceTestRunner {
  ...
  async simulateUser(scenario, userId) {
    ...
    try {
      ...
      const delay = (userId / scenario.virtualUsers) * scenario.rampUpTime * 1000
  ...
  analyzeResults(results, totalTime, scenario) {
    ...
    const allErrors = successful.flatMap(user => user.errors).concat(failed.map(f => f.reason))
    
    const avgResponseTime = allRequests.reduce((sum, req) => sum + req.responseTime, 0) / allRequests.length
    const maxResponseTime = Math.max(...allRequests.map(req => req.responseTime))
    const minResponseTime = Math.min(...allRequests.map(req => req.responseTime))
    
    const totalRequests = allRequests.length
    const errorRate = (allErrors.length / (totalRequests + allErrors.length)) * 100
    const throughput = totalRequests / (totalTime / 1000) // requests per second
    
    // Calculate percentiles
    const sortedResponseTimes = allRequests.map(req => req.responseTime).sort((a, b) => a - b)
    ...
    return {
      ...
      acceptance: {
        passed: avgResponseTime <= scenario.acceptanceCriteria.averageResponseTime,
      ...
      recommendations: this.generateRecommendations(avgResponseTime, errorRate, throughput, scenario)
    }
  }
  ...
  analyzeEndpointPerformance(requests) {
    ...
    Object.keys(endpointStats).forEach(endpoint => {
      ...
      stats.performance = stats.averageTime <= 300 ? 'good' : stats.averageTime <= 1000 ? 'acceptable' : 'poor'
    })
...
describe('Performance and Load Testing', () => {
  ...
  describe('Endpoint-Specific Performance Testing', () => {
    it('should test authentication endpoint performance', async () => {
      ...
      expect(results.endpointAnalysis['POST /api/auth/login'].performance).toBe('good')
      expect(results.acceptance.passed).toBe(true)
    }, 90000)
    ...
    it('should test analytics endpoint under load', async () => {
      ...
      expect(results.endpointAnalysis['GET /api/analytics/performance'].averageTime).toBeLessThan(1500)
    }, 120000)
    ...
    it('should test AI coach endpoint performance', async () => {
      ...
      expect(results.endpointAnalysis['POST /api/ai/coach/ask'].averageTime).toBeLessThan(2000)
      expect(results.acceptance.errorRateAcceptable).toBe(true)
    }, 120000)
  })
  ...
  describe('Database Performance Testing', () => {
    it('should test database query performance under load', async () => {
      ...
      expect(results.endpointAnalysis['GET /api/training/sessions'].averageTime).toBeLessThan(300)
      expect(results.endpointAnalysis['POST /api/training/session'].averageTime).toBeLessThan(500)
      
      // Complex analytics queries can be slower but should not time out
      expect(results.endpointAnalysis['GET /api/analytics/performance'].averageTime).toBeLessThan(2000)
    }, 120000)
  })
  ...
  describe('CDN and Caching Performance', () => {
    it('should test cached content delivery performance', async () => {
      ...
      expect(results.endpointAnalysis['GET /api/dashboard'].averageTime).toBeLessThan(150)
  ...
  describe('Performance Regression Testing', () => {
    it('should detect performance regressions', async () => {
      ...
      const baselineResults = await performanceRunner.runLoadTest(baselineScenario)
      ...
      const regressionResults = await performanceRunner.runLoadTest(baselineScenario)
      
      // Detect regression
      const responseTimeIncrease = regressionResults.responseTime.average / baselineResults.responseTime.average
      const throughputDecrease = baselineResults.summary.throughput / regressionResults.summary.throughput
      ...
      const hasRegression = responseTimeIncrease > 1.2 || throughputDecrease > 1.2
