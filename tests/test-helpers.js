...
export const createLoadTestScenario = (userCount = 100, duration = 30) => ({
  ...
  scenarios: [
    ...
    },
    {
      name: 'analytics_view',
      weight: 20,
      requests: ['GET /api/analytics/performance', 'GET /api/analytics/olympic']
    }
  ],
  acceptanceCriteria: {
  ...
  for (const [key, rules] of Object.entries(schema)) {
    ...
    if (key in data) {
      if (rules.type && typeof data[key] !== rules.type) {
        errors.push(`Invalid type for ${key}: expected ${rules.type}, got ${typeof data[key]}`)
      }
      
      if (rules.min && data[key] < rules.min) {
        errors.push(`Value for ${key} below minimum: ${data[key]} < ${rules.min}`)
      }
      
      if (rules.max && data[key] > rules.max) {
        errors.push(`Value for ${key} above maximum: ${data[key]} > ${rules.max}`)
      }
    }
  }
