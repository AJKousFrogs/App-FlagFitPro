...
test.describe('Dashboard Navigation and Core Features', () => {
  ...
  test('should display dashboard with all key components', async ({ page }) => {
    ...
    await expect(page.locator('#user-profile-section')).toContainText('athlete@example.com')
  })
  ...
  test('should handle quick actions from dashboard', async ({ page }) => {
    ...
    await expect(page.locator('.success-message')).toContainText('Nutrition logged')

    // Test quick coach chat
    await page.click('#quick-coach-chat')
    await expect(page.locator('#ai-coach-widget')).toBeVisible()
    await page.fill('#quick-question', 'How can I improve my 40-yard dash time?')
  ...
  test('should display upcoming events and reminders', async ({ page }) => {
    ...
    await expect(page.locator('#calendar-week-display')).not.toContainText('This Week')
  ...
  test('should show recent activity feed', async ({ page }) => {
    ...
    await expect(page.locator('.activity-item[data-type="training"]')).toBeVisible()

    await page.selectOption('#activity-filter', 'nutrition')
    await expect(page.locator('.activity-item[data-type="nutrition"]')).toBeVisible()
  })
  ...
  test('should display personalized recommendations', async ({ page }) => {
    ...
    await expect(page.locator('.success-message')).toContainText('Recommendation applied')

    // Dismiss recommendation
    await page.click('.recommendation-item:nth-child(2) .dismiss-btn')
    const dismissedRecommendations = await page.locator('.recommendation-item').count()
  ...
  test('should handle dashboard customization', async ({ page }) => {
    ...
    await expect(page.locator('.success-message')).toContainText('Dashboard customized')
