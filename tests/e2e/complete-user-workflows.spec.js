...
test.describe('Complete User Workflows - End-to-End Tests', () => {
  ...
  test.describe('User Authentication Workflow', () => {
    test('complete registration and login flow', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="welcome-message"]')).toContainText('Welcome, New Athlete')
    })
    ...
    test('login with existing credentials', async ({ page }) => {
      ...
      await expect(page).toHaveURL(/.*dashboard/)
      await expect(page.locator('[data-testid="user-name"]')).toContainText('Test Athlete')
    })
  ...
  test.describe('Training Session Management Workflow', () => {
    test('create and save training session', async ({ page }) => {
      ...
      await page.selectOption('[data-testid="session-type"]', 'flag_football_drill')
      ...
      // Add session notes
      await page.fill('[data-testid="session-notes"]', 'Pre-competition preparation session')
      
      // Save session
      await page.click('[data-testid="save-session-btn"]')
      
      // Verify success message
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Training session saved successfully')
      
      // Verify session appears in history
      await expect(page.locator('[data-testid="session-list"]')).toContainText('Sprint intervals')
      await expect(page.locator('[data-testid="session-list"]')).toContainText('Route running')
    })
    ...
    test('edit existing training session', async ({ page }) => {
      ...
      await page.fill('[data-testid="session-notes"]', 'Updated session notes - focused on speed')
      
      // Save changes
      await page.click('[data-testid="save-changes-btn"]')
      
      // Verify update
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Session updated successfully')
      await expect(page.locator('[data-testid="session-duration"]')).toContainText('75')
    })
    ...
    test('view training session analytics', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="intensity-trend"]')).toBeVisible()
    })
  })
  ...
  test.describe('Nutrition Tracking Workflow', () => {
    test('log daily nutrition intake', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Meal logged successfully')
      await expect(page.locator('[data-testid="daily-calories"]')).toContainText('500')
      await expect(page.locator('[data-testid="protein-total"]')).toContainText('32')
    })
    ...
    test('view nutrition analysis and recommendations', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="micronutrient-status"]')).toBeVisible()
      
      // Check recommendations
      await expect(page.locator('[data-testid="nutrition-recommendations"]')).toBeVisible()
      await expect(page.locator('[data-testid="olympic-readiness"]')).toContainText('excellent')
    })
  })
  ...
  test.describe('Performance Analytics Workflow', () => {
    test('view comprehensive performance dashboard', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="speed-metric"]')).toContainText('12.8')
      await expect(page.locator('[data-testid="agility-metric"]')).toContainText('8.7')
      await expect(page.locator('[data-testid="endurance-metric"]')).toContainText('58.5')
      
      // Check improvement indicators
      await expect(page.locator('[data-testid="speed-improvement"]')).toContainText('4.9')
      await expect(page.locator('[data-testid="agility-improvement"]')).toContainText('3.6')
      
      // Verify Olympic qualification tracking
      await expect(page.locator('[data-testid="olympic-score"]')).toContainText('78.2')
      await expect(page.locator('[data-testid="required-score"]')).toContainText('85.0')
      await expect(page.locator('[data-testid="qualification-probability"]')).toContainText('76')
    })
    ...
    test('interact with performance charts', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="chart-title"]')).toContainText('90 days')
  ...
  test.describe('AI Coach Interaction Workflow', () => {
    test('ask coaching question and receive guidance', async ({ page }) => {
      ...
      // Ask a question
      await page.fill('[data-testid="coach-question-input"]', 'How can I improve my 40-yard dash time for Olympic qualification?')
      await page.click('[data-testid="ask-coach-btn"]')
      
      // Wait for response
      await expect(page.locator('[data-testid="coach-response"]')).toBeVisible()
      await expect(page.locator('[data-testid="coach-response"]')).toContainText('technique refinement')
      
      // Verify recommendations appear
      await expect(page.locator('[data-testid="coach-recommendations"]')).toBeVisible()
      await expect(page.locator('[data-testid="exercise-recommendation"]')).toContainText('Explosive start drills')
      
      // Check confidence score
      await expect(page.locator('[data-testid="confidence-score"]')).toContainText('92')
    })
    ...
    test('follow up on coaching recommendations', async ({ page }) => {
      ...
      await page.fill('[data-testid="coach-question-input"]', 'What exercises should I do for speed?')
      await page.click('[data-testid="ask-coach-btn"]')
      ...
      await expect(page.locator('[data-testid="coach-response"]')).toContainText('specific drill recommendations')
      
      // Create training plan from recommendations
      await page.click('[data-testid="create-plan-btn"]')
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Training plan created')
    })
  ...
  test.describe('Olympic Qualification Tracking Workflow', () => {
    test('monitor Olympic qualification progress', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="qualification-status"]')).toContainText('in_progress')
      await expect(page.locator('[data-testid="current-ranking"]')).toContainText('45')
      
      // Check upcoming events
      await expect(page.locator('[data-testid="upcoming-events"]')).toBeVisible()
      await expect(page.locator('[data-testid="event-item"]')).toContainText('European Championship Qualifier')
      
      // View qualification timeline
      await page.click('[data-testid="timeline-tab"]')
      await expect(page.locator('[data-testid="qualification-timeline"]')).toBeVisible()
    })
    ...
    test('set Olympic preparation goals', async ({ page }) => {
      ...
      await page.fill('[data-testid="goal-title"]', 'Improve sprint time by 0.3 seconds')
      ...
      // Verify goal appears
      await expect(page.locator('[data-testid="goals-list"]')).toContainText('Improve sprint time')
      await expect(page.locator('[data-testid="goal-status"]')).toContainText('active')
    })
  })
  ...
  test.describe('Mobile Responsiveness Workflow', () => {
    ...
    test('mobile training session creation', async ({ page }) => {
      ...
      await page.selectOption('[data-testid="mobile-session-type"]', 'flag_football_drill')
  ...
  test.describe('Offline Functionality Workflow', () => {
    test('work offline and sync when back online', async ({ page, context }) => {
      await page.goto('/dashboard')
      ...
      // Fill offline form
      await page.fill('[data-testid="exercise-name-0"]', 'Offline sprint training')
      ...
      await expect(page.locator('[data-testid="sync-pending"]')).toContainText('1 item pending sync')
      ...
      // Verify sync success
      await expect(page.locator('[data-testid="sync-success"]')).toContainText('Data synced successfully')
  ...
  test.describe('Error Handling Workflow', () => {
    test('handle API errors gracefully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="error-message"]')).toContainText('Unable to load training sessions')
  ...
  test.describe('Accessibility Workflow', () => {
    ...
    test('verify screen reader compatibility', async ({ page }) => {
      ...
      await expect(page.locator('[aria-label="Performance metrics"]')).toBeVisible()
