...
test.describe("Notifications Center", () => {
  ...
  test("should show loading state when opening panel", async ({ page }) => {
    ...
    expect(hasLoading || hasLoadingText || await page.locator(".notification-item").count() > 0).toBeTruthy();
  });
  ...
  test("should show empty state when no notifications", async ({ page }) => {
    ...
    await expect(page.locator(".notification-empty-title")).toContainText("No notifications yet");
    await expect(page.locator(".notification-empty-text")).toBeVisible();
  });

  test("should show correct unread count in badge after initial load", async ({ page }) => {
  ...
  test("should decrement badge when marking single notification as read", async ({ page }) => {
    let unreadCount = 3;
    ...
    const initialBadge = await page.locator("#notification-badge").textContent();
