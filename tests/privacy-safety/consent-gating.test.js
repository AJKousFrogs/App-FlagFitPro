...
describe.skipIf(!canRunTests)('Consent Gating', () => {
  ...
  describe('Coach without consent', () => {
    ...
    it('should set consent_blocked=true when no consent exists', async () => {
      ...
      const { data, error } = await supabaseAdmin.rpc('check_performance_sharing', {
  ...
  describe('Coach with consent', () => {
    ...
    it('should respect health_sharing_enabled separately from performance', async () => {
      ...
      const { data: perfResult } = await supabaseAdmin.rpc('check_performance_sharing', {
        p_player_id: TEST_PLAYER_ID,
        p_team_id: TEST_TEAM_ID
      });

      const { data: healthResult } = await supabaseAdmin.rpc('check_health_sharing', {
        p_player_id: TEST_PLAYER_ID,
        p_team_id: TEST_TEAM_ID
      });
  ...
  describe('Consent helper functions', () => {
    it('check_ai_processing_enabled should return false by default (privacy-first)', async () => {
      ...
      const { data, error } = await supabaseAdmin.rpc('check_ai_processing_enabled', {
    ...
    it('check_metric_category_allowed should validate specific categories', async () => {
      ...
      const { data: allowedResult } = await supabaseAdmin.rpc('check_metric_category_allowed', {
        p_player_id: TEST_PLAYER_ID,
        p_team_id: TEST_TEAM_ID,
        p_category: 'performance'
      });

      const { data: notAllowedResult } = await supabaseAdmin.rpc('check_metric_category_allowed', {
        p_player_id: TEST_PLAYER_ID,
        p_team_id: TEST_TEAM_ID,
        p_category: 'health'
      });
  ...
  describe('Default privacy settings (privacy-first)', () => {
    it('should default to disabled sharing when no settings exist', async () => {
      ...
      await supabaseAdmin.from('privacy_settings').delete().eq('user_id', newUserId);
      await supabaseAdmin.from('team_sharing_settings').delete().eq('user_id', newUserId);

      // Check defaults
      const { data: perfSharing } = await supabaseAdmin.rpc('check_performance_sharing', {
        p_player_id: newUserId,
        p_team_id: TEST_TEAM_ID
      });

      const { data: healthSharing } = await supabaseAdmin.rpc('check_health_sharing', {
        p_player_id: newUserId,
        p_team_id: TEST_TEAM_ID
      });

      const { data: aiProcessing } = await supabaseAdmin.rpc('check_ai_processing_enabled', {
        p_user_id: newUserId
      });
