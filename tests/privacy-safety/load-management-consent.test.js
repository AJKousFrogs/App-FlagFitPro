...
describe.skipIf(!canRunDbTests)('Load Management Consent Compliance - Integration Tests', () => {
  let supabaseAdmin;
  ...
  describe('ConsentDataReader for coach context', () => {
    it('should return data for players with performance_sharing_enabled', async () => {
      const { ConsentDataReader, AccessContext } = await import(
        '../../netlify/functions/utils/consent-data-reader.cjs'
      );

      const reader = new ConsentDataReader(supabaseAdmin, { enableAuditLogging: false });
      ...
      expect(result.consentInfo.blockedPlayerIds).not.toContain(TEST_PLAYER_WITH_CONSENT);
    });
    ...
    it('should block data for players without consent', async () => {
      ...
      expect(result.consentInfo.blockedPlayerIds).toContain(TEST_PLAYER_NO_CONSENT);
    });
  });
  ...
  describe('INSUFFICIENT_DATA returns null metrics + warnings', () => {
    it('should return null ACWR for player with < 28 days of data', async () => {
      ...
      const { evaluateDataState } = await import(
        '../../netlify/functions/utils/data-state.cjs'
      );
    ...
    it('should include warning message for insufficient data', async () => {
      const { createDataResponse } = await import(
        '../../netlify/functions/utils/data-state.cjs'
      );
  ...
  describe('Response structure includes DataState', () => {
    it('should include dataState in ConsentDataReader response', async () => {
      ...
      expect(['NO_DATA', 'INSUFFICIENT_DATA', 'DEMO_DATA', 'REAL_DATA']).toContain(result.dataState);
    });
    ...
    it('should mark isReliable as false for insufficient data', async () => {
      const { wrapWithDataState } = await import(
        '../../netlify/functions/utils/data-state.cjs'
      );
