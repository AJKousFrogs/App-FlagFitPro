...
describe.skipIf(!canRunTests)('Deletion Lifecycle', () => {
  ...
  describe('Deletion initiation', () => {
    it('initiate_account_deletion should create pending request', async () => {
      const { data: requestId, error } = await supabaseAdmin.rpc('initiate_account_deletion', {
      ...
      const now = new Date();
      const daysDiff = Math.round((scheduledDate - now) / (1000 * 60 * 60 * 24));
  ...
  describe('Deletion cancellation', () => {
    it('cancel_account_deletion should restore access', async () => {
      ...
      const { data: requestId } = await supabaseAdmin.rpc('initiate_account_deletion', {
        p_user_id: TEST_USER_FOR_CANCEL,
        p_reason: 'Test cancellation'
      });

      // Now cancel it
      const { data: success, error } = await supabaseAdmin.rpc('cancel_account_deletion', {
    ...
    it('should not allow cancellation of completed deletions', async () => {
      ...
      const { data: success } = await supabaseAdmin.rpc('cancel_account_deletion', {
  ...
  describe('Deletion queue processing', () => {
    it('get_deletions_ready_for_processing returns only due deletions', async () => {
      ...
      const { data: readyDeletions, error } = await supabaseAdmin.rpc('get_deletions_ready_for_processing');

      expect(error).toBeNull();
      
      // Should include our overdue deletion
      const found = readyDeletions.find(d => d.user_id === 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee');
  ...
  describe('Emergency medical records retention', () => {
    it('should create emergency record with 7-year retention', async () => {
      const { data: recordId, error } = await supabaseAdmin.rpc('create_emergency_medical_record', {
    ...
    it('cleanup_expired_emergency_records should NOT delete records before 7 years', async () => {
      ...
      // Run cleanup
      const { data: deletedCount } = await supabaseAdmin.rpc('cleanup_expired_emergency_records');
    ...
    it('cleanup_expired_emergency_records SHOULD delete records past 7 years', async () => {
      ...
      // Run cleanup
      const { data: deletedCount, error } = await supabaseAdmin.rpc('cleanup_expired_emergency_records');
  ...
  describe('get_deletion_status helper', () => {
    it('should return correct status for pending deletion', async () => {
      const { data: status, error } = await supabaseAdmin.rpc('get_deletion_status', {
        p_user_id: TEST_USER_FOR_DELETION
      });
...
async function cleanupTestData(supabase) {
  ...
  for (const userId of testIds) {
    await supabase.from('emergency_medical_records').delete().eq('user_id', userId);
