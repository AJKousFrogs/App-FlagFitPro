...
| ~~Custom supplement list~~ ✅ **RESOLVED 2026-06-02** | supplements.js (`getUserSupplements`) | `user_supplements` table **created** (id/user_id/name/dosage/timing/category/active, RLS self-scoped, UNIQUE (user_id, name)). Read path now returns the athlete's real stack instead of a swallowed `[]`. Write path **built**: `POST /api/supplements/stack` upserts a supplement (set `active:false` to retire). Verified live. | routed (supplements.js) |
