...
try {
  ...
  // Test connection
  pool.on('connect', () => {
    console.log('✅ Algorithm database connected successfully');
  });
  
  pool.on('error', (err) => {
    console.error('❌ Algorithm database connection error:', err);
  });
  
} catch (error) {
  console.error('❌ Failed to create algorithm database pool:', error);
  pool = null;
}
...
async function safeQuery(query, params = []) {
  ...
  try {
    ...
    console.error('Algorithm database query error:', error);
