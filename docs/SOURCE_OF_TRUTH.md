...
| Travel legs + wellness suggestion (V2.1) | BUILT, NOT DEPLOYED | `event-travel.js`, `event-travel.service.ts`, `wellness/` → `event_travel` | Athlete declares a leg via API (no in-app entry form yet); wellness check-in offers (never silently applies) a travel-hours suggestion from a same-day leg. Migration `20260703030000` not applied to live DB. See `docs/v2/V2.1-plan-travel.md`. |
| Game-day caffeine timing (V2.1) | BUILT, NOT DEPLOYED | `supplements.component.ts` (reads `event_games`, `nextEvent`) | Dose/timing suggestion + sleep-cutoff top-up guard on competition-phase days. Depends on the same undeployed `event_games` migration for per-game precision; degrades to day-level event timing without it. |
