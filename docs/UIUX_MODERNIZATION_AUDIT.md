...
2. **Tap-to-inspect on charts** (§3.2) — glanceable is right for the hero; but when a coach *does* lean in, the chart should answer "what was that dip." Look-only charts stop one question short.
...
5. **Coach vs athlete mode:** the split is real (separate shells, guards, routes) but the coach lane is information-*poorer* than the athlete lane (§1-1/1-2). Fix by raising coach density, not by restyling.
...
- **Upgrade path:** keep the viz-dependency count at zero so the eventual Angular 22 bump only negotiates Storybook + lucide-angular peer ranges. Removing chart.js/html2canvas/jspdf (§1-5) *shrinks* that surface today.
...
| **Field-overlay heatmaps** (deck.gl, d3-hexbin) | Same: no route/coverage/xy data. deck.gl is geo-scale tooling; there is no geo. If positional capture is ever *bought* (wearables), revisit — as a data-pipeline project first, not a UI project |
