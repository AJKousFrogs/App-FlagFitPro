...
const generateAICoachMessage = useCallback(() => {
  const messages = {
    ...
    analytical: [
      "Based on your biometrics, today's optimal training window is 2-4 PM.",
    ],
    supportive: [
  ...
  const personalityMessages =
    messages[aiCoachPersonality] || messages.motivational;
  return personalityMessages[
    Math.floor(Math.random() * personalityMessages.length)
  ];
}, [aiCoachPersonality]);
```
