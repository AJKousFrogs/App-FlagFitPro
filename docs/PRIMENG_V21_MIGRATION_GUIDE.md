...
```html
<p-table [lazy]="true" (onLazyLoad)="loadData($event)"></p-table>
```
...
<p-table [lazy]="true" (onLazyLoad)="loadData($event)">
  ...
  <p-table [virtualScroll]="true" [virtualScrollItemSize]="46"></p-table
></p-table>
```
...
```html
<p-table [value]="players$ | async"></p-table>
```
...
// Panel Events
import { PanelBeforeToggleEvent, PanelAfterToggleEvent } from "primeng/panel";
...
test.describe("Game Tracker", () => {
  ...
  test("should select a game row", async ({ page }) => {
    ...
    // Verify selection
    await expect(page.locator("tbody tr:first-child")).toHaveClass(
      /p-highlight/,
    );
  });
});
...
_Generated for FlagFit Pro - Flag Football Management Application_
