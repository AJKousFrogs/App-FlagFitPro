...
function generateProtocol(plan: TravelPlan): RecoveryProtocol[] {
  ...
  for (let day = 1; day <= recoveryDays; day++) {
    const phase =
      day <= recoveryDays / 2 ? "post-arrival" : "competition-ready";
...
function calculateSleepWindow(day: number, tzDiff: number, direction: string) {
  ...
  return {
    bedTime: adjustTime(
      baseTime,
      direction === "eastward" ? -adjustment : adjustment,
    ),
    wakeTime: adjustTime(
      "07:00",
      direction === "eastward" ? -adjustment : adjustment,
...
const NUTRITION_RECOMMENDATIONS = {
  morning: [
    ...
    {
      category: "food",
      item: "Eggs or Greek yogurt",
      amount: "2 eggs / 150g",
      reason: "Protein for muscle preparation",
    },
    {
      category: "drink",
      item: "Water",
      amount: "500ml",
      reason: "Start hydrated",
    },
    {
      category: "supplement",
      item: "Electrolyte tablet",
      amount: "1 tab",
      reason: "Pre-load sodium/potassium",
    },
  ],
  preGame: [
    {
      category: "food",
      item: "Banana or energy bar",
      amount: "1 medium",
      reason: "Quick carbs, easy to digest",
    },
    {
      category: "drink",
      item: "Sports drink",
    ...
    {
      category: "action",
      item: "Avoid high-fat foods",
      reason: "Slow digestion affects performance",
    },
  ],
  halftime: [
    {
      category: "food",
      item: "Orange slices or energy gel",
      amount: "2-3 slices",
      reason: "Quick glucose",
    },
    {
      category: "drink",
      item: "Electrolyte drink",
      amount: "200ml",
      reason: "Replace sweat losses",
    },
  ],
  postGame: [
    {
      category: "drink",
      item: "Chocolate milk or protein shake",
      amount: "400ml",
      reason: "Optimal 3:1 carb:protein ratio",
    },
    {
      category: "food",
      item: "Banana + handful of nuts",
      reason: "Replenish glycogen + healthy fats",
    },
  ],
  betweenGames: [
    {
      category: "food",
      item: "PB&J sandwich (half)",
      reason: "Balanced macros, familiar food",
    },
    {
      category: "food",
      item: "Trail mix",
      amount: "small handful",
      reason: "Sustained energy release",
    },
    {
      category: "drink",
      item: "Water + electrolytes",
      amount: "400ml",
      reason: "Continue hydration",
    },
  ],
};
```
  ...
  "positions": [
    {
      "position": "QB",
      "players": [
        {"player_id": "p1", "depth": 1},
        {"player_id": "p2", "depth": 2}
      ]
    },
    ...
  ]
...
function calculateAttendanceRate(records: AttendanceRecord[]): number {
  ...
  return totalEvents > 0
    ? Math.round((presentOrExcused / totalEvents) * 100)
    : 0;
}
```
...
function needsReplacement(item: EquipmentItem): boolean {
  return (
    ...
    (item.purchaseDate && daysSince(item.purchaseDate) > 730)
  ); // 2 years
}
```
...
function calculatePerformanceScore(data: AnalyticsData): number {
  const weights = {
    ...
    loadProgression: 0.2, // Progressive overload
    recoveryQuality: 0.2, // Wellness scores
    acwrOptimality: 0.2, // Time in sweet spot
...
POST /api/ai/chat
{
  "message": "What's the best recovery protocol after a high-intensity session?",
...
{
  "success": true,
  "response": "Based on your current ACWR of 1.2 and readiness score of 75...",
  "session_id": "session-123",
  "message_id": "msg-456",
  "safety_check": {
    "risk_level": "low",
...
const POSITION_WEIGHT_PROFILES = {
  QB: {
    minBMI: 22,
    maxBMI: 27,
    notes: "Moderate build for mobility + arm strength",
  },
...
function assessSupplementImpact(
  ...
  if (wellness.energy <= 4) {
    ...
    if (!ironTaken) {
      recommendations.push(
        "Iron supplementation may help with low energy levels",
      );
    }
  }
...
function getSprintTrainingZones(fortyTime: number) {
  ...
  return {
    zone1_recovery: { speed: maxSpeed * 0.5, description: "Recovery jogs" },
    zone2_aerobic: { speed: maxSpeed * 0.65, description: "Aerobic base" },
    zone3_tempo: { speed: maxSpeed * 0.8, description: "Tempo runs" },
    zone4_threshold: { speed: maxSpeed * 0.9, description: "High intensity" },
    zone5_max: { speed: maxSpeed * 1.0, description: "Maximum sprints" },
  };
}
```
...
function calculateDefensiveEfficiency(db: DBStatistics): number {
  ...
  const positiveActions =
    db.tackles + db.passDeflections + db.interceptions * 2 + db.sacks * 2;
...
function calculatePressureRate(blitzer: BlitzerStatistics): number {
  ...
  const totalRushes =
    blitzer.sacks +
    blitzer.passDeflections +
    blitzer.qbRunContainSuccessful +
    blitzer.qbRunContainUnsuccessful;
  const successfulPressures =
    blitzer.sacks + blitzer.passDeflections + blitzer.qbRunContainSuccessful;
...
const aiResponse = {
  ...
  sources: [
    {
      title: "Heat Adaptation in Athletes (Périard 2015)",
      type: "system_evidence_base",
    },
    { title: "Team Summer Tournament Protocol", type: "team_knowledge" },
    {
      title: "Personal Nutrition Plan from Dr. Smith",
...
const CHAT_PERMISSIONS = {
  ...
  guest: {
    // For approved guests (trainers, nutritionists, etc.)
    canCreateChannels: false,
    canPinMessages: false,
...
const PHASE_NUTRITION = {
  menstrual: {
    ...
    notes:
      "Iron loss during period - increase iron-rich foods with vitamin C for absorption",
  },
  ...
  luteal_late: {
    ...
    hydrationMultiplier: 1.1,
    notes:
      "Cravings common - satisfy with nutrient-dense options. Magnesium helps PMS symptoms.",
  },
};
```
...
function predictNextPeriod(cycleHistory: CycleEntry[]): Date {
  ...
  const avgCycleLength =
    recentCycles.reduce((sum, cycle) => sum + cycle.length, 0) /
    recentCycles.length;
...
function getPlayerPlaybookStatus(playerId: string): PlaybookProgress {
  ...
  return {
    ...
    needsReview: playerProgress.filter(
      (p) => p.isMemorized && daysSince(p.lastStudied) > 7,
    ),
  };
}
```
...
function getFilmComplianceReport(teamId: string): FilmComplianceReport {
  ...
  return {
    ...
    bySession: requiredFilm.map((session) => ({
      ...
      completionRate:
        (session.completedBy.length / session.requiredFor.length) * 100,
    })),
  };
}
```
...
function detectConflicts(
  ...
  for (const existing of existingEvents) {
    ...
    if (
      eventsSameDay(newEvent, existing) &&
...
const HEAT_INDEX_GUIDELINES = {
  ...
  moderate: {
    max: 32.2,
    color: "yellow",
    message: "Use caution - increase rest breaks",
  },
  high: {
    max: 39.4,
    color: "orange",
    message: "Reduce intensity and duration",
  },
  veryHigh: { max: 46.1, color: "red", message: "Limit outdoor activity" },
  extreme: {
    max: Infinity,
    color: "purple",
    message: "Cancel outdoor activity",
  },
};
...
function assessHeatSafety(weather: WeatherData): HeatSafetyAssessment {
  ...
  if (heatIndex >= 39.4) {
    return {
      ...
      recommendation:
        "⚠️ Significantly reduce practice. Max 30 minutes with frequent breaks.",
    };
  }
...
async function monitorLightning(
  location: Coordinates,
): Promise<LightningStatus> {
...
async function checkWeatherForUpcomingEvents(): Promise<void> {
  ...
  for (const event of upcomingEvents) {
    ...
    const lightningStatus = await getLightningForecast(
      event.location.coordinates,
      event.startDateTime,
    );
    ...
    if (
      heatAssessment.riskLevel === "very_high" ||
      heatAssessment.riskLevel === "extreme"
    ) {
...
function findOptimalPracticeTime(
  ...
  const recommendations = timeSlots.map((slot) => {
    ...
    // Deduct for heat
    if (heatAssessment.riskLevel === "moderate") {
      score -= 20;
      warnings.push("Warm");
    }
    if (heatAssessment.riskLevel === "high") {
      score -= 40;
      warnings.push("Hot");
    }
    if (heatAssessment.riskLevel === "very_high") {
      score -= 70;
      warnings.push("Very hot");
    }
    if (heatAssessment.riskLevel === "extreme") {
      score = 0;
      warnings.push("Dangerous heat");
    }
    ...
    if (forecast.uvIndex > 10) {
      score -= 25;
      warnings.push("Extreme UV");
    }
...
8. Acta Scientiae et Intellectus (2025). "Training performance variations across menstrual cycle phases." (Note: "Intellectus" is correct Latin)
...
_This documentation is maintained by the FlagFit Pro development team. For questions, contact the product team._
