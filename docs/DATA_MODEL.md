...
## Progress log
- **2026-05-29 — Phase 12 done (live DB · security):** revoked RPC `EXECUTE` from anon/authenticated/public on all 32 trigger functions (they're never legit RPCs; trigger firing unaffected) — cleared the trigger subset of the SECURITY DEFINER advisor findings (32→0). Migration `20260529230000`. **Remaining security (deliberate):** per-function `anon` revokes on the non-trigger definer RPCs code calls; enable leaked-password protection (auth toggle).
