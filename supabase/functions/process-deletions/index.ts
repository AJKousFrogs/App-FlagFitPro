...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: pendingDeletions, error: fetchError } = await supabase.rpc(
    ...
    if (fetchError) {
      ...
      for (const deletion of pendingDeletions) {
        try {
          ...
          if (deleteError) {
            results.errors.push(
              `Failed to process deletion ${deletion.request_id}: ${deleteError.message}`,
            );
            ...
            if (authDeleteError) {
              results.errors.push(
                `Failed to delete auth user ${deletion.user_id}: ${authDeleteError.message}`,
              );
              results.deletionsFailed++;
            } else {
          ...
          results.errors.push(
            `Error processing deletion ${deletion.request_id}: ${message}`,
          );
          results.deletionsFailed++;
        }
      }
    ...
    if (cleanupError) {
      results.errors.push(
        `Failed to cleanup emergency records: ${cleanupError.message}`,
      );
    } else {
