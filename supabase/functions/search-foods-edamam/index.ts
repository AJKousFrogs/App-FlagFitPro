...
Deno.serve(async (req) => {
  ...
  try {
    const { query, pageSize = 25, pageNumber = 1 } = await req.json() as FoodSearchRequest;
    ...
    const foods = edamamData.hints?.slice(0, pageSize).map((hint: any) => {
    ...
    return new Response(
      JSON.stringify({
        success: true,
        data: foods,
        totalHits: edamamData.parsed?.length + edamamData.hints?.length || foods.length,
