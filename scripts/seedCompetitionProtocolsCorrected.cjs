  ...
  async seedOlympicGamesProtocols() {
    ...
    try {
      ...
      if (tableCheck.rows.length === 0) {
        console.log(
          "⚠️ Olympic Games protocols table does not exist, skipping...",
        );
        return;
      }

      console.log(
        "📋 Olympic Games table structure:",
        tableCheck.rows.map((r) => `${r.column_name} (${r.data_type})`),
      );

      // For now, just log the structure since we need to see what columns exist
      console.log(
        "🔍 Need to examine Olympic Games table structure before seeding",
      );
    } catch (error) {
      console.log(
        "⚠️ Olympic Games protocols table not accessible:",
        error.message,
      );
    }
  }
