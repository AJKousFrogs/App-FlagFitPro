...
const isMain =
  process.argv[1]?.endsWith("seedEnhancedTrainingCategories.js") ||
...
const WEEKS_TO_LA28 = Math.ceil(
  (LA28_DATE.getTime() - new Date().getTime()) / (1000 * 3600 * 24 * 7),
);
  ...
  try {
    ...
    for (const category of enhancedTrainingCategories) {
      ...
      const values = [
        ...
        category.la28_weekly_target,
        category.la28_total_sessions,
        category.la28_minimum_proficiency,
        category.session_duration_minutes,
        category.rest_hours_required,
        category.energy_expenditure,
        category.skill_transfer_rate,
        category.fatigue_factor,
        category.recovery_factor,
        category.plateau_factor,
        category.synergy_factor,
      ];
    ...
    for (const factor of mathematicalCorrectionFactors) {
      ...
      const values = [
        ...
        factor.min_value,
        factor.max_value,
        factor.adjustment_rate,
        factor.decay_factor,
        factor.recovery_rate,
        factor.research_source,
      ...
      const result = await client.query(query, values);
      console.log(
        `✅ Inserted correction factor: ${result.rows[0].factor_name}`,
      );
    }
    ...
    for (const requirement of la28OlympicRequirements) {
      ...
      const values = [
        ...
        requirement.target_proficiency,
        requirement.elite_proficiency,
        requirement.minimum_weekly_sessions,
        requirement.recommended_weekly_sessions,
        requirement.total_sessions_by_la28,
        requirement.deadline_date,
        requirement.weeks_remaining,
        requirement.critical_path_weeks,
        requirement.importance_weight,
        requirement.difficulty_level,
      ];

      const result = await client.query(query, values);
      console.log(
        `✅ Inserted LA28 requirement: ${result.rows[0].requirement_name}`,
      );
    }
    ...
    console.log(
      `📊 Total categories inserted: ${enhancedTrainingCategories.length}`,
    );
    console.log(
      `📊 Total correction factors inserted: ${mathematicalCorrectionFactors.length}`,
    );
    console.log(
      `📊 Total LA28 requirements inserted: ${la28OlympicRequirements.length}`,
    );
...
export { seedEnhancedTrainingCategories };
