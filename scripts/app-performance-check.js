...
class AppPerformanceChecker {
  ...
  analyzeBuildSize(distPath) {
    ...
    try {
      ...
      if (totalBytes > 5 * 1024 * 1024) { // >5MB
        this.results.recommendations.push('Build size is large, consider optimization');
      }
      if (jsBytes > 2 * 1024 * 1024) { // >2MB JS
        this.results.recommendations.push('JavaScript bundle is large, consider code splitting');
      }
  ...
  async checkBundleAnalysis() {
    ...
    try {
      ...
      if (existsSync(analyzerPath)) {
        ...
        this.results.recommendations.push('Run bundle analysis: npm run build:analyze');
      }
    ...
    try {
      ...
      try {
        ...
        if (outdatedCount > 0) {
          ...
          this.results.recommendations.push('Update outdated packages: npm update');
        } else {
  ...
  generateReport() {
    ...
    console.log(`🔨 Build: ${build.status === 'error' ? '❌' : '✅'} ${build.status}`);
    ...
    console.log(`📦 Bundle: ${bundle.status === 'analyzed' ? '✅' : 'ℹ️'} ${bundle.status}`);
    
    // Runtime Performance
    const runtime = this.results.runtime;
    console.log(`⚡ Runtime: ${runtime.status === 'running' ? '✅' : 'ℹ️'} ${runtime.status}`);
    
    // Memory Usage
    const memory = this.results.memory;
    console.log(`💾 Memory: ${memory.status === 'checked' ? '✅' : '❌'} ${memory.status}`);
    if (memory.usage.rss) {
...
async function main() {
  ...
  switch (command) {
    case 'monitor':
      console.log('🔄 Starting continuous performance monitoring (Ctrl+C to stop)...\n');
      const monitor = async () => {
        console.clear();
