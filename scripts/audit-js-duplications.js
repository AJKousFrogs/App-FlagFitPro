...
function walkDir(dir, ext, files = [], opts = {}) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      if (opts.skipDirs && opts.skipDirs.includes(e.name)) {
        continue;
      }
  ...
  for (const file of files) {
    ...
    while ((m = strRegex.exec(content)) !== null) {
      ...
      if (lit.length > 12 && !lit.includes("${") && !lit.startsWith("http")) {
        ...
        if (!literalFiles.has(lit)) {
          literalFiles.set(lit, []);
        }
...
function findSimilarFunctionBlocks(files) {
  ...
  const funcRegex =
    /(?:async\s+)?(?:function\s+)?(\w+)\s*\([^)]*\)\s*\{([\s\S]{1,500}?)\n\s*\}/g;
  for (const file of files) {
  ...
  for (const file of files) {
    ...
    for (const line of imports) {
      ...
      if (m) {
        ...
        if (!reqs.get(pkg).includes(rel)) {
          reqs.get(pkg).push(rel);
        }
      }
    }
...
function main() {
  ...
  console.log(
    `📁 Scanned ${files.length} JS files (scripts/ + netlify/functions/)\n`,
  );
  ...
  if (repeatedLiterals.length === 0) {
    ...
    repeatedLiterals.slice(0, 15).forEach(({ literal, count, files: fs }) => {
      const preview =
        literal.length > 50 ? `${literal.slice(0, 47)}...` : literal;
