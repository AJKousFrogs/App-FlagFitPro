...
async function testLogin() {
  ...
  for (const credentials of testCredentials) {
    ...
    try {
      ...
      if (response.ok && data.success && data.data.token) {
        ...
        log(
          `   🔑 Token received: ${data.data.token.substring(0, 20)}...`,
          "green",
        );
        log(
          `   👤 User: ${data.data.user.fullName} (${data.data.user.email})`,
          "green",
        );
        passedTests++;
      } else {
  ...
  if (passedTests === totalTests) {
    log(
      "\n🎉 All login tests passed! Your authentication system is working correctly.",
      "green",
    );
    log("\n💡 Next Steps:", "bright");
    ...
    log(
      "\n⚠️  Some login tests failed. Check your server and database connection.",
      "yellow",
    );
  }
}
