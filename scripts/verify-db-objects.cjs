...
async function checkTriggers(supabase) {
  ...
  if (error || !triggers) {
    for (const trigger of REQUIRED_TRIGGERS) {
      logWarn(`Trigger: ${trigger}`, 'Cannot verify (no sql RPC) - check via migration logs');
    }
  } else {
    for (const trigger of REQUIRED_TRIGGERS) {
      if (triggerNames.includes(trigger)) {
        const triggerInfo = triggers?.find(t => t.tgname === trigger);
        logPass(`Trigger: ${trigger}`, `Attached to: ${triggerInfo?.table_name || 'unknown'}`);
      } else {
        logFail(`Trigger: ${trigger}`, `Trigger '${trigger}' does not exist or is not attached`);
      }
    }
  }
...
async function checkRLS(supabase) {
  ...
  for (const table of RLS_REQUIRED_TABLES) {
    if (tableRLS[table] === true) {
      ...
      logWarn(`RLS: ${table}`, `Table '${table}' not found or RLS status unknown`);
    }
  }
...
async function runVerifyDatabaseBootstrap(supabase) {
  ...
  const { data: results, error } = await supabase.rpc('verify_database_bootstrap');

  if (error) {
    logWarn('verify_database_bootstrap', `Function returned error: ${error.message}`);
    return;
  }
