...
function auditComponent(componentPath) {
  ...
  const inlineTemplateMatch = componentContent.match(/template:\s*`([\s\S]*?)`/);
  ...
  if (templateContent && result.hasScss) {
    ...
    const inlineStylesMatch = componentContent.match(/styles:\s*\[\s*`([\s\S]*?)`/);
...
if (require.main === module) {
  ...
  function scanDirectory(dir) {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.name.endsWith('.component.ts') && !entry.name.includes('.spec.')) {
  ...
  console.log(JSON.stringify({
    ...
    summary: {
      ...
      missingScss: issues.filter(r => !r.hasScss && !r.hasInlineStyles && r.hasTemplate).map(r => ({
        component: r.componentName,
        path: r.component
      }))
    }
