...
class SupplementResearchSeeder {
  ...
  async seedCreatineResearch() {
    ...
    const creatineStudies = [
      {
        study_title: 'Effects of Creatine Supplementation on Performance and Training Adaptations',
      ...
      {
        study_title: 'Creatine Supplementation and Sprint Performance in Elite Athletes',
    ...
    for (const study of creatineStudies) {
      try {
        ...
        await this.pool.query(query, [
          ...
          study.doi, study.evidence_level, study.sport_specific, study.competition_level,
          study.dosage_mg_per_kg, study.loading_phase_days, study.maintenance_dose_mg_per_day,
          study.strength_improvement_percentage, study.power_improvement_percentage,
          study.endurance_improvement_percentage, study.flag_football_relevance_score,
          study.safety_profile, study.side_effects, study.contraindications, study.created_at
        ]);
  ...
  async seedBetaAlanineResearch() {
    ...
    const betaAlanineStudies = [
      {
        study_title: 'Beta-Alanine Supplementation and High-Intensity Exercise Performance',
      ...
      {
        study_title: 'Effects of Beta-Alanine on Sprint Performance in Team Sport Athletes',
    ...
    for (const study of betaAlanineStudies) {
      try {
        ...
        console.error(`❌ Error inserting beta-alanine study: ${error.message}`);
      }
    }
  }
  ...
  async seedSupplementsTable() {
    ...
    const supplements = [
      {
        ...
        primary_benefits: ['increased strength', 'improved power output', 'enhanced recovery'],
        dosage_form: 'powder',
      ...
      },
      {
        ...
        primary_benefits: ['increased muscular endurance', 'delayed fatigue', 'improved sprint performance'],
        dosage_form: 'capsule',
      ...
      },
      {
        ...
        primary_benefits: ['increased alertness', 'improved focus', 'enhanced endurance'],
        dosage_form: 'capsule',
      ...
      },
      {
        ...
        primary_benefits: ['muscle protein synthesis', 'recovery support', 'muscle maintenance'],
        dosage_form: 'powder',
    ...
    for (const supplement of supplements) {
      try {
        ...
        await this.pool.query(query, [
          supplement.supplement_name, supplement.supplement_type, supplement.primary_benefits,
          supplement.dosage_form, supplement.recommended_dosage, supplement.timing,
          supplement.evidence_level, supplement.wada_status, supplement.flag_football_relevance,
          supplement.cost_per_month, supplement.created_at
        ]);
    ...
    for (const plan of supplementPlans) {
      try {
        ...
        await this.pool.query(query, [
          ...
          JSON.stringify(plan.supplement_assignments), plan.created_at
        ]);
