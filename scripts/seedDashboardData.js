...
const sampleData = {
  ...
  dailyQuotes: [
    ...
    {
      quote_text: 'The difference between the impossible and the possible lies in determination.',
      author: 'Tommy Lasorda',
      category: 'motivation',
      is_active: true
    },
    {
      quote_text: 'Success is no accident. It is hard work, perseverance, learning, studying, sacrifice and most of all, love of what you are doing.',
      author: 'Pelé',
      category: 'motivation',
      is_active: true
    }
  ...
  try {
    ...
    for (const session of sampleData.trainingSessions) {
      await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [session.user_id, session.session_date, session.session_type, session.status, session.duration_minutes, session.performance_score]);
    }
    ...
    for (const metric of sampleData.performanceMetrics) {
      await pool.query(`
        ...
        VALUES ($1, $2, $3, $4)
      `, [metric.user_id, metric.performance_score, metric.metric_type, metric.created_at]);
    }
    ...
    for (const chemistry of sampleData.teamChemistry) {
      await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [chemistry.user_id, chemistry.chemistry_score, chemistry.communication_score, chemistry.trust_score, chemistry.leadership_score, chemistry.created_at]);
    }
    ...
    for (const olympic of sampleData.olympicQualification) {
      ...
      `, [olympic.user_id, olympic.qualification_probability, olympic.world_ranking, olympic.days_until_championship, olympic.european_championship_date, olympic.world_championship_date, olympic.olympic_date, olympic.created_at]);
    }
    ...
    for (const benchmark of sampleData.performanceBenchmarks) {
      await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5)
      `, [benchmark.user_id, benchmark.metric_name, benchmark.current_value, benchmark.target_value, benchmark.unit]);
    }
    ...
    for (const reward of sampleData.sponsorRewards) {
      await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [reward.user_id, reward.available_points, reward.current_tier, reward.products_available, reward.tier_progress_percentage, reward.created_at]);
    }
    ...
    for (const product of sampleData.sponsorProducts) {
      await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5)
      `, [product.product_name, product.points_cost, product.relevance_score, product.category, product.is_featured]);
    }
    ...
    for (const wearable of sampleData.wearablesData) {
      await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      `, [wearable.user_id, wearable.device_type, wearable.heart_rate, wearable.hrv, wearable.sleep_score, wearable.training_load, wearable.last_sync, wearable.connection_status]);
    }
    ...
    for (const notification of sampleData.notifications) {
      await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [notification.user_id, notification.notification_type, notification.message, notification.is_read, notification.priority, notification.created_at]);
    }
