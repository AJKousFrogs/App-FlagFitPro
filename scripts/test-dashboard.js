...
async function testEndpoint(endpoint, description) {
    try {
        ...
        if (response.ok && data.success) {
            ...
            log(`   📊 Data received: ${JSON.stringify(data.data).substring(0, 100)}...`, 'green');
        } else {
...
async function runTests() {
    ...
    if (passedTests === totalTests) {
        log('\n🎉 All tests passed! Your dashboard API is working correctly.', 'green');
    } else {
        log('\n⚠️  Some tests failed. Check your server and database connection.', 'yellow');
    }
