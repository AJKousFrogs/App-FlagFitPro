  ...
  try {
    ...
    if (data.resultList && data.resultList.result) {
      return data.resultList.result.map(article => ({
        ...
        pdf_url: article.fullTextUrlList?.fullTextUrl?.find(url => url.availability === 'Open access')?.url,
    ...
    console.error(`Error fetching from Europe PMC for query "${query}":`, error.message);
    return [];
  }
}
...
async function fetchSemanticScholar(query, category, maxResults = 50) {
  ...
  try {
    ...
    console.error(`Error fetching from Semantic Scholar for query "${query}":`, error.message);
    return [];
  }
}
...
function extractTags(article, category) {
  ...
  if (text.includes('cold') || text.includes('cryotherapy') || text.includes('ice')) tags.push('cold_therapy');
...
function determineEvidenceLevel(studyType, citationCount) {
...
async function insertArticle(article) {
  ...
  const evidenceLevel = determineEvidenceLevel(studyType, article.citation_count || 0);
