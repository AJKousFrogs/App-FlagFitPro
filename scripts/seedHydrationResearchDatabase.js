...
class HydrationResearchSeeder {
  ...
  async seedHydrationResearchStudies() {
    ...
    const studies = [
      {
        study_title: 'Hydration and Performance in Team Sports: A Systematic Review and Meta-Analysis',
        authors: ['Sawka, M.N.', 'Burke, L.M.', 'Eichner, E.R.', 'Maughan, R.J.', 'Montain, S.J.', 'Stachenfeld, N.S.'],
      ...
      {
        study_title: 'Fluid and Electrolyte Balance in Elite Team Sport Athletes',
      ...
      {
        study_title: 'Hydration Strategies for Football Players: From Training to Competition',
        authors: ['Casa, D.J.', 'Stearns, R.L.', 'Lopez, R.M.', 'Ganio, M.S.', 'McDermott, B.P.', 'Walker, A.'],
    ...
    for (const study of studies) {
      ...
      `, [
        study.study_title, study.authors, study.publication_year, study.journal,
        study.doi, study.study_type, study.evidence_level, study.sample_size,
        study.population_studied, study.key_findings, study.practical_applications,
        study.sport_specific, study.competition_level
      ]);
    }
  ...
  async seedIFAFHydrationProtocols() {
    ...
    for (const protocol of protocols) {
      await this.pool.query(`
        ...
        ON CONFLICT DO NOTHING
      `, [
        protocol.competition_type, protocol.competition_level, protocol.games_per_day,
        protocol.game_duration_minutes, protocol.total_playing_time_minutes, protocol.time_between_games_minutes,
        protocol.typical_temperature_celsius, protocol.typical_humidity_percentage, protocol.indoor_outdoor,
        protocol.altitude_meters, protocol.pre_game_hydration_ml_per_kg, protocol.pre_game_timing_hours,
        protocol.during_game_hydration_ml_per_15min, protocol.between_games_hydration_ml_per_kg,
        protocol.post_game_hydration_ml_per_kg, protocol.sodium_mg_per_liter, protocol.potassium_mg_per_liter,
        protocol.magnesium_mg_per_liter, protocol.calcium_mg_per_liter, protocol.urine_color_target,
  ...
  async seedTrainingHydrationProtocols() {
    ...
    for (const protocol of protocols) {
      await this.pool.query(`
        ...
        ON CONFLICT DO NOTHING
      `, [
        protocol.training_type, protocol.training_duration_minutes, protocol.training_intensity,
        protocol.pre_training_hydration_ml_per_kg, protocol.pre_training_timing_hours,
        protocol.during_training_hydration_ml_per_15min, protocol.post_training_hydration_ml_per_kg,
        protocol.sodium_replacement_mg_per_hour, protocol.potassium_replacement_mg_per_hour,
        protocol.temperature_adjustment_factor, protocol.humidity_adjustment_factor,
  ...
  async seedSupplementResearch() {
    ...
    const creatineStudies = [
      {
        ...
        position_specific_benefits: ['quarterback', 'receiver', 'defensive back'],
    ...
    for (const study of creatineStudies) {
      await this.pool.query(`
        ...
        ON CONFLICT DO NOTHING
      `, [
        study.creatine_form, study.dosage_mg_per_kg, study.loading_phase_days,
        study.maintenance_dose_mg_per_day, study.strength_improvement_percentage,
        study.power_improvement_percentage, study.sprint_performance_improvement,
        study.muscle_mass_gain_kg, study.side_effects, study.flag_football_relevance_score,
        study.position_specific_benefits, study.study_duration_weeks, study.population_size
      ]);
    }
    ...
    const betaAlanineStudies = [
      {
        ...
        flag_football_benefits: ['repeated sprint ability', 'muscular endurance', 'recovery between plays'],
    ...
    for (const study of betaAlanineStudies) {
      await this.pool.query(`
        ...
        ON CONFLICT DO NOTHING
      `, [
        study.dosage_mg_per_day, study.administration_frequency, study.loading_phase_weeks,
        study.muscular_endurance_improvement, study.anaerobic_power_improvement,
        study.time_to_exhaustion_improvement, study.paresthesia_incidence_percentage,
        study.flag_football_benefits, study.optimal_timing
      ]);
    }
    ...
    const caffeineStudies = [
      {
        ...
        pre_game_timing_recommendations: ['60 minutes before kickoff', 'avoid within 6 hours of bedtime'],
        during_game_use_recommendations: ['caffeine gum for halftime', 'avoid energy drinks during play']
      }
    ];
    ...
    for (const study of caffeineStudies) {
      await this.pool.query(`
        ...
        ON CONFLICT DO NOTHING
      `, [
        study.dosage_mg_per_kg, study.timing_minutes_before_exercise, study.form,
        study.endurance_improvement_percentage, study.power_improvement_percentage,
        study.cognitive_enhancement_score, study.reaction_time_improvement,
        study.habitual_caffeine_use_impact, study.pre_game_timing_recommendations,
  ...
  async seedInjuryPrevention() {
    ...
    for (const protocol of preventionProtocols) {
      await this.pool.query(`
        ...
        ON CONFLICT DO NOTHING
      `, [
        protocol.injury_type, protocol.risk_factors, protocol.hydration_prevention_protocols,
        protocol.flag_football_specific, protocol.prevention_effectiveness_percentage
      ]);
    }
  ...
  async seedResearchSources() {
    ...
    for (const source of sources) {
      await this.pool.query(`
        ...
        ON CONFLICT DO NOTHING
      `, [
        source.source_name, source.source_type, source.url, source.impact_factor,
        source.peer_review_status, source.update_frequency, source.primary_domains,
        source.sports_coverage, source.hydration_focus, source.performance_nutrition_focus,
        source.access_type, source.data_quality_score, source.machine_readable_format,
  ...
  async runAllSeeders() {
    try {
      ...
      console.log('🎉 Hydration research database seeding completed successfully!');
    } catch (error) {
...
module.exports = HydrationResearchSeeder;
