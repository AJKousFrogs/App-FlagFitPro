...
class ComprehensiveDatabaseAudit {
  ...
  async auditTableStructure(tableName) {
    try {
      ...
      const structureResult = await this.pool.query(structureQuery, [
        tableName,
      ]);
      ...
      structureResult.rows.forEach((column) => {
        ...
        console.log(
          `     - ${column.column_name}: ${column.data_type}${maxLength} ${nullable}${defaultValue}`,
        );
      });
  ...
  async auditSpecificSystems() {
    ...
    for (const system of systems) {
      ...
      for (const tableName of system.tables) {
        try {
          ...
          if (count > 0) {
            ...
            console.log(
              `   ⚠️ ${tableName}: 0 records (table exists but empty)`,
            );
            implementedTables++;
          }
        } catch (_error) {
      ...
      const completionPercentage =
        (implementedTables / system.tables.length) * 100;
      const dataPercentage = totalRecords > 0 ? " (with data)" : " (no data)";

      console.log(
        `   📊 Completion: ${completionPercentage.toFixed(0)}%${dataPercentage}`,
      );
    }
  }
...
export default ComprehensiveDatabaseAudit;
