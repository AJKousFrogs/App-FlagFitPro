  ...
  files.forEach((file) => {
    ...
    if (result.changed) {
      ...
      result.changes.forEach((change) => {
        console.log(
          `   ${change.type}: ${change.old} → ${change.replacement} (${change.count}x)`,
        );
      });
    } else {
...
async function main() {
  ...
  const screenName = args
    .find((arg) => arg.startsWith("--screen="))
    ?.split("=")[1];
  ...
  if (componentName) {
    ...
    const componentNames = [
      ...new Set(componentFiles.map((f) => path.basename(f, ".component.ts"))),
    ];
