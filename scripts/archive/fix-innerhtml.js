...
const simplePatterns = [
  ...
  // element.innerHTML = variable (where variable doesn't contain HTML)
  {
    ...
    replace: (match, elem, variable) => {
      ...
      if (variable.toLowerCase().includes('html') || variable.toLowerCase().includes('markup')) {
...
function processFile(filePath) {
  try {
    ...
    if (content.includes('setSafeContent') ||
        content.includes('eslint-disable') && content.includes('no-restricted-syntax') ||
