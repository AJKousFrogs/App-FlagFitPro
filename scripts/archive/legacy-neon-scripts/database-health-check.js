...
class DatabaseHealthChecker {
  constructor() {
    this.connectionString = process.env.DATABASE_URL || process.env.VITE_DATABASE_URL;
    this.results = {
  ...
  async checkSchema() {
    ...
    try {
      ...
      const requiredTables = [
        'users', 'training_sessions', 'training_goals', 
        'analytics_events', 'teams', 'games',
        'foods', 'nutrients', 'recovery_protocols',
        'ai_coaches', 'research_studies'
      ];
      ...
      this.results.schema.missing = requiredTables.filter(table => 
      ...
      if (this.results.schema.missing.length === 0) {
        ...
        console.log(`⚠️  Missing tables: ${this.results.schema.missing.join(', ')}`);
        this.results.recommendations.push('Run database migrations: npm run db:migrate');
      }
    ...
    try {
      ...
      for (const query of countQueries) {
        try {
          ...
          if (count === 0) {
            this.results.recommendations.push(`Seed ${tableName} data: npm run db:seed`);
          }
        } catch (error) {
      ...
      try {
        ...
        if (slowQueries.length > 0) {
          ...
          slowQueries.forEach(q => {
            console.log(`      ${q.query?.substring(0, 50)}... (${q.mean_time}ms avg)`);
          });
        }
      } catch (error) {
  ...
  async checkBackupStatus() {
    ...
    try {
      ...
      if (backupTables.length > 0) {
        console.log(`✅ Backup system detected (${backupTables.length} tables)`);
  ...
  generateReport() {
    ...
    console.log(`🔌 Connectivity: ${conn.status === 'connected' ? '✅' : '❌'} ${conn.status}`);
    ...
    console.log(`📋 Schema: ${schema.status === 'complete' ? '✅' : '⚠️'} ${schema.status}`);
    ...
    console.log(`📊 Data: ${data.status === 'checked' ? '✅' : '❌'} ${data.status}`);
    if (data.counts) {
    ...
    console.log(`⚡ Performance: ${perf.status === 'checked' ? '✅' : '❌'} ${perf.status}`);
    if (perf.metrics) {
...
async function main() {
  ...
  switch (command) {
    case 'monitor':
      console.log('🔄 Starting continuous database monitoring (Ctrl+C to stop)...\n');
      const monitor = async () => {
        console.clear();
        console.log(`🏈 Database Health Monitor - ${new Date().toLocaleTimeString()}\n`);
