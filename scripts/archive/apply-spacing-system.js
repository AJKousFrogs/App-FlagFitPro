...
function addSpacingSystem(content) {
  ...
  const designSystemPattern =
    /(<link[^>]*href=["']\.\/src\/comprehensive-design-system\.css["'][^>]*>)/i;
...
function main() {
  console.log(
    "🎨 Applying Comprehensive Spacing System to all HTML pages...\n",
  );
  ...
  for (const filePath of htmlFiles) {
    try {
      ...
      if (hasSpacingSystem(content)) {
        console.log(
          `⏭️  Skipped: ${relativePath} (already has spacing-system.css)`,
        );
        skipped++;
        continue;
      }
      ...
      if (updatedContent !== content) {
        ...
        console.log(
          `⚠️  No changes: ${relativePath} (could not find insertion point)`,
        );
        skipped++;
      }
    } catch (error) {
