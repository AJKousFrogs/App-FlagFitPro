...
function fixIconStyles(htmlContent) {
    ...
    updated = updated.replace(
        ...
        (match, iconName, styles) => {
            ...
            let cleanStyles = styles.replace(/color:\s*[^;]+/g, '').replace(/stroke:\s*[^;]+/g, '');
            ...
            cleanStyles += ' color: var(--icon-color-muted); stroke: var(--icon-color-muted);';
    ...
    updated = updated.replace(
        ...
        (match, iconName, styles) => {
            ...
            cleanStyles += ' color: var(--icon-color-secondary); stroke: var(--icon-color-secondary);';
            return match.replace(/style="[^"]*"/, `style="${cleanStyles.trim()}"`);
        }
    );
    ...
    updated = updated.replace(
        ...
        (match, iconName, attrs) => {
            if (attrs.includes('style=')) {
                ...
                styles = styles.replace(/color:\s*[^;]+/g, '').replace(/stroke:\s*[^;]+/g, '');
                styles = styles.replace(/;;/g, ';').replace(/;\s*;/g, ';');
                // Only add default white if not in sidebar/header/search (those have specific colors)
                if (!attrs.includes('sidebar-icon') && !attrs.includes('header-icon') && !attrs.includes('search-icon')) {
                    if (!styles.includes('color:')) {
                        styles += ' color: var(--icon-color-primary); stroke: var(--icon-color-primary);';
                    }
                }
                ...
                if (!attrs.includes('sidebar-icon') && !attrs.includes('header-icon') && !attrs.includes('search-icon')) {
                    attrs += ' style="color: var(--icon-color-primary); stroke: var(--icon-color-primary);"';
                }
            }
