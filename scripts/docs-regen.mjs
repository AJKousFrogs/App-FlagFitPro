...
function loadLive() {
  const snap = JSON.parse(
    readFileSync(join(OUT_DIR, "live-schema.snapshot.json"), "utf8"),
  );
  return {
    tables: snap.tables,
    views: snap.views,
    generatedOn: snap.generatedOn,
  };
}
...
function typesTableNames() {
  ...
  for (const line of text.split("\n")) {
    ...
    if (/^ {4}(Views|Functions|Enums|CompositeTypes): \{/.test(line)) {
      section = null;
      continue;
    }
...
function parseFunctions(fnPaths) {
  ...
  const files = readdirSync(dir).filter(
    (f) =>
      f.endsWith(".js") && f !== "supabase-client.js" && f !== "validation.js",
  );
  // All /api references in the Angular frontend (one grep, reused).
  let feRefs = "";
  try {
    feRefs = execSync(`grep -rhoE "/api/[A-Za-z0-9/_-]+" angular/src || true`, {
      cwd: ROOT,
  ...
  for (const file of files) {
    ...
    if (am) methods = [...am[1].matchAll(/"([A-Z]+)"/g)].map((m) => m[1]);
      ...
      const hm = new Set(
        [...src.matchAll(/httpMethod\s*===?\s*"([A-Z]+)"/g)].map((m) => m[1]),
      );
      [...src.matchAll(/req\.method\s*===?\s*"([A-Z]+)"/g)].forEach((m) =>
        hm.add(m[1]),
      );
      methods = [...hm];
    }
    ...
    const exercised =
      paths.some((p) => feRefs.includes(p.replace(/\/\*$/, ""))) ||
      Boolean(importedBy);
    fns.push({
      name,
      methods,
...
function migrationMentions() {
  ...
  for (const d of ["database/migrations", "supabase/migrations"]) {
    ...
    let files = [];
    try {
      files = readdirSync(abs).filter((f) => f.endsWith(".sql"));
    } catch {
      continue;
    }
...
function main() {
  ...
  const missingFromTypes = [...liveNames]
    .filter((n) => !typesNames.has(n))
    .sort(); // live, types not regenerated
  ...
  for (const fn of fns)
    for (const t of fn.tables) (tableEndpoints[t] ||= new Set()).add(fn.name);

  const tableNames = Object.keys(tables).sort();
  const driftTables = tableNames.filter(
    (t) => !new RegExp(`\\b${t}\\b`).test(migBlob),
  );
  ...
  if (driftTables.length)
    dm += `**DRIFT (live, no migration file):** ${driftTables.map((t) => `\`${t}\``).join(", ")}\n\n`;
  dm += `## Tables\n\n`;
  for (const t of tableNames) {
    ...
    for (const { col, type, nullable } of tables[t])
      dm += `- \`${col}\` ${type}${nullable ? "" : " · not null"}\n`;
    dm += `\n`;
  }
  dm += `## Views\n\n`;
  for (const v of Object.keys(views).sort()) {
    dm += `### \`${v}\` (view)\n`;
    for (const { col, type, nullable } of views[v])
      dm += `- \`${col}\` ${type}${nullable ? "" : " · not null"}\n`;
    dm += `\n`;
  }
  ...
  for (const f of fns)
    for (const x of f.tables)
      if (!liveTableSet.has(x) && !STORAGE_BUCKETS.has(x))
        unknownTableRefs.add(x);
  console.log(
    `DATA_MODEL: ${tableNames.length} tables, ${Object.keys(views).length} views, ${driftTables.length} DRIFT`,
  );
  console.log(
    `types staleness: ${staleInTypes.length} stale-in-types, ${missingFromTypes.length} live-missing-from-types`,
  );
  console.log(
    `ENDPOINTS:  ${fns.length} functions, ${exercised.length} exercised, ${orphaned.length} ORPHANED -> ${orphaned.map((o) => o.name).join(", ")}`,
  );
  console.log(`DRIFT tables: ${driftTables.join(", ") || "(none)"}`);
  console.log(
    `Unknown table refs (⚠️ verify): ${[...unknownTableRefs].join(", ") || "(none)"}`,
  );
}
