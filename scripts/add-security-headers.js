    ...
    processHTMLFile(filePath) {
        try {
            ...
            if (content.includes('Content-Security-Policy')) {
                console.log(`Skipping ${path.relative(projectRoot, filePath)} - already has CSP`);
                return false;
            }
...
`;
        ...
        try {
            ...
            if (!existingConfig.includes('X-Frame-Options') || !existingConfig.includes('Content-Security-Policy')) {
    ...
    async run() {
        ...
        console.log(`📊 Summary: ${this.updatedFiles}/${this.htmlFiles.length} files updated`);
