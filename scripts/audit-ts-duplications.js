  ...
  for (const file of files) {
    ...
    while ((m = strRegex.exec(content)) !== null) {
      ...
      if (lit.length > 12 && !lit.includes("${") && !lit.startsWith("http")) {
        ...
        if (!literalFiles.get(lit).includes(rel)) {
          literalFiles.get(lit).push(rel);
        }
      }
    }
  }
...
function findSimilarFunctionBlocks(files) {
  ...
  const funcRegex =
    /(?:private|public|protected)?\s*(?:async\s+)?(\w+)\s*\([^)]*\)\s*(?::\s*[^{]+)?\s*\{([\s\S]{1,500}?)\n\s*\}/g;
  for (const file of files) {
...
function findUnusedLikeImports(files) {
  ...
  for (const file of files) {
    ...
    for (const imp of imports) {
      ...
      if (matches.length <= 1) {
        suspicious.push({ file: rel, import: imp });
      }
    }
  }
