...
function cleanupStyles(htmlContent) {
    ...
    updated = updated.replace(/\.sidebar\s*\{[^}]*background:\s*var\(--white\);[^}]*\}/gs, '');
    updated = updated.replace(/\.sidebar\s*\{[^}]*background:\s*var\(--dark-card-bg\);[^}]*\}/gs, '');
    ...
    updated = updated.replace(/\.nav-item\s*\{[^}]*display:\s*flex;[^}]*\}/gs, '');
    updated = updated.replace(/\.nav-item:hover\s*\{[^}]*\}/gs, '');
    updated = updated.replace(/\.nav-item\.active\s*\{[^}]*\}/gs, '');
    
    // Remove duplicate sidebar comments
    updated = updated.replace(/<!-- Sidebar -->\s*<!-- Sidebar - Icon Only -->/g, '<!-- Sidebar - Icon Only -->');
    
    // Ensure top-bar uses fixed position (from dark-theme.css)
    updated = updated.replace(/\.top-bar\s*\{[^}]*display:\s*flex;[^}]*margin-bottom:[^}]*\}/gs, '');
