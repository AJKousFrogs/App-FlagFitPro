...
const isMain =
  process.argv[1]?.endsWith("seedCompetitionProtocolsFinal.js") ||
...
class CompetitionProtocolsSeeder {
  ...
  async seedEuropeanChampionshipProtocols() {
    ...
    const protocols = [
      {
        ...
        pre_tournament_hydration_protocol:
          "Begin hydration protocol 7 days before tournament with 20ml/kg/day baseline",
        daily_hydration_targets_ml_per_kg: 18.0,
        between_games_hydration_strategy:
          "150ml every 20 minutes between games, electrolyte replacement for games >2 hours apart",
        recovery_hydration_protocol:
          "Post-game rehydration: 150% of weight loss within 2 hours, continue for 24 hours",
        hydration_status_checks_per_day: 4,
        body_weight_monitoring_frequency: "pre_post_each_game",
        cognitive_testing_schedule: [
          "reaction_time",
          "decision_making",
          "memory_recall",
        ],
      ...
      },
      {
        ...
        pre_tournament_hydration_protocol:
          "Enhanced hydration protocol 10 days before with climate adaptation training",
        daily_hydration_targets_ml_per_kg: 20.0,
        between_games_hydration_strategy:
          "200ml every 15 minutes between games, increased sodium for high humidity conditions",
        recovery_hydration_protocol:
          "Advanced recovery: 160% of weight loss within 1.5 hours, protein-electrolyte mix",
    ...
    for (const protocol of protocols) {
      try {
        ...
        const checkQuery =
          "SELECT id FROM european_championship_protocols WHERE championship_year = $1";
        ...
        if (checkResult.rows.length > 0) {
          ...
          const values = [
            protocol.championship_year,
            protocol.host_country,
            protocol.climate_zone,
            protocol.teams_participating,
            protocol.games_per_team,
            protocol.tournament_duration_days,
            protocol.average_temperature_celsius,
            protocol.average_humidity_percentage,
            protocol.altitude_variations_meters,
            protocol.pre_tournament_hydration_protocol,
            protocol.daily_hydration_targets_ml_per_kg,
            protocol.between_games_hydration_strategy,
            protocol.recovery_hydration_protocol,
            protocol.hydration_status_checks_per_day,
            protocol.body_weight_monitoring_frequency,
            protocol.cognitive_testing_schedule,
            protocol.hydration_related_injuries,
            protocol.performance_consistency_score,
            protocol.athlete_feedback_score,
          ];

          await this.pool.query(updateQuery, values);
          console.log(
            `✅ Updated European Championship ${protocol.championship_year} protocol`,
          );
        } else {
          // Insert new record
          const insertQuery = `
          ...
          console.log(
            `✅ Inserted European Championship ${protocol.championship_year} protocol`,
          );
        }
      } catch (error) {
        console.error(
  ...
  async seedWorldChampionshipProtocols() {
    ...
    const protocols = [
      {
        ...
        qualification_process:
          "Regional qualifiers + wildcard entries based on world rankings",
      ...
      },
      {
        ...
        qualification_process:
          "Expanded regional qualifiers + performance-based wildcards",
        tournament_format: "Enhanced group stage + double elimination",
        climate_variations_across_venues: [
          "temperate",
          "cool",
          "moderate_humidity",
        ],
    ...
    for (const protocol of protocols) {
      try {
        // First check if record exists
        const checkQuery =
          "SELECT id FROM world_championship_protocols WHERE championship_year = $1";
        const checkResult = await this.pool.query(checkQuery, [
          protocol.championship_year,
        ]);
        ...
        if (checkResult.rows.length > 0) {
          ...
          const values = [
            ...
            protocol.climate_zone,
            protocol.teams_participating,
            protocol.qualification_process,
            protocol.tournament_format,
            protocol.climate_variations_across_venues,
            protocol.altitude_challenges,
            protocol.travel_impact_on_hydration,
            protocol.personalized_hydration_plans,
            protocol.real_time_hydration_monitoring,
            protocol.emergency_hydration_protocols,
            protocol.data_collection_protocols,
            protocol.performance_correlation_studies,
            protocol.long_term_follow_up_studies,
          ];

          await this.pool.query(updateQuery, values);
          console.log(
            `✅ Updated World Championship ${protocol.championship_year} protocol`,
          );
        } else {
          // Insert new record
          const insertQuery = `
          ...
          await this.pool.query(insertQuery, values);
          console.log(
            `✅ Inserted World Championship ${protocol.championship_year} protocol`,
          );
        }
      } catch (error) {
        console.error(
  ...
  async seedOlympicGamesProtocols() {
    ...
    for (const protocol of protocols) {
      try {
        // First check if record exists
        const checkQuery =
          "SELECT id FROM olympic_games_protocols WHERE olympic_year = $1";
        const checkResult = await this.pool.query(checkQuery, [
        ...
        if (checkResult.rows.length > 0) {
          ...
          const values = [
            protocol.olympic_year,
            protocol.host_city,
            protocol.flag_football_status,
            protocol.anti_doping_compliance,
            protocol.international_standards_application,
            protocol.cultural_dietary_considerations,
            protocol.wearable_technology_integration,
            protocol.real_time_biometric_monitoring,
            protocol.ai_powered_hydration_optimization,
            protocol.research_collaborations,
            protocol.knowledge_transfer_programs,
            protocol.long_term_impact_studies,
          ];

          await this.pool.query(updateQuery, values);
          console.log(
            `✅ Updated Olympic Games ${protocol.olympic_year} protocol`,
          );
        } else {
          // Insert new record
          const insertQuery = `
          ...
          await this.pool.query(insertQuery, values);
          console.log(
            `✅ Inserted Olympic Games ${protocol.olympic_year} protocol`,
          );
        }
      } catch (error) {
        console.error(
  ...
  async runAllSeeders() {
    console.log(
      "🚀 Starting Competition Protocols seeding (Final Version)...\n",
    );
...
export default CompetitionProtocolsSeeder;
