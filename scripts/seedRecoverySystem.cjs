    ...
    const recoveryProtocols = [
      {
        ...
        protocol_description:
          "Comprehensive recovery protocol for post-game recovery",
      ...
      },
      {
        ...
        protocol_description:
          "Specialized recovery protocol for injured athletes",
    ...
    for (const protocol of recoveryProtocols) {
      try {
        ...
        await this.pool.query(query, [
          ...
          protocol.target_athlete_type,
          protocol.recovery_type,
          protocol.duration_minutes,
          protocol.intensity_level,
          JSON.stringify(protocol.recovery_phases),
    ...
    const recoveryEquipment = [
      {
        ...
        usage_instructions:
          "Roll slowly over tight muscles for 1-2 minutes per area",
    ...
    for (const equipment of recoveryEquipment) {
      try {
        ...
        await this.pool.query(query, [
          ...
          equipment.target_muscle_groups,
          equipment.usage_instructions,
          equipment.maintenance_requirements,
          equipment.cost_range,
          equipment.durability_months,
          equipment.flag_football_relevance,
          equipment.created_at,
        ]);

        console.log(
          `✅ Inserted recovery equipment: ${equipment.equipment_name}`,
        );
      } catch (error) {
        console.error(
    ...
    for (const session of recoverySessions) {
      try {
        ...
        await this.pool.query(query, [
          ...
          session.perceived_recovery_score,
          session.muscle_soreness_level,
          session.fatigue_level,
          session.sleep_quality_previous_night,
          session.stress_level,
          session.hydration_status,
          session.nutrition_status,
          session.equipment_used,
          session.notes,
          session.created_at,
        ]);

        console.log(
          `✅ Inserted recovery session for user ID: ${session.user_id}`,
        );
      } catch (error) {
  ...
  async seedRecoveryRecommendations() {
    ...
    const recoveryRecommendations = [
      ...
      },
      {
        ...
        priority_level: "high",
        recommendation_text:
          "Rehydrate with 150% of fluid loss within 2 hours post-exercise",
        implementation_steps: [
      ...
      },
      {
        ...
        priority_level: "medium",
        recommendation_text:
          "Include light activity on rest days to promote blood flow",
        implementation_steps: [
    ...
    for (const recommendation of recoveryRecommendations) {
      try {
        ...
        await this.pool.query(query, [
          ...
          recommendation.target_athlete_type,
        ...
        console.log(
          `✅ Inserted recovery recommendation: ${recommendation.recommendation_type}`,
        );
      } catch (error) {
        console.error(
    ...
    for (const analytics of recoveryAnalytics) {
      try {
        ...
        await this.pool.query(query, [
          ...
          analytics.average_recovery_score,
          analytics.recovery_consistency,
          analytics.sleep_quality_trend,
          analytics.stress_management_score,
          analytics.hydration_compliance,
          analytics.nutrition_compliance,
          analytics.equipment_utilization,
          analytics.recommendations_followed,
          analytics.overall_recovery_grade,
          analytics.areas_for_improvement,
          analytics.next_week_goals,
          analytics.created_at,
        ]);

        console.log(
          `✅ Inserted recovery analytics for user ID: ${analytics.user_id}`,
        );
      } catch (error) {
        console.error(
    ...
    for (const profile of recoveryProfiles) {
      try {
        ...
        await this.pool.query(query, [
          ...
          profile.stress_triggers,
          profile.recovery_goals,
          profile.preferred_recovery_times,
          profile.recovery_environment,
          profile.recovery_equipment_owned,
          profile.recovery_equipment_desired,
          profile.recovery_budget_monthly,
          profile.created_at,
        ]);

        console.log(
          `✅ Inserted recovery profile for athlete ID: ${profile.athlete_id}`,
        );
      } catch (error) {
