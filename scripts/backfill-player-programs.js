 ...
 *
 * Usage:
 *   node scripts/backfill-player-programs.js [--dry-run]
 *
 * Options:
...
function getProgramIdForUser(user, config) {
  ...
  if (user.profile_metadata?.position) {
    ...
    if (programId) {
      console.log(
        `  → Using position from profile metadata: ${user.profile_metadata.position}`,
      );
      return programId;
    }
  }
...
async function main() {
  ...
  const usersWithoutPrograms = allUsers.filter(
    (user) => !usersWithPrograms.has(user.id),
  );

  console.log(
    `📋 Found ${usersWithoutPrograms.length} users WITHOUT active programs\n`,
  );
  ...
  for (const assignment of assignmentsToCreate) {
    const { error } = await supabase.from("player_programs").insert(assignment);

    if (error) {
      console.error(
        `❌ Error creating assignment for ${assignment.player_id}:`,
        error.message,
      );
      errorCount++;
    } else {
