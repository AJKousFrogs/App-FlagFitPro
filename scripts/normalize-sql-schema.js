...
const inputFile = path.join(__dirname, '../database/migration_results/all_migrations_consolidated.sql');
const outputFile = path.join(__dirname, '../database/migration_results/all_migrations_consolidated_normalized.sql');
...
normalized = normalized.replace(
  ...
  (match, offset, string) => {
    ...
    const context = string.substring(Math.max(0, offset - 30), offset + match.length);
    if (context.includes('TRIGGER') || context.includes('CREATE TRIGGER') || context.includes('DROP TRIGGER')) {
      const before = string.substring(Math.max(0, offset - 20), offset);
      if (!before.includes('auth.users') && !before.includes('public.users')) {
...
normalized = normalized.replace(
  ...
  (match, offset, string) => {
    ...
    const context = string.substring(Math.max(0, offset - 50), offset + match.length);
    if (context.includes('CREATE FUNCTION') || context.includes('CREATE OR REPLACE FUNCTION')) {
...
normalized = normalized.replace(
  ...
  (match) => {
    return match.replace(/\bON\s+training_sessions\b/gi, 'ON public.training_sessions');
  }
);
...
console.log(`   Normalized size: ${(normalized.length / 1024 / 1024).toFixed(2)} MB`);
...
const normalizedPublicUsersRefs = (normalized.match(/\bpublic\.users\b/gi) || []).length;

console.log('\n📊 Statistics:');
console.log(`   Original 'users' references: ${originalUsersRefs}`);
console.log(`   Normalized 'users' references: ${normalizedUsersRefs}`);
console.log(`   Original 'public.users' references: ${originalPublicUsersRefs}`);
console.log(`   Normalized 'public.users' references: ${normalizedPublicUsersRefs}`);
