...
function applyUnifiedTheme(htmlContent, filePath) {
    ...
    if (['login.html', 'register.html', 'reset-password.html', 'index.html'].includes(fileName)) {
        return updated;
    }
    ...
    const oldSidebarPattern = /<div class="sidebar">[\s\S]*?<\/div>\s*<\/div>/;
    ...
    if (updated.includes('sidebar-header') || updated.includes('nav-section')) {
        ...
        const sidebarMatch = updated.match(/<div class="sidebar">[\s\S]*?(?=<main|<div class="main-content"|<\/body>)/);
        if (sidebarMatch) {
...
htmlFiles.forEach(file => {
    try {
        ...
        if (content !== updated) {
            ...
            console.log(`⏭️  Skipped: ${path.relative(path.join(__dirname, '..'), file)} (no changes needed)`);
        }
    } catch (error) {
