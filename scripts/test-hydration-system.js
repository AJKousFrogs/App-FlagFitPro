...
async function testHydrationSystem() {
  ...
  try {
    ...
    const wadaQuery =
      "SELECT COUNT(*) as count FROM wada_prohibited_substances";
    const wadaResult = await pool.query(wadaQuery);
    console.log(
      `✅ Found ${wadaResult.rows[0].count} WADA prohibited substances`,
    );

    // Test 3: Check hydration research
    console.log("\n3️⃣ Checking hydration research data...");
    const researchQuery =
      "SELECT COUNT(*) as count FROM hydration_research_studies";
    const researchResult = await pool.query(researchQuery);
    console.log(
      `✅ Found ${researchResult.rows[0].count} hydration research studies`,
    );
    ...
    console.log(
      `✅ Found ${ifafResult.rows[0].count} IFAF hydration protocols`,
    );
    ...
    try {
      const viewQuery =
        "SELECT COUNT(*) as count FROM flag_football_hydration_recommendations";
