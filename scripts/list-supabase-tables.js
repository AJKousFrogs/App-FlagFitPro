...
if (!supabaseKey) {
  ...
  console.error('Please set SUPABASE_SERVICE_KEY or SUPABASE_ANON_KEY environment variable');
  process.exit(1);
}
...
async function listAllTables() {
  ...
  if (errorTables.length > 0) {
    console.log(`⚠️  Errors: ${errorTables.length} tables (may exist but have RLS restrictions)`);
  }
  ...
  if (existingTables.length > 0) {
    ...
    existingTables.forEach((table, index) => {
      ...
      console.log(`${status} ${(index + 1).toString().padStart(3)}. ${table.name}`);
      if (table.error) {
