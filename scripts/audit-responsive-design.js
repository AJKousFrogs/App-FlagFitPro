...
function getAllFiles(dir, extension) {
    ...
    list.forEach(file => {
        ...
        } else if (file.endsWith(extension) && !file.includes('design-system-example')) {
...
function checkViewportMeta(htmlContent, filePath) {
    const hasViewport = htmlContent.includes('viewport') || htmlContent.includes('meta name="viewport"');
...
function checkMediaQueries(cssContent) {
    ...
    matches.forEach(match => {
        ...
        } else if (match.includes('max-width: 768') || match.includes('max-width: 767')) {
            breakpoints.mobile.push(match);
        } else if (match.includes('max-width: 1024') || match.includes('max-width: 1023')) {
            breakpoints.tablet.push(match);
        } else if (match.includes('min-width: 1025') || match.includes('min-width: 1024')) {
...
function checkTouchTargets(htmlContent, cssContent) {
    ...
    buttonMatches.forEach(match => {
        ...
        if (heightMatch) {
            ...
            if (height < 44) {
                issues.push(`Button height ${height}px is below 44px minimum touch target`);
            }
        }
    });
    
    // Check for small clickable areas
    const smallClickablePattern = /(?:width|height):\s*([0-9]+)px[^}]*cursor:\s*pointer/gi;
    const clickableMatches = cssContent.match(smallClickablePattern) || [];
...
function checkSidebarMobile(cssContent) {
    ...
    const hasMobileSidebar = cssContent.includes('sidebar') && 
                            (cssContent.includes('@media') || cssContent.includes('transform: translateX'));
...
function checkFormsMobile(cssContent) {
    ...
    const hasInputMobile = cssContent.includes('input') && cssContent.includes('@media');
    const hasFontSize = cssContent.includes('font-size: 16px') || cssContent.includes('font-size: 1rem');
...
function auditFile(filePath, isHTML) {
    ...
    if (isHTML) {
        ...
        if (content.includes('<img') && !content.includes('srcset') && !content.includes('sizes')) {
            audit.warnings.push('Images may not be responsive (missing srcset/sizes)');
        }
        
        // Check for fixed widths
        if (content.includes('width:') && content.includes('px') && !content.includes('max-width')) {
            audit.warnings.push('May contain fixed widths (check for responsive issues)');
        }
    } else {
...
console.log(`Scanning ${htmlFiles.length} HTML files and ${cssFiles.length} CSS files...\n`);
...
results.css.forEach(audit => {
    if (audit.issues.length > 0 || audit.warnings.length > 0 || audit.stats.mediaQueries) {
        console.log(`\n${audit.file}`);
        if (audit.stats.mediaQueries) {
            console.log(`   📊 Media Queries: ${audit.stats.mediaQueries.total} total`);
            console.log(`      - Mobile: ${audit.stats.mediaQueries.breakpoints.mobile.length}`);
            console.log(`      - Tablet: ${audit.stats.mediaQueries.breakpoints.tablet.length}`);
            console.log(`      - Desktop: ${audit.stats.mediaQueries.breakpoints.desktop.length}`);
        }
        if (audit.issues.length > 0) {
            console.log(`   ❌ Issues (${audit.issues.length}):`);
            audit.issues.forEach(issue => console.log(`      - ${issue}`));
...
// Generate report
const reportPath = path.join(__dirname, '..', 'RESPONSIVE_DESIGN_AUDIT_REPORT.md');
...
### Common Devices
${Object.entries(deviceBreakpoints).map(([device, size]) => `- **${device}**: ${size.width}×${size.height}px`).join('\n')}

## Detailed Results

### HTML Files
${results.html.map(audit => {
    if (audit.issues.length === 0 && audit.warnings.length === 0) {
        return `### ✅ ${audit.file}\n- No issues found\n`;
    }
...
### CSS Files
${results.css.map(audit => {
    if (audit.issues.length === 0 && audit.warnings.length === 0 && (!audit.stats.mediaQueries || audit.stats.mediaQueries.total === 0)) {
        return `### ✅ ${audit.file}\n- No issues found\n`;
    }
