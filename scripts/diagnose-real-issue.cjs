...
async function diagnose() {
  ...
  try {
    ...
    if (usersError) {
      ...
      // Alternative: Check public.users table
      ...
      var userId = publicUsers.id;
      console.log(`✅ Found user: ${publicUsers.email}`);
      console.log(`   User ID: ${userId}`);
      console.log(`   Name: ${publicUsers.full_name || 'N/A'}`);
      console.log(`   Position (users table): ${publicUsers.position || 'N/A'}`);
      console.log(`   Jersey (users table): ${publicUsers.jersey_number || 'N/A'}\n`);
    } else {
    ...
    if (!allTeams || allTeams.length === 0) {
      ...
      console.log('   This is the problem - no teams exist for you to select.\n');
    } else {
      ...
      const hasDomestic = allTeams.find(t => t.name?.toLowerCase().includes('domestic'));
      const hasInternational = allTeams.find(t => t.name?.toLowerCase().includes('international'));
    ...
    if (!membership) {
      ...
      console.log('   This is why the team dropdown might be empty or not saving.\n');
    } else {
    ...
    if (!allTeams || allTeams.length === 0) {
      ...
      console.log('   2. Or run: node scripts/create-ljubljana-teams.cjs\n');
    } else if (!membership) {
