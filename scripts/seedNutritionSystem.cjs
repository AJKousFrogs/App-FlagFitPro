    ...
    for (const plan of nutritionPlans) {
      try {
        ...
        await this.pool.query(query, [
          ...
          JSON.stringify(plan.meal_timing),
          plan.is_active,
          plan.created_at,
        ]);
    ...
    for (const profile of nutritionProfiles) {
      try {
        ...
        await this.pool.query(query, [
          ...
          profile.competition_level,
          profile.dietary_restrictions,
          profile.food_preferences,
          profile.allergies,
          profile.supplement_use,
          profile.hydration_goals_ml_per_day,
          profile.protein_goals_g_per_day,
          profile.carb_goals_g_per_day,
          profile.fat_goals_g_per_day,
          profile.created_at,
        ]);

        console.log(
          `✅ Inserted nutrition profile for athlete ID: ${profile.athlete_id}`,
        );
      } catch (error) {
    ...
    const recommendations = [
      {
        ...
        rationale:
          "Provides sustained energy release and prevents hunger during competition",
        evidence_level: "high",
        created_at: new Date(),
      },
      {
        ...
        rationale:
          "Maintains blood glucose and electrolyte balance during intense activity",
        evidence_level: "high",
        created_at: new Date(),
      },
      {
      ...
      },
      {
        ...
        rationale:
          "Provides adequate fuel for training session without causing discomfort",
        evidence_level: "high",
        created_at: new Date(),
      },
    ];
    ...
    for (const recommendation of recommendations) {
      try {
        ...
        await this.pool.query(query, [
          ...
          recommendation.hydration_ml,
          recommendation.food_suggestions,
        ...
        console.log(
          `✅ Inserted nutrition recommendation: ${recommendation.recommendation_type}`,
        );
      } catch (error) {
        console.error(
    ...
    for (const correlation of correlations) {
      try {
        ...
        await this.pool.query(query, [
          ...
          correlation.study_duration_weeks,
          correlation.athlete_type,
          correlation.nutrition_factor,
          correlation.performance_improvement_percentage,
        ...
        console.log(
          `✅ Inserted performance correlation: ${correlation.correlation_type}`,
        );
      } catch (error) {
        console.error(
    ...
    for (const target of nutritionTargets) {
      try {
        ...
        await this.pool.query(query, [
          ...
          target.daily_carbs_g,
          target.daily_fat_g,
          target.daily_hydration_ml,
          target.meal_frequency,
          target.pre_workout_calories,
          target.post_workout_calories,
          target.is_active,
          target.created_at,
        ]);

        console.log(
          `✅ Inserted nutrition targets for user ID: ${target.user_id}`,
        );
      } catch (error) {
