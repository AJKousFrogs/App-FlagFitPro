...
async function updateSecurityPatches() {
  ...
  exec('npm update netlify-cli @supabase/supabase-js jsonwebtoken nodemailer chart.js');
...
async function checkOutdated() {
  ...
  try {
    ...
    log('ℹ️  Some packages are outdated. See UPDATE_REPORT.md for details.', 'yellow');
  }
}
