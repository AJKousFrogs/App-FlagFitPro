...
class TestAutomation {
  ...
  async cleanTestArtifacts() {
    ...
    for (const dir of artifactDirs) {
      try {
        await fs.rm(path.join(projectRoot, dir), { recursive: true, force: true })
      } catch (error) {
  ...
  async runLinting() {
    ...
    try {
      // ESLint
      try {
        await this.execCommand('npx eslint . --ext .js,.jsx,.ts,.tsx --format json --output-file eslint-report.json')
        lintResults.eslint.passed = true
      } catch (error) {
        const report = await this.readJsonFile('eslint-report.json')
        lintResults.eslint.errors = report?.filter(r => r.errorCount > 0) || []
        lintResults.eslint.warnings = report?.filter(r => r.warningCount > 0) || []
      }
      ...
      try {
        ...
        lintResults.prettier.issues = error.stdout?.split('\n').filter(Boolean) || []
      }
      ...
      try {
        ...
        lintResults.typescript.errors = error.stdout?.split('\n').filter(Boolean) || []
      }
      
      this.testResults.details.linting = lintResults
      
      const allPassed = Object.values(lintResults).every(result => result.passed)
      console.log(allPassed ? '✅ Code quality checks passed' : '⚠️ Code quality issues found')
  ...
  async runUnitTests() {
    ...
    try {
      const result = await this.execCommand('npm run test:unit -- --coverage --reporter=json --outputFile=unit-test-results.json')
      
      const testReport = await this.readJsonFile('unit-test-results.json')
      const coverageReport = await this.readJsonFile('coverage/coverage-summary.json')
      ...
      this.testResults.details.unit = {
        ...
        duration: testReport?.testResults?.reduce((sum, t) => sum + t.perfStats.runtime, 0) || 0
      }
      ...
      const coverageMet = this.testResults.coverage.unit.lines >= this.config.coverageThreshold
      console.log(`✅ Unit tests completed. Coverage: ${this.testResults.coverage.unit.lines}% ${coverageMet ? '✅' : '⚠️'}`)
  ...
  async runIntegrationTests() {
    ...
    try {
      const result = await this.execCommand('npm run test:integration -- --reporter=json --outputFile=integration-test-results.json')
      
      const testReport = await this.readJsonFile('integration-test-results.json')
      ...
      console.log(`✅ Integration tests completed: ${this.testResults.details.integration.passed}/${this.testResults.details.integration.total} passed`)
      
    } catch (error) {
      console.error('❌ Integration tests failed:', error.message)
      this.testResults.failures.push({ suite: 'integration', error: error.message })
      throw error
    }
  }
  ...
  async runE2ETests() {
    ...
    try {
      ...
      await this.execCommand('npx playwright install --with-deps chromium firefox webkit')
      
      // Run E2E tests
      const result = await this.execCommand('npx playwright test --reporter=json --output-dir=e2e-test-results')
      
      const testReport = await this.readJsonFile('e2e-test-results/report.json')
      ...
      console.log(`✅ E2E tests completed: ${this.testResults.details.e2e.passed}/${this.testResults.details.e2e.total} passed`)
  ...
  async runPerformanceTests() {
    ...
    try {
      const result = await this.execCommand('npm run test:performance -- --output-dir=performance-reports')
      
      const performanceReport = await this.readJsonFile('performance-reports/performance-summary.json')
      ...
      const performancePassed = 
        this.testResults.performance.avgResponseTime <= this.config.performanceThreshold.avgResponseTime &&
        this.testResults.performance.errorRate <= this.config.performanceThreshold.errorRate &&
        this.testResults.performance.memoryUsage <= this.config.performanceThreshold.memoryUsage
      
      console.log(`✅ Performance tests completed ${performancePassed ? '✅' : '⚠️'}`)
      
    } catch (error) {
      console.error('❌ Performance tests failed:', error.message)
      this.testResults.failures.push({ suite: 'performance', error: error.message })
  ...
  async runTestsInParallel(testSuites) {
    ...
    if (failures.length > 0) {
      ...
      throw new Error(`Parallel test execution failed: ${failures.map(f => f.suite).join(', ')}`)
    }
  }
  ...
  async generateTestReport() {
    ...
    this.testResults.summary = {
      totalTests: Object.values(this.testResults.details).reduce((sum, suite) => sum + (suite.total || 0), 0),
      totalPassed: Object.values(this.testResults.details).reduce((sum, suite) => sum + (suite.passed || 0), 0),
      totalFailed: Object.values(this.testResults.details).reduce((sum, suite) => sum + (suite.failed || 0), 0),
      totalDuration: Object.values(this.testResults.details).reduce((sum, suite) => sum + (suite.duration || 0), 0),
  ...
  calculateOverallCoverage() {
    ...
    return Math.round(
      (coverage.unit.lines + coverage.unit.functions + coverage.unit.branches + coverage.unit.statements) / 4
    )
  }
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <table>
                ...
                <tbody>
                    ${Object.entries(this.testResults.details).map(([suite, data]) => `
                        <tr>
        ...
        <div class="section">
            ...
            <div style="background: #f8d7da; border: 1px solid #f5c6cb; border-radius: 4px; padding: 15px;">
                ${this.testResults.failures.map(failure => `
    ...
    await fs.writeFile(path.join(projectRoot, 'test-results', 'report.html'), html)
  }
...
Generated on ${new Date(this.testResults.timestamp).toLocaleString()}
    ...
    await fs.writeFile(path.join(projectRoot, 'test-results', 'summary.md'), summary.trim())
  }
  ...
  async cleanup() {
    ...
    const tempFiles = ['eslint-report.json', 'unit-test-results.json', 'integration-test-results.json']
  ...
  async execCommand(command, options = {}) {
    const { timeout = this.config.timeoutMs, env = {}, ignoreError = false } = options
