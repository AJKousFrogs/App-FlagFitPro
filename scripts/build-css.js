...
class CSSBuilder {
    ...
    Object.entries(this.bundleMap).forEach(([bundleName, patterns]) => {
      ...
      patterns.forEach((pattern) => {
        if (pattern.includes("**")) {
          ...
          const matchingFile = this.cssFiles.find(
            (file) =>
      ...
      bundleStats[bundleName] = {
        ...
        compression: (
          ((totalSize - minifiedCSS.length) / totalSize) *
          100
        ).toFixed(1),
      };

      console.log(
        `✅ Created ${bundleName}: ${bundleFiles.length} files, ${(minifiedCSS.length / 1024).toFixed(1)}KB`,
      );
    });
  ...
  createCriticalCSS() {
    ...
    console.log(
      `✅ Created critical.css: ${(minifiedCriticalCSS.length / 1024).toFixed(1)}KB`,
    );
...
`;
    ...
    fs.writeFileSync(
      path.join(templateDir, "optimized-css-loading.html"),
      cssLoadingTemplate,
    );
  ...
  updateHTMLFiles() {
    ...
    htmlFiles.forEach((filePath) => {
      try {
        ...
        if (
          content.includes("./dist/css/main-bundle.css") &&
          !content.includes("./dist/css/components-bundle.css")
        ) {
          content = content.replace(
            '<link rel="stylesheet" href="./dist/css/main-bundle.css">',
  ...
  generateReport(bundleStats, criticalCSSSize) {
    const totalOriginalSize = Object.values(bundleStats).reduce(
      (sum, bundle) => sum + bundle.originalSize,
      0,
    );
    const totalBundledSize = Object.values(bundleStats).reduce(
      (sum, bundle) => sum + bundle.bundledSize,
      0,
    );
    ...
    console.log(
      `   Original size: ${(report.summary.originalSize / 1024).toFixed(1)}KB`,
    );
    console.log(
      `   Bundled size: ${(report.summary.bundledSize / 1024).toFixed(1)}KB`,
    );
    console.log(
      `   Size saved: ${report.summary.sizeSaved} (${report.summary.compressionRatio})`,
    );
