        ...
        (match, iconName, attrs) => {
            ...
            if (attrs.includes('style=')) {
                ...
                if (attrs.includes('color:')) {
                    attrs = attrs.replace(/color:\s*[^;]+;/g, 'color: var(--icon-color-primary);');
                } else {
        ...
        (match, iconName, attrs) => {
            if (attrs.includes('style=')) {
                attrs = attrs.replace(/style="([^"]*)"/, (m, styles) => {
                    ...
                    let newStyles = styles.replace(/color:\s*[^;]+;/g, '').replace(/stroke:\s*[^;]+;/g, '');
                    return `style="${newStyles}; color: var(--icon-color-muted); stroke: var(--icon-color-muted);"`;
                });
            } else {
                attrs += ' style="color: var(--icon-color-muted); stroke: var(--icon-color-muted);"';
            }
            return match.replace(/<i data-lucide="[^"]+"([^>]*)>/, `<i data-lucide="${iconName}"${attrs}>`);
        }
    );
    ...
    updated = updated.replace(
        ...
        (match, iconName, attrs) => {
            if (attrs.includes('style=')) {
                ...
                attrs += ' style="color: var(--icon-color-secondary); stroke: var(--icon-color-secondary);"';
            }
            return match.replace(/<i data-lucide="[^"]+"([^>]*)>/, `<i data-lucide="${iconName}"${attrs}>`);
        }
    );
...
htmlFiles.forEach(file => {
    try {
        ...
        if (content !== updated) {
            ...
            console.log(`✅ Updated icons: ${path.relative(path.join(__dirname, '..'), file)}`);
        }
    } catch (error) {
