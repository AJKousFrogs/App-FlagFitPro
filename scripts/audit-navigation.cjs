...
function checkFile(filePath) {
    ...
    const hasSidebar = /class=["']sidebar["']/.test(content) || /<aside.*sidebar/.test(content);
    const hasDashboardContainer = /class=["']dashboard-container["']/.test(content);
...
htmlFiles.forEach(file => {
    ...
    if (!result.hasSidebar && !['login.html', 'register.html', 'index.html'].includes(file)) {
        problems.push('MISSING SIDEBAR');
        issues.missingSidebar.push(file);
    }
    
    if (!result.hasDashboardContainer && !['login.html', 'register.html', 'index.html'].includes(file)) {
    ...
    if (result.hasSidebar) {
        if (result.hasNavSection && result.hasNavItem) {
            ...
            // Icon-only sidebar
            if (file === 'dashboard.html') {
                problems.push('INCONSISTENT: Uses icon-only sidebar (should be dashboard-style)');
                issues.inconsistentStructure.push({ file, type: 'icon-only' });
            }
        }
    }
    
    if (!result.hasMainContent && !['login.html', 'register.html', 'index.html'].includes(file)) {
...
const report = {
    ...
    filesWithIssues: issues.missingSidebar.length + issues.inconsistentStructure.length + issues.missingDashboardContainer.length + issues.layoutIssues.length
};
