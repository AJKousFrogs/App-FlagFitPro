...
class HealthChecker {
  ...
  async runComprehensiveCheck() {
    ...
    try {
      ...
      if (this.results.warnings.length > 0) {
        ...
        this.results.warnings.slice(0, 3).forEach(warning => console.log(`  ⚠️  ${warning}`));
      }
  ...
  async checkDependencies() {
    ...
    try {
      ...
      try {
        ...
        if (auditResult.vulnerabilities) {
          ...
          if (vulnCount > 0) {
            ...
            if (vulnCount > 20) {
              this.results.criticalIssues.push('Too many security vulnerabilities (>20)');
            }
          }
        }
      } catch (auditError) {
      ...
      deps.status = deps.score > 70 ? 'good' : deps.score > 40 ? 'warning' : 'critical';
  ...
  async checkTests() {
    ...
    try {
      ...
      // Try to run tests
      try {
        const { stdout, stderr } = await execAsync('npm run test:unit', { timeout: 60000 });
        ...
        if (total > 0) {
          ...
          if (passRate < 50) {
            this.results.criticalIssues.push(`Low test pass rate: ${passRate.toFixed(1)}%`);
          } else if (passRate < 80) {
            this.results.warnings.push(`Test pass rate needs improvement: ${passRate.toFixed(1)}%`);
          }
        } else {
      ...
      tests.status = tests.score > 70 ? 'good' : tests.score > 40 ? 'warning' : 'critical';
    ...
    try {
      ...
      for (const file of sensitiveFiles) {
        try {
          ...
          sensitiveFound++;
          this.results.warnings.push(`Sensitive file found: ${file} (ensure it's in .gitignore)`);
        } catch {
      ...
      for (const file of productionFiles) {
        try {
          ...
          if (content.includes('demo-token') || content.includes('console.log') || content.includes('debug')) {
            debugCodeFound++;
      ...
      if (debugCodeFound > 0) {
        ...
        this.results.warnings.push(`Debug code found in ${debugCodeFound} production files`);
      } else {
      ...
      security.status = security.score > 70 ? 'good' : security.score > 40 ? 'warning' : 'critical';
  ...
  async checkCodeQuality() {
    ...
    try {
      ...
      codeQuality.status = codeQuality.score > 70 ? 'good' : codeQuality.score > 40 ? 'warning' : 'critical';
  ...
  async checkConfiguration() {
    ...
    try {
      ...
      config.status = config.score > 70 ? 'good' : config.score > 40 ? 'warning' : 'critical';
    ...
    try {
      ...
      try {
        ...
        if (nodeModulesSize > 500 * 1024 * 1024) { // 500MB
          performance.issues.push('Large node_modules size may affect build performance');
      ...
      performance.status = performance.score > 70 ? 'good' : performance.score > 40 ? 'warning' : 'critical';
  ...
  async checkDocumentation() {
    ...
    try {
      ...
      try {
        ...
        const requiredSections = ['installation', 'usage', 'features', 'development'];
        let foundSections = 0;
      ...
      docs.status = docs.score > 70 ? 'good' : docs.score > 40 ? 'warning' : 'critical';
  ...
  calculateOverallHealth() {
    ...
    if (this.results.overallHealth < 50) {
      this.results.recommendations.push('🚨 Critical: Not ready for production deployment');
      this.results.recommendations.push('Focus on fixing critical issues first');
    } else if (this.results.overallHealth < 70) {
      this.results.recommendations.push('⚠️  Warning: Needs improvement before production');
