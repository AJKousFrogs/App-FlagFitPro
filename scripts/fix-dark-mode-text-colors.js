...
function fixDarkTextColors(content) {
    ...
    Object.entries(darkTextColors).forEach(([darkColor, lightColor]) => {
        ...
        const colorRegex = new RegExp(`color:\\s*${darkColor.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'gi');
        updated = updated.replace(colorRegex, `color: ${lightColor}`);
        
        // Replace in style attributes
        const styleRegex = new RegExp(`style="([^"]*?)color:\\s*${darkColor.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}([^"]*?)"`, 'gi');
    ...
    updated = updated.replace(
        ...
        (match, before, color, after) => {
            const upperColor = `#${color.toUpperCase()}`;
            if (darkTextColors[upperColor] && !match.includes('background: var(--primary')) {
                return match.replace(`color: #${color}`, `color: var(--dark-text-primary)`);
            }
            return match;
        }
    );
