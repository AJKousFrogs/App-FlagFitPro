...
async function findJSFiles(dir, fileList = []) {
  ...
  for (const file of files) {
    ...
    } else if (extname(file) === ".js" && !file.includes(".test.") && !file.includes(".spec.")) {
  ...
  try {
    ...
    if (watch) {
      ...
      const builds = jsFiles.map(async (entryPoint) => {
        ...
        const outputPath = join(distDir, "js", relativePath.replace(".js", ".min.js"));
        ...
        console.log(`✅ Built: ${relativePath} → ${outputPath.replace(rootDir + "/", "")}`);
      });
