...
const hardcodedSpacingPattern = /(padding|margin|gap|top|bottom|left|right):\s*\d+px/gi;
const customFontPattern = /font-family:\s*[^'"]*['"](?!Inter|Roboto)[^'"]*['"]/gi;
...
function checkFile(filePath) {
    ...
    if (!content.includes('class="main-content"') && fileName !== 'workout.html') {
        issues.push('Missing main-content');
        violations.missingMainContent.push(fileName);
    }
    
    // Check for dashboard-container
    if (!content.includes('class="dashboard-container"') && fileName !== 'workout.html') {
    ...
    styleMatches.forEach(styleBlock => {
        ...
        if (colorMatches) {
            colorMatches.forEach(match => {
                ...
                if (!match.includes('var(') && !match.match(/rgba?\(0,\s*0,\s*0,\s*0\.\d+\)/)) {
