...
function fixLoggerCalls(filePath) {
  ...
  const errorUnknownPattern = /(\s+)(this\.logger\.error\([^,)]+,\s*)(\w+)(\s*\))/g;
  content = content.replace(errorUnknownPattern, (match, indent, prefix, errorVar, suffix) => {
    if (errorVar === 'err' || errorVar === 'error' || errorVar.toLowerCase().includes('error')) {
      modified = true;
  ...
  const stringContextPatterns = [
    ...
    { 
      regex: /(this\.logger\.(info|debug|warn|success)\([^,)]+,\s*)([a-zA-Z_][a-zA-Z0-9_.]*\.(message|id|name|type|code))(\s*[,)])/g,
