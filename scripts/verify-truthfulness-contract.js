 ...
 *
 * Usage:
 *   BASE_URL=http://localhost:8888 AUTH_TOKEN=your-token node scripts/verify-truthfulness-contract.js
 *
 * Requirements:
...
async function testCase1_MissingReadiness() {
  ...
  const hasGenericNote = data.data?.mainSession?.exercises?.some(
    (ex) =>
...
async function testCase2_GetReturnsTruth() {
  ...
  const { response, data } = await apiRequest(
    "GET",
    `/api/daily-protocol?date=${testDate}`,
  );
  ...
  if (!data.data) {
    console.log(
      "⚠️  No protocol exists for this date (expected after generate)",
    );
    results.tests.push({
      name: "Test Case 2",
      status: "SKIPPED",
...
async function testCase3_NoProgram() {
  ...
  const { data: checkData } = await apiRequest(
    "GET",
  ...
  if (checkData.data?.assignment) {
    console.log(
      `✓ Current user has program: ${checkData.data.assignment.program.name}`,
    );
  ...
  assertions.push(
    assert(data.success === false, "success === false", data.success),
  );
...
async function testCase4_SportLayerOverride() {
  ...
  if (override) {
    ...
    assertions.push(
      assert(typeof override === "string", "override is a string", override),
    );
    ...
    if (override === "flag_practice") {
      ...
      const hasPracticeDayNotes = mainExercises.some(
        (ex) =>
    ...
    console.log(
      "ℹ️  No override for this date (user may not have practice scheduled)",
    );
    assertions.push(
      assert(true, "No override expected (user config dependent)", null),
    );
  }
