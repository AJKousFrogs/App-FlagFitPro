...
const TOKEN_CATEGORIES = {
  color: [
    "color", "primitive", "brand", "status", "surface", "text", "border",
    "hover", "interactive", "workout", "rarity", "staff", "position",
    "phase", "chart", "metallic",
  ],
  ...
  "z-index": ["z-index", "z-"],
  sizing: [
    "button-height", "input-height", "touch-target", "avatar", "icon",
    "progress", "badge", "card-min-height",
  ],
  gradient: ["gradient"],
  focus: ["focus", "outline"],
...
function getTokenUsage(tokenName, category) {
  ...
  if (name.includes("primary-green") && !name.includes("rgb") && !name.includes("hover") && !name.includes("light") && !name.includes("subtle")) {
    return "**PRIMARY BRAND COLOR**";
  }
  if (name.includes("hover")) return "Hover state";
  if (name.includes("active")) return "Active/pressed state";
  if (name.includes("focus")) return "Focus state";
  if (name.includes("disabled")) return "Disabled state";
  if (name.includes("subtle") || name.includes("ultra-subtle")) return "Subtle backgrounds";
  if (name.includes("light")) return "Light variant";
  if (name.includes("default") || name.includes("md")) return "**Default**";
  if (name.includes("button")) return "**ALL buttons** - raised rectangular style";
  if (name.includes("full") || name.includes("9999")) return "**RESTRICTED** - Pill shape";
...
async function generateAudit() {
  ...
  for (const file of TOKEN_FILES) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      const purpose = file.includes("design-system-tokens") ? "**PRIMARY** - Single source of truth for all tokens" :
                     file.includes("_variables") ? "SCSS maps and helper functions" :
                     file.includes("token-mapping") ? "PrimeNG component token mapping" :
                     file.includes("typography") ? "Typography utility classes" :
                     file.includes("spacing") ? "Spacing utility classes" :
                     file.includes("layout") ? "Layout patterns and utilities" :
                     file.includes("hover") ? "Hover state tokens" :
                     file.includes("premium-interactions") ? "Animation tokens" :
                     file.includes("standardized-components") ? "Component styles using tokens" :
                     file.includes("_mixins") ? "SCSS mixins" :
                     "Main stylesheet";
  ...
  // Border Colors
  const borderColors = tokensByCategory.color.filter(t => t.name.includes("color-border"));
  markdown += `### ${sectionNum - 1}.10 Border Colors\n\n`;
  markdown += generateTokenTable(borderColors, "color");
  markdown += `\n`;
  
  // Status Colors
  const statusColors = tokensByCategory.color.filter(t => t.name.includes("color-status"));
  markdown += `### ${sectionNum - 1}.11 Status Colors\n\n`;
  markdown += generateTokenTable(statusColors, "color");
  markdown += `\n`;
  
  // Interactive State Colors
  const interactiveColors = tokensByCategory.color.filter(t => t.name.includes("color-interactive"));
  markdown += `### ${sectionNum - 1}.12 Interactive State Colors\n\n`;
  markdown += generateTokenTable(interactiveColors, "color");
  markdown += `\n`;
  
  // Hover State Colors
  const hoverColors = tokensByCategory.color.filter(t => t.name.includes("hover-"));
  markdown += `### ${sectionNum - 1}.13 Hover State Colors\n\n`;
  markdown += generateTokenTable(hoverColors, "color");
  markdown += `\n`;
  
  // Hover Shadows
  const hoverShadows = tokensByCategory.shadow.filter(t => t.name.includes("hover-shadow"));
  ...
  const specialColors = tokensByCategory.color.filter(t => 
  ...
  const spacingTokens = tokensByCategory.spacing.filter(t => t.name.match(/--space-[0-9]/));
  ...
  const shadowTokens = tokensByCategory.shadow.filter(t => !t.name.includes("hover-shadow"));
  ...
  const durationTokens = animationTokens.filter(t => t.name.includes("transition") && !t.name.includes("ease"));
  ...
  markdown += generateTokenTable(layoutTokens.filter(t => t.name.includes("container")), "layout");
  markdown += `\n`;
  ...
  markdown += generateTokenTable(sizingTokens.filter(t => t.name.includes("button")), "sizing");
  markdown += `\n`;
