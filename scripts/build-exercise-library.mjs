...
import { TISSUES, MOVEMENT_TO_TISSUES } from "../database/library/tissue-registry.mjs";
import { FAMILIES } from "../database/library/families.mjs";

const here = dirname(fileURLToPath(import.meta.url));
const root = resolve(here, "..");

const ENUMS = {
  contraction_type: ["isometric", "concentric", "eccentric", "isotonic", "plyometric", "stretch", "mixed"],
  joint_emphasis: ["knee_bent", "knee_straight", "neutral", "n/a"],
  loading_rate_band: ["none", "low", "moderate", "high", "very_high"],
  evidence_tier: ["META", "RCT", "COHORT", "CONSENSUS", "HEURISTIC", "CONTESTED"],
  category: ["rehab", "isometrics", "mobility", "warm_up", "cool_down", "recovery", "foam_roll", "strength", "plyometrics", "conditioning", "skill_drills", "speed", "agility", "power"],
};
  ...
  for (const fam of FAMILIES) {
    for (const v of fam.variants) {
      ...
      const row = {
        ...
        is_high_intensity: ["high", "very_high"].includes(ov.loadingRateBand ?? fam.loadingRateBand),
        ...
        difficulty_level: defaults.difficulty ?? fam.defaults.difficulty ?? "intermediate",
...
function toSql(rows) {
  const cols = [
    "name", "slug", "category", "subcategory", "description", "how_text",
    "coaching_cues", "muscle_groups", "target_muscles", "movement_pattern",
    "applicable_positions", "is_high_intensity", "load_contribution_au",
    "default_sets", "default_reps", "default_hold_seconds", "default_duration_seconds",
    "difficulty_level", "equipment_needed", "active", "tissue_targets",
    "contraction_type", "joint_emphasis", "loading_rate_band", "peak_load_bw",
    "evidence_tier", "rehab_stage",
  ];
  const values = rows
  ...
  const enrichCols = [
    "tissue_targets", "contraction_type", "joint_emphasis",
    "loading_rate_band", "peak_load_bw", "evidence_tier", "rehab_stage",
  ];
...
function stats(rows) {
  ...
  return { total: rows.length, byCategory: by("category"), byTissue: rows.flatMap((r) => r.tissue_targets).reduce((m, t) => ((m[t] = (m[t] || 0) + 1), m), {}), byEvidence: by("evidence_tier") };
}
...
if (process.argv.includes("--print-sql")) {
  ...
  writeFileSync(resolve(root, "database/seed-exercise-library.generated.sql"), toSql(rows));
  writeFileSync(
    resolve(root, "database/library/manifest.generated.json"),
    JSON.stringify({ generatedAt: new Date().toISOString(), ...stats(rows), slugs: rows.map((r) => r.slug) }, null, 2),
  );
