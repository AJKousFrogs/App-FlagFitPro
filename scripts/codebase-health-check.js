...
class CodebaseHealthCheck {
  ...
  async scanServiceFiles() {
    ...
    try {
      ...
      const jsFiles = files.filter(file => file.endsWith('.js') && !file.includes('.backup'));
  ...
  async analyzeFile(filePath, fileType) {
    try {
      ...
      // Extract functions
      const functionMatches = content.match(/(?:async\s+)?(?:function\s+)?(\w+)\s*\([^)]*\)\s*\{|(\w+)\s*:\s*(?:async\s+)?(?:function\s*)?\([^)]*\)\s*=>/g);
      if (functionMatches) {
        functionMatches.forEach(match => {
          ...
          if (funcName && !this.isCommonName(funcName)) {
            ...
            this.functions.get(funcName).push({ file: filename, type: fileType, line: this.getLineNumber(content, match) });
          }
        });
      }
      ...
      if (importMatches) {
        importMatches.forEach(match => {
          ...
          this.imports.get(importPath).push({ file: filename, statement: match });
        });
      }

      // Extract exports
      const exportMatches = content.match(/export\s+(?:default\s+)?(?:class\s+)?(\w+)|module\.exports\s*=\s*(\w+)/g);
      if (exportMatches) {
        exportMatches.forEach(match => {
          ...
          if (exportName) {
            ...
            this.exports.get(exportName).push({ file: filename, statement: match });
          }
        });
      }
  ...
  async checkFileSpecificIssues(content, filename, fileType) {
    ...
    if (content.includes('JSON.parse') && content.includes('await') && content.includes('map(')) {
      performanceIssues.push('Potential memory issues with large data processing');
    }
  ...
  generateSummary() {
    const totalIssues = Object.values(this.issues).reduce((sum, issueArray) => sum + issueArray.length, 0);
  ...
  generateRecommendations() {
    ...
    if (this.issues.duplicateFunctions.length > 0) {
      const highSeverityDups = this.issues.duplicateFunctions.filter(d => d.severity === 'high');
  ...
  displayReport(report) {
    ...
    if (report.detailedFindings.databaseConnectionDuplication.length > 0) {
      ...
      report.detailedFindings.databaseConnectionDuplication.forEach(dbIssue => {
  ...
  isCommonName(name) {
    const commonNames = ['constructor', 'render', 'init', 'setup', 'destroy', 'start', 'stop'];
