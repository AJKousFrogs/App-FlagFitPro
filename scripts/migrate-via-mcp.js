...
async function migrateViaMCP() {
  ...
  const supabaseMigrations = migrationFiles.filter(
    (f) => f.type === "supabase",
  );
  const databaseMigrations = migrationFiles.filter(
    (f) => f.type === "database",
  );
  ...
  const reportPath = path.join(
    PROJECT_DIR,
    "database/migration_results/mcp_migration_report.json",
  );
  ...
  console.log();
  console.log(
    "The migrations have been prepared. To apply them using MCP tools:",
  );
  console.log();
  console.log(
    "1. Use the MCP Supabase 'apply_migration' tool for each migration",
  );
