...
async function checkConsistency(athleteId = null) {
  ...
  console.log(
    `Scope: ${athleteId ? `Athlete ${athleteId}` : "All Athletes"}\n`,
  );
  ...
  try {
    ...
    let query = supabase
      ...
      .select(
        "id, user_id, athlete_id, session_date, date, duration_minutes, rpe, intensity_level",
        { count: "exact" },
      );
    ...
    if (tsError) {
      ...
      const withRPE =
        trainingSessions?.filter((s) => s.rpe !== null && s.rpe !== undefined)
          .length || 0;
      const withDuration =
        ...
        trainingSessions?.filter(
          (s) =>
            ((s.rpe !== null && s.rpe !== undefined) ||
              s.intensity_level !== null) &&
            s.duration_minutes !== null &&
            s.duration_minutes !== undefined,
        ).length || 0;
      const withIntensity =
        trainingSessions?.filter(
          (s) => s.intensity_level !== null && s.intensity_level !== undefined,
        ).length || 0;
      ...
      console.log(
        `   Missing RPE but has intensity: ${withIntensity - withRPE}`,
      );
    ...
    let sessionsQuery = supabase
      .from("sessions")
      .select("id, athlete_id, date, duration_minutes, rpe", {
        count: "exact",
      });
    ...
    if (sError) {
      ...
      const withRPE =
        sessions?.filter((s) => s.rpe !== null && s.rpe !== undefined).length ||
        0;
      const withDuration =
        sessions?.filter(
          (s) =>
    ...
    let tlmQuery = supabase
      ...
      .select(
        "id, user_id, session_date, session_duration, session_rpe, training_load",
        { count: "exact" },
      );
    ...
    if (tlmError) {
      ...
      const withRPE =
        loadMetrics?.filter(
          (s) => s.session_rpe !== null && s.session_rpe !== undefined,
        ).length || 0;
      const withDuration =
        loadMetrics?.filter(
          (s) =>
      ...
      const withBoth =
        loadMetrics?.filter(
          (s) =>
            s.session_rpe !== null &&
            s.session_rpe !== undefined &&
            s.session_duration !== null &&
            s.session_duration !== undefined,
        ).length || 0;
      const withCalculatedLoad =
        loadMetrics?.filter((s) => {
          if (!s.session_rpe || !s.session_duration) {
            return false;
          }
      ...
      console.log(`   With both: ${withBoth}`);
      console.log(
        `   With correctly calculated training_load: ${withCalculatedLoad}`,
      );
    ...
    if (athleteId) {
      ...
      if (acwrError) {
        ...
        console.log(
          "   Note: compute_acwr function may not exist or may use different table",
        );
      } else {
        ...
        if (acwrTest && acwrTest.length > 0) {
          ...
          console.log(
            `   Sample - Date: ${sample.session_date}, Load: ${sample.load}, ACWR: ${sample.acwr}`,
          );
        }
      }
    } else {
    ...
    const tsMissingRPE =
      (tsCount || 0) -
      (trainingSessions?.filter((s) => s.rpe !== null).length || 0);
    const tsHasIntensity =
      trainingSessions?.filter((s) => s.intensity_level !== null).length || 0;

    if (tsMissingRPE > 0 && tsHasIntensity > 0) {
      console.log(
        `   🔧 ${tsMissingRPE} training_sessions missing RPE but have intensity_level`,
      );
      console.log(
        `      Run: SELECT sync_intensity_to_rpe(); to copy intensity_level to rpe`,
      );
    }

    if (
      (tsCount || 0) -
        (trainingSessions?.filter((s) => s.duration_minutes !== null).length ||
          0) >
      0
    ) {
