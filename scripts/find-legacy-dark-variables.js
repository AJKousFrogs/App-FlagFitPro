...
const LEGACY_PATTERNS = [/--dark-[a-z-]+/gi, /--dark[A-Z][a-zA-Z]*/g];
  ...
  LEGACY_PATTERNS.forEach((pattern) => {
    ...
    for (const match of matches) {
      ...
      issues.push({
        ...
        context: line,
        replacement:
          DESIGN_SYSTEM_MAPPING[variable] || findReplacement(variable),
      });
    }
  });
...
async function main() {
  ...
  // Report
  console.log(
    `Found ${allIssues.length} legacy variable usage(s) across ${Object.keys(issuesByFile).length} file(s)\n`,
  );
