...
if (!databaseUrl) {
  ...
  console.error(
    "   postgresql://postgres.[PROJECT_REF]:[SERVICE_KEY]@aws-0-[REGION].pooler.supabase.com:5432/postgres",
  );
  process.exit(1);
}
...
async function listAllTables() {
  ...
  try {
    ...
    console.log(
      `📊 FOUND ${result.rows.length} TABLES IN YOUR SUPABASE DATABASE\n`,
    );
    console.log("=".repeat(70));

    result.rows.forEach((row, index) => {
      console.log(
        `${(index + 1).toString().padStart(3)}. ${row.table_name.padEnd(40)} (${row.column_count} columns)`,
      );
    });
    ...
    const categories = {
      ...
      Training: result.rows.filter(
        (r) =>
          r.table_name.includes("training") ||
          r.table_name.includes("exercise") ||
          r.table_name.includes("workout"),
      ),
      Analytics: result.rows.filter(
        (r) =>
          r.table_name.includes("analytics") || r.table_name.includes("event"),
      ),
      Communication: result.rows.filter(
        (r) =>
          r.table_name.includes("chat") ||
          r.table_name.includes("notification") ||
          r.table_name.includes("post") ||
          r.table_name.includes("comment"),
      ),
      Wellness: result.rows.filter(
        (r) =>
          r.table_name.includes("wellness") ||
          r.table_name.includes("hydration") ||
          r.table_name.includes("nutrition") ||
          r.table_name.includes("recovery"),
      ),
      Research: result.rows.filter(
        (r) =>
          r.table_name.includes("research") ||
          r.table_name.includes("knowledge"),
      ),
      Other: [],
    };
    ...
    console.error("\n💡 Make sure you have:");
    console.error(
      "   1. DATABASE_URL or SUPABASE_DB_URL set in your .env file",
    );
