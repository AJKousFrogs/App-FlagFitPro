...
async function processSupplements() {
  ...
  const supplements = ['iron', 'creatine', 'protein', 'magnesium', 'vitamin d', 'beta-alanine'];
  ...
  for (const supplement of supplements) {
    ...
    `, [
      ...
      generateSupplementAnswer(supplement, dosageInfo, safetyInfo, absorptionInfo),
    ...
    console.log(`  ✓ Processed ${supplement}: ${articles.rows.length} articles`);
  }
}
...
async function processInjuries() {
  ...
  const injuries = ['ankle_sprain', 'hamstring_strain', 'shoulder_impingement', 'acl_injury'];
...
async function processRecoveryMethods() {
  ...
  for (const method of methods) {
    ...
    console.log(`  ✓ Processed ${method.name}: ${articles.rows.length} articles`);
  }
}
...
// Helper functions
function extractDosageInfo(articles, supplement) {
  // Extract dosage information from articles
  const dosages = [];
  for (const article of articles) {
    const text = `${article.abstract || ''} ${article.key_findings || ''}`.toLowerCase();
    // Simple extraction - can be improved with NLP
    const dosageMatch = text.match(/(\d+)\s*(?:mg|g|mcg)\s*(?:per\s*day|daily)/i);
    if (dosageMatch) {
