...
const results = {
  backendFrontend: { status: 'pending', tests: 0, passed: 0, failed: 0, errors: [] },
  designSystem: { status: 'pending', tests: 0, passed: 0, failed: 0, errors: [] },
...
async function runBackendFrontendTests() {
  try {
    ...
    const { code, stdout, stderr } = await runCommand(
      'npx',
      ['vitest', 'run', 'tests/integration/api-integration.test.js', '--reporter=verbose'],
      rootDir,
    ...
    if (totalMatch) {
      ...
      results.backendFrontend.failed = failedMatch ? parseInt(failedMatch[1]) : 0;
      results.backendFrontend.tests = results.backendFrontend.passed + results.backendFrontend.failed;
    }
...
async function runDesignSystemTests() {
  try {
    ...
    if (!serverRunning) {
      console.log('⚠️  Angular server is not running. Design system tests require the server.');
      console.log('   To run these tests, start the server with: npm run dev:angular');
    ...
    const { code, stdout, stderr } = await runCommand(
      'npx',
      ['playwright', 'test', 'e2e/design-system-compliance.spec.ts', '--reporter=list'],
    ...
    results.designSystem.tests = results.designSystem.passed + results.designSystem.failed;
...
async function runUXTests() {
  try {
    ...
    if (!serverRunning) {
      console.log('⚠️  Angular server is not running. UX tests require the server.');
      console.log('   To run these tests, start the server with: npm run dev:angular');
      console.log('   Then run: cd angular && npx playwright test e2e/ux-inconsistencies.spec.ts\n');
    ...
    const { code, stdout, stderr } = await runCommand(
      'npx',
      ['playwright', 'test', 'e2e/ux-inconsistencies.spec.ts', '--reporter=list'],
      join(rootDir, 'angular'),
...
function generateReport() {
  ...
  const report = {
    summary: {
      ...
      totalTests: results.backendFrontend.tests + results.designSystem.tests + results.ux.tests,
      totalPassed: results.backendFrontend.passed + results.designSystem.passed + results.ux.passed,
      totalFailed: results.backendFrontend.failed + results.designSystem.failed + results.ux.failed,
    },
    tests: {
  ...
  console.log(`   ⏭️  Skipped: ${[results.backendFrontend, results.designSystem, results.ux].filter(r => r.status === 'skipped').length}\n`);

  console.log('📋 DETAILED RESULTS:\n');

  // Backend/Frontend
  console.log('1. Backend + Frontend Integration Inconsistencies:');
  console.log(`   Status: ${results.backendFrontend.status === 'passed' ? '✅ PASSED' : results.backendFrontend.status === 'skipped' ? '⏭️  SKIPPED' : '❌ FAILED'}`);
  console.log(`   Tests: ${results.backendFrontend.tests} (${results.backendFrontend.passed} passed, ${results.backendFrontend.failed} failed)`);
  if (results.backendFrontend.errors.length > 0) {
    console.log(`   Errors: ${results.backendFrontend.errors.length}`);
    results.backendFrontend.errors.slice(0, 3).forEach(err => console.log(`     - ${err}`));
  }
  console.log('');

  // Design System
  console.log('2. UI Design System Inconsistencies:');
  console.log(`   Status: ${results.designSystem.status === 'passed' ? '✅ PASSED' : results.designSystem.status === 'skipped' ? '⏭️  SKIPPED' : '❌ FAILED'}`);
  console.log(`   Tests: ${results.designSystem.tests} (${results.designSystem.passed} passed, ${results.designSystem.failed} failed)`);
  if (results.designSystem.errors.length > 0) {
    console.log(`   Errors: ${results.designSystem.errors.length}`);
    results.designSystem.errors.slice(0, 3).forEach(err => console.log(`     - ${err}`));
  }
  console.log('');

  // UX
  console.log('3. UX Inconsistencies:');
  console.log(`   Status: ${results.ux.status === 'passed' ? '✅ PASSED' : results.ux.status === 'skipped' ? '⏭️  SKIPPED' : '❌ FAILED'}`);
  console.log(`   Tests: ${results.ux.tests} (${results.ux.passed} passed, ${results.ux.failed} failed)`);
  if (results.ux.errors.length > 0) {
    console.log(`   Errors: ${results.ux.errors.length}`);
    results.ux.errors.slice(0, 3).forEach(err => console.log(`     - ${err}`));
  }
  console.log('');
  ...
  return report.summary.totalFailed === 0 && 
         results.designSystem.status !== 'skipped' && 
         results.ux.status !== 'skipped';
}
