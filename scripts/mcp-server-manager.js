...
class MCPServerManager {
    ...
    try {
      ...
      results.forEach((result, index) => {
        ...
        if (result.status === 'fulfilled') {
          console.log(`✅ ${this.servers[serverName].name} started successfully`);
        } else {
          console.error(`❌ ${this.servers[serverName].name} failed to start:`, result.reason);
        }
      });
  ...
  async waitForServerReady(serverName, port, maxAttempts = 10) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      console.log(`⏳ Waiting for ${server.name} to be ready (${attempt}/${maxAttempts})...`);
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
    
    throw new Error(`${server.name} did not become ready within ${maxAttempts} seconds`);
  }
  ...
  showStatus() {
    ...
    Object.entries(this.servers).forEach(([key, server]) => {
      ...
      console.log(`${statusIcon} ${server.name}: ${server.status} (${portInfo})`);
    });
  ...
  async healthCheck() {
    ...
    const healthPromises = Object.entries(this.servers).map(async ([key, server]) => {
      ...
      try {
        ...
        if (response.ok) {
          return { server: server.name, status: 'healthy', port: server.port };
        } else {
          return { server: server.name, status: 'unhealthy', port: server.port };
        }
      } catch (error) {
        return { server: server.name, status: 'unreachable', port: server.port, error: error.message };
      }
    });
    ...
    results.forEach(result => {
      ...
      console.log(`${icon} ${result.server}: ${result.status}${result.port ? ` (port ${result.port})` : ''}`);
      if (result.error) {
