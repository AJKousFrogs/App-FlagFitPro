...
// Expected green theme colors from design tokens
const expectedGreenColors = {
    primary: ['#10c96b', '#0ab85a', '#089949', '#036d35', '#40ccaf', '#70d8c3', '#a0e4d7', '#d0f0eb', '#f0f9f7'],
    secondary: ['#89c300', '#6fa600', '#558400', '#3a5c00', '#a1cf30', '#b9db68', '#d1e7a0', '#e8f3d8', '#f5f9f0'],
    tertiary: ['#cc9610', '#b37700', '#9a5800', '#673c00', '#d6ab40', '#e0c070', '#ead5a0', '#f4ead0', '#faf5e8']
};

// Colors that should NOT be used (non-green theme)
const forbiddenColors = {
    purple: ['#667eea', '#764ba2', '#8b5cf6', '#7c3aed', '#a78bfa', '#6366F1', '#4F46E5', '#4338CA'],
    blue: ['#3b82f6', '#1e40af', '#0ea5e9', '#0284c7', '#0369a1', '#5271FF'],
    pink: ['#ec4899', '#fb7185', '#f43f5e', '#be185d', '#9d174d'],
    oldBlue: ['#1e3a8a', '#0c2d6b']
};

// Required CSS files for design system
...
function checkCSSLinks(content, filePath) {
    ...
    const hasUIDesignSystem = content.includes('ui-design-system.css');
    const hasComprehensive = content.includes('comprehensive-design-system.css');
    
    // Check if it's a main app page (not login/register)
    const isMainPage = !filePath.includes('login') && !filePath.includes('register') && !filePath.includes('reset-password');
    ...
    if (matches.length > 0) {
        matches.forEach(match => {
            ...
            if (!match.includes('var(--') && !match.includes('rgba(0,0,0')) {
                issues.push(`Hardcoded color in inline style: ${match.substring(0, 100)}`);
            }
        });
    }
...
function auditFile(filePath) {
    ...
    const forbiddenIssues = checkForForbiddenColors([...hexColors, ...rgbaColors]);
    ...
    if (!content.includes('theme-switcher.js') && !filePath.includes('login') && !filePath.includes('register') && !filePath.includes('reset-password')) {
...
results.audits.forEach(audit => {
    if (audit.issues.length > 0 || audit.warnings.length > 0) {
        console.log(`\n📄 ${audit.file}`);
        console.log(`   Stats: ${audit.stats.hexColors} hex colors, ${audit.stats.rgbaColors} rgba colors, ${audit.stats.cssVariables} CSS variables`);
