...
async function demoHydrationSystem() {
  ...
  try {
    ...
    ifafResult.rows.forEach((row, index) => {
      ...
      console.log(`   During: ${row.during_game_hydration_ml_per_15min}ml/15min`);
    ...
    trainingResult.rows.forEach((row, index) => {
      ...
      console.log(`   During: ${row.during_training_hydration_ml_per_15min}ml/15min`);
    ...
    creatineResult.rows.forEach((row, index) => {
      ...
      console.log(`   Strength Improvement: ${row.strength_improvement_percentage}%`);
      console.log(`   Flag Football Relevance: ${row.flag_football_relevance_score}/10`);
      console.log('');
    });
    ...
    const statsQueries = [
      { name: 'WADA Prohibited Substances', query: 'SELECT COUNT(*) as count FROM wada_prohibited_substances' },
      { name: 'Hydration Research Studies', query: 'SELECT COUNT(*) as count FROM hydration_research_studies' },
      { name: 'IFAF Protocols', query: 'SELECT COUNT(*) as count FROM ifaf_hydration_protocols' },
      { name: 'Training Protocols', query: 'SELECT COUNT(*) as count FROM training_hydration_protocols' },
      { name: 'Creatine Research', query: 'SELECT COUNT(*) as count FROM creatine_research' },
      { name: 'Beta-Alanine Research', query: 'SELECT COUNT(*) as count FROM beta_alanine_research' },
      { name: 'Caffeine Research', query: 'SELECT COUNT(*) as count FROM caffeine_research' }
    ];
