...
class FilterManager {
    ...
    handleFilterClick(button) {
        ...
        const allButtons = container.querySelectorAll(button.className.split(' ').map(c => '.' + c).join(','));
    ...
    getFilterGroup(container) {
        ...
        if (container.classList.contains('notification-tabs')) return 'notifications';
    ...
    setupOverlayTriggers() {
        ...
        if (notificationTrigger) {
            notificationTrigger.addEventListener('click', () => this.openNotificationOverlay());
        }
        ...
        document.querySelectorAll('.action-btn').forEach(btn => {
            if (btn.textContent.toLowerCase().includes('close')) {
                btn.addEventListener('click', (e) => {
                    e.preventDefault();
                    this.closeOverlay(btn.closest('.search-overlay, .notification-overlay'));
                });
            }
        });

        // Close on overlay background click
        document.querySelectorAll('.search-overlay, .notification-overlay').forEach(overlay => {
    ...
    setupKeyboardNavigation() {
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Escape') {
                const activeOverlay = document.querySelector('.search-overlay.active, .notification-overlay.active');
                if (activeOverlay) {
    ...
    setupAccessibilityAttributes(button) {
        ...
        const container = this.findFilterContainer(button);
        const filterGroup = this.getFilterGroup(container);
        button.setAttribute('aria-label', `Filter by ${button.textContent.trim()} in ${filterGroup}`);
    }
    ...
    updateFilmContent(filterValue, container) {
        ...
        const placeholder = container.querySelector('.film-placeholder') || container.querySelector('.film-card');
        if (placeholder) {
    ...
    updateSchedule(filterValue, container) {
        ...
        const scheduleItems = container.querySelectorAll('.schedule-item, .game-card');
